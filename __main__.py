...
def main():
    ...
    if "--vibe-demo" in sys.argv or "--demo" in sys.argv:
        # Demo modes skip auth validation intentionally
    ...
    # FIX: validate secret before accepting any MCP connections.
    # This hard-fails at startup rather than silently allowing anonymous access.
...
async def _interactive_loop():
    ...
    for name in [
        "vibe_architect", "vibe_code", "vibe_review", "vibe_verify",
    ...
    while True:
        try:
            # FIX: was blocking input() inside async function, which freezes the event loop.
            # Use run_in_executor so the event loop stays responsive during input waits.
            cmd = await loop.run_in_executor(None, lambda: input("vibeserve> "))
            cmd = cmd.strip()
