...
export async function initVibeServeClient(): Promise<Client | null> {
  ...
  if (!pythonCmd) {
    console.error("[MCP Client] Python not found. Install Python 3.10+ and add to PATH.");
    return null;
  }
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('MCP connection timeout (10s)')), 10000))
    ]);
