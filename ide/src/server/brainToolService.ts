...
class BrowserSessionManager {
	...
	private sessions = new Map<
		string,
		{ context: BrowserContext; page: Page; lastActive: number }
	>();
...
const BRAIN_PATH = join(process.cwd(), "ide/packages/deterministic-brain");

export function findBrainDir(): string | null {
	if (existsSync(join(BRAIN_PATH, "main.py"))) return BRAIN_PATH;
	// Fallback: check if running from ide directory
	const altPath = join(process.cwd(), "packages/deterministic-brain");
	if (existsSync(join(altPath, "main.py"))) return altPath;
	return null;
}
...
export async function runDeterministicBrain(
	...
	if (!brainDir) {
		throw new Error(
			`Deterministic brain service is unavailable. Expected at: ${BRAIN_PATH}`,
		);
	}
	...
	return new Promise((resolve, reject) => {
		...
		proc.on("close", (code) => {
			...
			if (code === 0) resolve(stdout);
				...
				reject(
					new Error(stderr || `Deterministic brain exited with code ${code}`),
				);
		});
		proc.on("error", (err) => {
			clearTimeout(timer);
	...
	try {
		...
		for (const stmt of statements) {
			if (stmt.startsWith('new_tab("') || stmt.startsWith("new_tab('")) {
				...
				output +=
					JSON.stringify(
						{ url, title, viewport: size, loadState: "complete" },
						null,
						2,
					) + "\n";
...
export async function runDeterministicBrainSafe(
	options: BrainQueryOptions,
): Promise<{
	success: boolean;
	output: string;
...
export async function runBrowserHarnessSafe(
	options: BrowserCommandOptions,
): Promise<{
	success: boolean;
	output: string;
...
export async function runBrowserE2ETest(
	url: string,
	testScript: string,
): Promise<{
	success: boolean;
