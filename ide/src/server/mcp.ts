...
import { getRepo, searchTrendingRepos } from "../services/githubService.js";
import {
	buildGraph,
	getGraphSummary,
	graphAvailable,
	queryGraph,
} from "../services/graphifyService.js";
...
const TOOL_POLICIES: Record<
	string,
	{ category: ToolCategory; minRole: string }
> = {
...
async function authorizeTool(name: string, args: any): Promise<void> {
	...
	if (!policy)
		throw new Error(`Security Violation: Unrecognized tool "${name}"`);

	// Request-Scoped Identity: Prioritize context passed via args
	const context = args?.__nexus_context || {};
	const actor =
		context.userId || process.env.NEXUS_MCP_IDENTITY || "mcp-stdio-client";
	...
	if (roles.indexOf(role) < roles.indexOf(policy.minRole)) {
		...
		throw new Error(
			`Forbidden: Tool "${name}" requires "${policy.minRole}" role.`,
		);
	}

	// 2. Guardrails Check
	const { validateAction } = useGuardrailsStore.getState();
	const { allowed, reason } = validateAction(
		policy.category as "read" | "write" | "execute" | "network" | "admin",
		JSON.stringify(args),
	);
	if (!allowed) {
		await logAuditEvent({
	...
	tools: [
		{
			name: "search_repos",
			description:
				"Search trending GitHub repos by query. Returns top 20 results with stars, forks, topics.",
			inputSchema: {
				type: "object",
				properties: {
					query: {
						type: "string",
						description: "Search query (e.g. 'AI agent framework')",
					},
					days: {
						type: "number",
		...
		},
		{
			...
			description:
				"Use Gemini AI to analyze synergies and integration opportunities across a list of repos.",
			inputSchema: {
				type: "object",
				properties: {
					repos: {
						...
						items: { type: "string" },
						description:
							"List of repo names to analyze (e.g. ['langchain', 'autogen'])",
					},
				},
				required: ["repos"],
			},
		},
		{
			...
			description:
				"Trigger a Nexus Alpha build pipeline for a list of repositories.",
			inputSchema: {
				type: "object",
				properties: {
					repos: {
		...
		},
		{
			...
			description:
				"Control a headless browser using a Playwright-based DSL. Use for automation, scraping, or web interactions. Isolated incognito context is created per call.",
			inputSchema: {
				type: "object",
				properties: {
					command: {
						type: "string",
						description:
							"DSL statements to execute. Example: new_tab('https://google.com'); wait_for_load(); print(page_info())",
					},
				},
				required: ["command"],
			},
		},
		{
			...
			description:
				"Query the deterministic brain neuro-symbolic AI reasoning engine with 5 lanes: coding, business_logic, agent_brain, tool_calling, cross_domain. Auto-routes by default.",
			inputSchema: {
				type: "object",
				properties: {
					query: {
					...
					lane: {
						type: "string",
						description:
							"Force a specific lane (coding, business_logic, agent_brain, tool_calling, cross_domain). Default: auto-select",
						enum: [
							"coding",
							"business_logic",
							"agent_brain",
					...
					verbose: {
						type: "boolean",
						description: "Show full trace output",
						default: false,
					},
				},
				required: ["query"],
		...
		},
		{
			name: "vibe_check",
			description:
				"Run all VibeCoder quality gates (type safety, lint, security, bundle, deps, structure) and return a scored build report with letter grade (S/A/B/C/D/F). Perfect for vibe coders who want to know their build quality at a glance.",
			inputSchema: {
				type: "object",
				properties: {
					repoCount: {
						type: "number",
						description: "Number of repos in the build (default: 1)",
						default: 1,
					},
					durationMs: {
						type: "number",
						description: "Build duration in milliseconds (default: 0)",
						default: 0,
					},
				},
			},
		},
		{
			name: "vibe_history",
			description:
				"Get VibeCoder build history, benchmark data, improvement trends, and AI-generated optimization suggestions.",
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					...
					mode: {
						type: "string",
						description: "ast (free) or deep (LLM semantic edges)",
						enum: ["ast", "deep"],
						default: "ast",
					},
				},
		...
		},
		{
			name: "graphify_query",
			description:
				"Query the knowledge graph instead of reading entire files. Find functions, modules, and their connections with minimal tokens.",
			inputSchema: {
				type: "object",
				properties: {
					query: {
						type: "string",
						description: "What to search for (function name, module, concept)",
					},
				},
				required: ["query"],
			},
		},
		{
			...
			description:
				"Get a summary of the knowledge graph — top nodes, languages, connection counts. Quick overview without reading the full graph.",
			inputSchema: { type: "object", properties: {} },
		},
		{
			name: "toon_compress",
			description:
				"Compress JSON/YAML/CSV to TOON format (30-60% token reduction). Ideal for large API responses or structured data going to LLMs.",
			inputSchema: {
				type: "object",
				properties: {
					content: {
						type: "string",
						description: "JSON/YAML/CSV content to compress",
					},
					aggressive: {
						type: "boolean",
		...
		},
		{
			name: "toon_stats",
			description:
				"Get TOON token savings statistics — total calls, tokens saved, average reduction.",
			inputSchema: { type: "object", properties: {} },
		},
		{
			name: "sequential_think",
			description:
				"Break down complex tasks into ordered steps using Sequential Thinking MCP. Provides structured reasoning with hypotheses, revisions, and branch tracking.",
			inputSchema: {
				type: "object",
				properties: {
					query: {
						type: "string",
						description:
							"The task or problem to break down into thinking steps",
					},
					steps: {
						type: "number",
						description: "Number of thinking steps to generate (default: 5)",
						default: 5,
					},
					branch: {
						type: "string",
						description:
							"Optional branch name to explore alternative reasoning paths",
					},
				},
				required: ["query"],
			},
...
function generateSequentialThinking(
	query: string,
	totalSteps: number,
	branch?: string,
): {
	...
	const phases = [
		{
			...
			hypothesis:
				"The core requirement can be decomposed into measurable sub-objectives",
			needsVerification: true,
		},
		{
			thought: "Research existing solutions and patterns",
			hypothesis:
				"Prior art exists that can reduce implementation effort and risk",
			needsVerification: true,
		},
		{
			thought: "Design architecture and interfaces",
			hypothesis:
				"A modular design with clear boundaries reduces coupling and improves testability",
			needsVerification: true,
		},
		{
			thought: "Implement core functionality iteratively",
			hypothesis:
				"Test-driven implementation catches regressions early and ensures correctness",
			needsVerification: true,
		},
		{
			thought: "Integration testing and edge case handling",
			hypothesis:
				"Edge cases and boundary conditions are the primary source of defects",
			needsVerification: true,
		},
		{
			thought: "Performance analysis and optimization",
			hypothesis:
				"Measure before optimizing; premature optimization adds complexity without benefit",
			needsVerification: false,
		},
		{
			thought: "Documentation and knowledge transfer",
			hypothesis:
				"Clear documentation reduces onboarding time and prevents misuse",
			needsVerification: false,
		},
		{
			thought: "Deploy, monitor, and iterate",
			hypothesis:
				"Observability prevents silent failures and enables data-driven improvement",
			needsVerification: true,
		},
	];
	...
	switch (name) {
		...
		case "trigger_pipeline": {
			...
			return {
				content: [
					{
						type: "text",
						text: `Pipeline ${result.simulated ? "simulated" : "enqueued"}. ID: ${result.id}`,
					},
				],
			};
		}
		...
		case "browser_automation": {
			...
			try {
				...
				return {
					content: [
						{ type: "text", text: result || "Command executed successfully" },
					],
				};
			} catch (err) {
				return {
					content: [
						{
							type: "text",
							text: `Browser automation error: ${err instanceof Error ? err.message : String(err)}`,
						},
					],
				};
			}
		...
		case "deterministic_brain": {
			const { query, lane, verbose } = args as {
				query: string;
				lane?: string;
				verbose?: boolean;
			};
			try {
				...
				return {
					content: [
						{ type: "text", text: result || "Query processed successfully" },
					],
				};
			} catch (err) {
				return {
					content: [
						{
							type: "text",
							text: `Deterministic brain error: ${err instanceof Error ? err.message : String(err)}`,
						},
					],
				};
			}
		...
		case "vibe_check": {
			const { repoCount = 1, durationMs = 0 } = args as {
				repoCount?: number;
				durationMs?: number;
			};
			...
			if (!score) {
				return {
					content: [
						{
							type: "text",
							text: "No builds scored yet. Run a pipeline to get your first VibeCoder score!",
						},
					],
				};
			}
		...
		case "graphify_build": {
			const { targetPath = ".", mode = "ast" } = args as {
				targetPath?: string;
				mode?: "ast" | "deep";
			};
			...
			if (!fullPath.startsWith(process.cwd())) {
				throw new Error(
					`Security Violation: targetPath "${targetPath}" is outside the project root.`,
				);
			}
			...
			if (!graphAvailable()) {
				return {
					content: [
						{
							type: "text",
							text: "Graphify not installed. Run: pip install graphifyy && graphify install",
						},
					],
				};
			}
		...
		case "graphify_query": {
			...
			if (results.length === 0) {
				return {
					content: [
						{
							type: "text",
							text: `No graph nodes matched "${query}". Build the graph first with graphify_build.`,
						},
					],
				};
			}
			return {
				content: [
					{ type: "text", text: JSON.stringify(results.slice(0, 15), null, 2) },
				],
			};
		}
		...
		case "toon_compress": {
			const { content, aggressive = false } = args as {
				content: string;
				aggressive?: boolean;
			};
		...
		case "sequential_think": {
			const {
				...
				steps = 5,
				branch,
			} = args as { query: string; steps?: number; branch?: string };
