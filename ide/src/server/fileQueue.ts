...
export class FileQueue extends EventEmitter {
	...
	async removeRepeatable(
		...
		this.scheduled = this.scheduled.filter(
			(s) =>
				!(s.name === name && (!opts?.pattern || s.pattern === opts.pattern)),
		);
		await this._save();
	}
	...
	private _trim(): void {
		...
		if (completed.length > maxComplete) {
			this.jobs = this.jobs.filter(
				(j) =>
					j.status !== "completed" ||
					completed.indexOf(j) >= completed.length - maxComplete,
			);
		}
		if (failed.length > 50) {
			this.jobs = this.jobs.filter(
				(j) => j.status !== "failed" || failed.indexOf(j) >= failed.length - 50,
			);
		}
		// Hard cap
		...
		this.pollTimer = setInterval(() => {
			...
			for (const s of this.scheduled) {
				if (now >= s.nextRunAt && !this._firing.has(s.name)) {
					...
					const job = _createJob(s.name, s.data, {
	...
	private _scheduleJob(scheduled: ScheduledJob): void {
		...
		scheduled.timer = setTimeout(() => {
			...
			const job = _createJob(scheduled.name, scheduled.data, {
				repeat: { pattern: scheduled.pattern },
			});
			this.jobs.push(job);
	...
	private async _processJob(queue: FileQueue, job: FileJob): Promise<void> {
		...
		try {
			...
			if (job.attempts < maxAttempts) {
				...
				const delay =
					(job.opts?.backoff?.delay || 5000) * 2 ** (job.attempts - 1);
