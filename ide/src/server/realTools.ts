...
export async function runShellCommand(
	...
	if (perm.action === "deny") {
		...
		throw new Error(
			`Permission Denied: Execution of "${command}" is restricted by policy.`,
		);
	}
	...
	if (perm.action === "ask") {
		...
		if (
			...
			(process.env.NODE_ENV !== "development" && !process.env.AUTO_APPROVE)
		) {
			await logAuditEvent({
				actor: "Nexus Agent",
...
export async function runBuildCommand(): Promise<ShellResult> {
	...
	try {
		...
		console.warn(
			"[BUILD] Dependency install failed:",
