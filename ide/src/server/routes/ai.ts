	...
	for (let attempt = 1; attempt <= retries; attempt++) {
		...
		try {
			...
			if (err.name === "AbortError") {
				console.warn(
					`[Pipeline] Call timed out (${timeoutMs}ms), attempt ${attempt}/${retries}`,
				);
			}
			if (attempt < retries) {
...
function scaffoldProject(
	appDir: string,
	intent: string,
): { packageJson: any; tsconfig: any; viteConfig: string; files: string[] } {
	const files: string[] = [];
...
function eslintCheck(appDir: string): { valid: boolean; errors: string } {
	try {
		...
		return {
			valid: false,
			errors: (err.stdout?.toString() || "").slice(0, 1000),
		};
	}
}
...
function parseManifest(architectOutput: string): FileManifestEntry[] {
	...
	const pathRegex =
		/(?:^|[\s])(src\/[^\s`\])>,"']+\.[a-z]+|components\/[^\s`\])>,"']+\.[a-z]+|pages\/[^\s`\])>,"']+\.[a-z]+|stores\/[^\s`\])>,"']+\.[a-z]+|hooks\/[^\s`\])>,"']+\.[a-z]+|types\/[^\s`\])>,"']+\.[a-z]+|lib\/[^\s`\])>,"']+\.[a-z]+)/gim;
	let match;
...
): Promise<{ status: "green" | "red"; issues: string[] }> {
	const issues: string[] = [];
...
async function executePipeline(
	...
	try {
		...
		pipeline.files = scaffold.files.map((f) => ({
			path: f,
	...
	try {
		const archOutput = await callAIWithRetry({
			systemPrompt:
				"You are a senior software architect. Design detailed technical architectures. Output structured plans with sections: Component Tree, Data Flow, API Routes, Database Schema, File Structure. Use markdown.",
		...
		updateStep(
			1,
			"done",
			`Architecture generated (${archOutput.length} chars)`,
		);
	} catch (err: any) {
	...
	for (let i = 0; i < manifest.length; i++) {
		...
		updateStep(
			3,
			"running",
			`Generating ${file.path} (${i + 1}/${manifest.length})...`,
		);
		const result = await generateFile(
			appDir,
			file,
			ctx.architect_output || "",
			pipeline.files.map((f) => f.path),
		);
		pipeline.files.push({
			path: file.path,
	...
	updateStep(
		3,
		"done",
		`Generated ${greenCount} files (${redCount} with issues)`,
	);
	...
	if (tsResult.valid && esResult.valid) {
		...
		pipeline.files.push({
			path: "_validation",
			status: "red",
			issues: [tsResult.errors, esResult.errors].filter(Boolean),
		});
	}
...
export function registerAIRoutes(app: any) {
	app.post("/api/ai/chat", async (c: any) => {
		...
		const apiKey =
			process.env.OPENCODE_API_KEY || process.env.VITE_OPENCODE_API_KEY;

		if (!apiKey) {
			return c.json({
				response: "No API key configured. Add OPENCODE_API_KEY to .env.local",
			});
		}
		...
		try {
						...
						messages: [
							{
								role: "system",
								content:
									"You are VibeServe, an AI coding assistant that builds production-ready full-stack applications. Respond concisely with actionable plans.",
							},
			...
			if (!res.ok) {
				...
				return c.json({
					response: `API error (${res.status}): ${err.slice(0, 200)}`,
				});
			}
			...
			const content =
				msg?.content || msg?.reasoning_content || "No response from model";
			return c.json({ response: content });
		} catch (err: any) {
			return c.json({
				response: `Failed to connect to OpenCode Go API: ${err.message}`,
			});
		}
	});
						...
						messages: [
							{
								role: "system",
								content:
									"You transform code. Return ONLY the transformed code, no explanations.",
							},
							{
								role: "user",
								content: `Transform this code: "${instruction}"\n\n\`\`\`\n${code}\n\`\`\`\n\nReturn only the transformed code.`,
							},
						],
						max_tokens: 1000,
					}),
	...
	app.post("/api/ai/explain", async (c: any) => {
		...
		return c.json({
			explanation: `This appears to be: ${typeof code === "string" ? code.slice(0, 50) : "code"}...`,
		});
	});
	...
	app.post("/api/pipeline/run", async (c: any) => {
		...
		const pipeline: PipelineState = {
			...
			intent,
			startedAt: Date.now(),
			steps: PIPELINE_STEP_ORDER.map((sid) => ({
				id: sid,
