	...
	try {
		const monaco = (
			window as unknown as {
				monaco?: {
					editor?: {
						getModelMarkers?: (filter?: { owner?: string }) => MonacoMarker[];
					};
				};
			}
		).monaco;
		...
		<div className="flex flex-col h-full">
			...
			>
				...
				<button
					...
					style={{
						background:
							filter === "error" ? "var(--bg-tertiary)" : "transparent",
						color: "var(--error)",
					}}
				>
					Errors ({errorCount})
				...
				<button
					...
					style={{
						background:
							filter === "warning" ? "var(--bg-tertiary)" : "transparent",
						color: "var(--warning)",
					}}
				>
				...
				{infoCount > 0 && (
					<button
						...
						style={{
							background:
								filter === "info" ? "var(--bg-tertiary)" : "transparent",
							color: "var(--info)",
						}}
					>
						Info ({infoCount})
				...
				{filtered.length === 0 ? (
					...
					filtered.map((p) => (
						...
						>
							<span
								style={{
									color: severityConfig[p.severity].color,
									marginTop: "1px",
								}}
							>
								{severityConfig[p.severity].icon}
							</span>
							<div className="min-w-0">
								...
								>
									<span>
										{fileNameFromPath(p.file)}:{p.line}:{p.column}
									</span>
									<span>{p.source}</span>
								</div>
							</div>
