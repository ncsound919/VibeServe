...
export function AgentQueue() {
  const { pipelineSteps, isPipelineRunning, setPipelineRunning, setPipelineSteps, updatePipelineStep } = useAIStore();
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          data.steps?.forEach((step: any) => updatePipelineStep(step.id, { status: step.status, detail: step.detail }));
        ...
        <div className="p-3 space-y-1">
          {pipelineSteps.map((step) => (
            <div key={step.id} className="flex items-center gap-2 py-1">
              <div
                ...
                style={{
                  background:
                    ...
                    step.status === 'error' ? 'var(--error)' : 'var(--text-muted)',
              ...
              {step.status === 'done' && <span style={{ color: 'var(--success)', fontSize: '10px' }}>✓</span>}
              {step.status === 'running' && <span style={{ color: 'var(--accent)', fontSize: '10px' }}>...</span>}
              {step.status === 'error' && <span style={{ color: 'var(--error)', fontSize: '10px' }}>✗</span>}
            </div>
          ))}
        </div>
      )}
