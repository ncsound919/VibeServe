...
import { generateQualityReport, getQualityTrends, getQualityHistory, saveQualityRun } from './services/qualityScoringService';
import { getWikiStats } from './services/llmWikiService';
import type { PipelineExecution } from './types';

const LOOPS = parseInt(process.argv.find(a => a.startsWith('--loops='))?.split('=')[1] ?? '3');
...
async function benchmarkPipelineLoops(): Promise<RunResult[]> {
  ...
  for (let run = 1; run <= LOOPS; run++) {
    ...
    try {
      exec = await runAutomatedPipeline('VibeServe IDE', (e) => {
        if (e.currentStep !== lastPhase && e.progress > 0) {
          const now = Date.now();
          phases.push({ phase: e.currentStep, time: now - lastTime, passed: e.status !== 'failed' });
          lastTime = now;
      ...
      errors.push(`Run ${run} crashed: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    console.log(`  Done: ${(duration / 1000).toFixed(1)}s | Status: ${exec.status.toUpperCase()} | Quality: ${quality.overall}% (${quality.overallGrade})`);
  }
...
async function benchmarkMcpTools(): Promise<McpToolStats[]> {
  ...
  for (const toolName of testTools) {
    ...
    try {
      await callMcpTool(toolName, toolName === 'vibe_health' ? {} :
        toolName === 'vibe_architect' ? { intent: 'test' } :
        toolName === 'vibe_verify' ? { files: [{ path: 'test.ts', content: 'const x = 1;', language: 'typescript' }] } :
        toolName === 'vibe_compress' ? { data: { test: true } } :
        toolName === 'vibe_benchmark' ? { iterations: 1 } :
        {});
      stats.push({ name: toolName, available: true, responseTime: Date.now() - start });
    } catch (err) {
      stats.push({ name: toolName, available: false, responseTime: Date.now() - start, error: err instanceof Error ? err.message : String(err) });
    }
  }
  ...
  for (const s of stats) {
    ...
    const time = s.available ? `${s.responseTime}ms` : (s.error ?? 'unavailable');
...
async function benchmarkApiEndpoints(): Promise<ApiEndpointStats[]> {
  ...
  console.log(`  Results: ${passed.length}/${stats.length} OK, ${failed.length} failed`);

  for (const s of stats) {
    const icon = s.statusCode >= 200 && s.statusCode < 400 ? '✅' : '❌';
    console.log(`  ${icon} ${s.method.padEnd(4)} ${s.endpoint.padEnd(32)} ${s.statusCode} | ${s.responseTime}ms | ${s.bodySize}B`);
  }
...
function analyzeResults(runs: RunResult[], mcpStats: McpToolStats[], apiStats: ApiEndpointStats[]): { insights: string[]; recommendations: string[] } {
  ...
  insights.push(`Average quality: ${avgScore}% over ${runs.length} runs (${trend})`);
  ...
  // E2E trends
  const avgE2e = Math.round(runs.reduce((s, r) => s + r.e2ePassRate, 0) / runs.length * 100);
  ...
  // API health
  const apiHealthy = apiStats.filter(s => s.statusCode >= 200 && s.statusCode < 400).length;
  ...
  // Low quality
  if (avgScore < 70) {
    const lowestDimension = runs.map(r => ({
    ...
    if (lowestDimension.audit <= lowestDimension.review) {
      recommendations.push('Audit quality is weakest dimension — improve security scanning and test coverage');
    }
    if (lowestDimension.review <= lowestDimension.audit) {
      recommendations.push('Review quality is weakest — add more static analysis rules and coverage checks');
    }
  }

  // Build failures
  if (buildSuccess < runs.length) {
    recommendations.push('Fix intermittent build failures — check dependency resolution and lockfile consistency');
  }

  // E2E issues
  if (avgE2e < 50) {
    recommendations.push(`E2E tests only ${avgE2e}% passing — update test mocks to match current API responses`);
  }

  // MCP gaps
  if (mcpAvailable < mcpTotal) {
    recommendations.push(`Fix ${mcpTotal - mcpAvailable} broken MCP tools — check VibeServe server logs`);
  }

  // Performance
  if (avgDuration > 60) {
    recommendations.push('Pipeline exceeds 60s — consider parallelizing phases (Security + Static Analysis can run concurrently)');
  }

  // Learning loop
  if (totalErrors > 0) {
    recommendations.push('Errors detected across runs — enable auto-fix loop to self-heal');
  }
...
async function main() {
  ...
  console.log(`║   Loops: ${LOOPS} | Mode: ${QUICK ? 'quick' : 'full (MCP + API)'}                        ║`);
  console.log('╚════════════════════════════════════════════════════════╝');
  ...
  // Phase 5: Analysis
  const { insights, recommendations } = analyzeResults(pipelineRuns, mcpTools, apiEndpoints);
  ...
  for (const r of pipelineRuns) {
    console.log(`  ${String(r.run).padEnd(4)} | ${(r.duration / 1000).toFixed(1).padEnd(6)}s | ${r.status.padEnd(7)} | ${String(r.quality.overall).padEnd(3)}% (${r.quality.overallGrade}) | ${Math.round(r.e2ePassRate * 100).toString().padEnd(3)}%  | ${r.buildPassed ? '✅' : '❌'}`);
  }
  console.log('─'.repeat(70));

  console.log('\n📈 Quality Trends:');
  const qt = qualityTrends;
  console.log(`  Pipeline: ${qt.pipelineTrend} | Review: ${qt.reviewTrend} | Audit: ${qt.auditTrend}`);
  ...
  console.log('\n🌐 API Health:');
  const apiOk = apiEndpoints.filter(e => e.statusCode >= 200 && e.statusCode < 400);
  const apiFail = apiEndpoints.filter(e => e.statusCode < 200 || e.statusCode >= 400);
  ...
  console.log(`  Pipeline:  ${q.pipeline.score}% (${q.pipeline.grade})  Completion: ${q.pipeline.dimensions.completion}%`);
  console.log(`  Review:    ${q.review.score}% (${q.review.grade})    Coverage: ${q.review.dimensions.coverage}%`);
  console.log(`  Audit:     ${q.audit.score}% (${q.audit.grade})       Security: ${q.audit.dimensions.securityPass}%`);
  console.log(`  ──────────────────────────`);
  console.log(`  OVERALL:   ${q.overall}% (${q.overallGrade})`);
}

main().catch(err => {
  console.error('\nBenchmark engine crashed:', err instanceof Error ? err.message : String(err));
  process.exit(1);
});
