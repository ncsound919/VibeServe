...
const STATUS_DOT: Record<StepStatus, string> = {
	...
	active:
		"w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse ring-2 ring-emerald-500/30",
							...
							<button
								...
								className={cn(
									...
									selectedStep === step.id &&
										"bg-white/5 ring-1 ring-emerald-500/20 text-white",
									step.status === "active" && "border",
									step.status === "pending" &&
										selectedStep !== step.id &&
										"text-[#4a4b50] hover:text-[#8E9299] hover:bg-[#151619]",
							...
							>
								...
								<div className="flex-1 min-w-0 text-left">
									<div className="flex items-center gap-2">
										<span className="text-xs font-mono truncate hidden lg:block">
											{step.label}
										</span>
										{step.status === "active" && (
											<span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 hidden lg:inline">
												NOW
											</span>
										)}
									</div>
								...
								{hasSubSteps && (
									<span className="ml-auto hidden lg:block">
										{isExpanded ? (
											<ChevronDown size={12} className="text-[#4a4b50]" />
										) : (
							...
							<AnimatePresence>
								{step.status === "active" && step.contextualTools && (
									<motion.div
										...
										className="ml-9 space-y-0.5 mt-0.5 hidden lg:block"
									>
							...
							<AnimatePresence>
								{isExpanded && hasSubSteps && (
									<motion.div
										...
										animate={{ opacity: 1, height: "auto" }}
										exit={{ opacity: 0, height: 0 }}
										className="ml-9 border-l border-[#1a1b1e] pl-3 space-y-0.5 mt-0.5 hidden lg:block"
									>
										{step.subSteps!.map((sub) => (
											...
											>
												...
												{sub.duration && (
													<span className="text-[#4a4b50] ml-auto">
														{sub.duration}
													</span>
												)}
											</div>
				...
				<button
					...
					className={cn(
						...
						activeTab === "Settings"
							...
							: "text-[#4a4b50] hover:text-white hover:bg-[#151619]",
					)}
				>
					<Settings size={18} />
