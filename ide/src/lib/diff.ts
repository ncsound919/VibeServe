...
export function diffObjects<T>(a: T, b: T): Array<{ type: 'CREATE' | 'REMOVE' | 'CHANGE'; path: string[]; value?: any; oldValue?: any }> {
  if (Array.isArray(a) && Array.isArray(b)) {
    return diffArrays(a, b);
  }
  const changes: Array<{ type: 'CREATE' | 'REMOVE' | 'CHANGE'; path: string[]; value?: any; oldValue?: any }> = [];
...
function diffArrays(a: any[], b: any[]): Array<{ type: 'CREATE' | 'REMOVE' | 'CHANGE'; path: string[]; value?: any; oldValue?: any }> {
  const changes: Array<{ type: 'CREATE' | 'REMOVE' | 'CHANGE'; path: string[]; value?: any; oldValue?: any }> = [];
