...
export async function generateText(
	...
	if (process.env.GEMINI_API_KEY) {
		try {
			...
			console.warn(
				`[LLMClient] Gemini returned ${res.status}: ${res.statusText}`,
			);
		} catch (err) {
			console.warn(
				"[LLMClient] Gemini failed:",
	...
	if (process.env.OPENROUTER_API_KEY) {
		try {
			...
			console.warn(
				`[LLMClient] OpenRouter returned ${res.status}: ${res.statusText}`,
			);
		} catch (err) {
			console.warn(
				"[LLMClient] OpenRouter failed:",
				err instanceof Error ? err.message : String(err),
			);
		}
	}
