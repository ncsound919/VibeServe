...
const addMemory = (content: string, tier: MemoryTier, importance = 0.5, tags: string[] = []) => {
  const memory: Memory = {
...
const query = (opts?: { query?: string; tier?: MemoryTier; limit?: number }) => {
...
const getStats = (): MemoryStats => {
  ...
  return {
    ...
    byTier,
    avgImportance: state.memories.length ? totalImportance / state.memories.length : 0,
  };
};
