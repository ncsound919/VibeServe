...
function extractFileFromError(stderr: string): string {
	const match = stderr.match(
		/(?:^|\s)((?:src\/|lib\/|app\/|pages\/|components\/)?[\w\-/]+\.(?:tsx?|jsx?|vue|svelte|css|scss))(?:\(|:|\s|$)/m,
	);
	return match ? match[1] : "";
}

const TS_ERROR_STRATEGIES: Record<
	string,
	{
		strategy: DiagnosisResult["fixStrategy"];
		template: (parts: TsErrorParts) => string;
	}
> = {
	TS2304: {
		strategy: "add_import",
...
- "unknown": cannot determine fix`;
		...
		logger.warn(
			CTX,
			`LLM diagnosis failed: ${err instanceof Error ? err.message : String(err)}`,
		);
		return null;
	}
}
...
function validateStrategy(s: string): DiagnosisResult["fixStrategy"] {
	const valid: DiagnosisResult["fixStrategy"][] = [
		"replace",
		"add_import",
		"add_dep",
		"fix_config",
	...
	return valid.includes(s as "retry" | "add_dep" | "fix_code" | "re_architect")
		? (s as DiagnosisResult["fixStrategy"])
		: "unknown";
}
...
function regexFallbackDiagnosis(
	...
	if (tsErrors.length > 0) {
		...
		if (errorConfig) {
			...
			const actualContent = existsSync(fullPath)
...
export async function diagnoseError(
	...
	logger.info(
		CTX,
		`Diagnosing build error: ${buildError.stderr.slice(0, 120)}`,
	);
	...
	if (llmResult) {
		logger.info(
			CTX,
			`LLM diagnosis: ${llmResult.fixStrategy} — ${llmResult.fixDescription}`,
		);
		return llmResult;
	}
	...
	logger.info(
		CTX,
		`Fallback diagnosis: ${fallback.fixStrategy} — ${fallback.fixDescription}`,
	);
	return fallback;
}
