...
export async function generalAgentNode(state: { query: string }): Promise<{
	...
	try {
		const text = await generateText(state.query, {
			temperature: 0.7,
			maxTokens: 512,
		});
		return { response: text, context: [] };
	} catch {
		return { response: "I could not process that request.", context: [] };
	}
}
