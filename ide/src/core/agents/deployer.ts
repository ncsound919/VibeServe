...
export function checkDeployAvailability(): DeployAvailability {
	...
	try {
		safeExec("docker", ["--version"], { timeout: 5000 });
		avail.docker = true;
	} catch {}

	try {
		safeExec("vercel", ["--version"], { timeout: 5000 });
		avail.vercel = true;
	} catch {}

	try {
		safeExec("netlify", ["--version"], { timeout: 5000 });
		avail.netlify = true;
	} catch {}
...
export async function deploy(config: DeployConfig): Promise<DeployResult> {
	...
	if (!existsSync(safeDir)) {
		return {
			...
			logs: [],
			error: `App directory does not exist: ${safeDir}`,
		};
	}
	...
	switch (config.target) {
		...
		case "netlify":
			return deployNetlify(config);
		case "zip":
			return deployZip(config);
		default:
			return {
				success: false,
				target: config.target,
...
function generateDockerfile(config: DeployConfig): string {
	const hasServer = existsSync(
		path.join(config.appDir, "server", "src", "index.ts"),
	);
	const hasClient = existsSync(
		path.join(config.appDir, "client", "package.json"),
	);
...
function generateVercelConfig(config: DeployConfig): string {
	const isSPA =
		existsSync(path.join(config.appDir, "index.html")) ||
		...
		return JSON.stringify(
			{
				...
				outputDirectory: config.appDir.includes("client")
					? "client/dist"
					: "dist",
				framework: "vite",
...
async function deployDocker(config: DeployConfig): Promise<DeployResult> {
	...
	try {
		...
		if (
			!existsSync(path.join(config.appDir, "nginx.conf")) &&
		...
		try {
			safeExec("docker", ["build", "-t", "nexus-app", "."], {
				cwd: config.appDir,
				timeout: 120000,
			...
			logs.push(
				`[DOCKER] Docker build failed (may be rate-limited or Docker not running): ${msg}`,
			);
		}
...
async function deployVercel(config: DeployConfig): Promise<DeployResult> {
	...
	try {
		...
		try {
			const output = await safeExecLegacy("vercel --prod --yes 2>&1", {
				cwd: config.appDir,
				timeout: 180000,
				maxBuffer: 5 * 1024 * 1024,
			});
			...
			const msg = err instanceof Error ? err.message.slice(0, 200) : "unknown";
			logs.push(
				`[VERCEL] Vercel CLI not authenticated or unavailable. Run \`vercel login\` first.`,
			);
...
async function deployNetlify(config: DeployConfig): Promise<DeployResult> {
	...
	try {
		...
		try {
			await safeExecLegacy("netlify deploy --prod --dir=dist 2>&1", {
				cwd: config.appDir,
				timeout: 180000,
				maxBuffer: 5 * 1024 * 1024,
			});
			...
			const msg = err instanceof Error ? err.message.slice(0, 200) : "unknown";
			logs.push(
				"[NETLIFY] Netlify CLI not authenticated or unavailable. Run `netlify login` first.",
			);
	...
	try {
		...
		if (isWin) {
			safeExec(
				...
				[
					"-Command",
					`Compress-Archive -Path '${config.appDir.replace(/'/g, "''")}\\*' -DestinationPath '${zipPath.replace(/'/g, "''")}' -Force`,
				],
				{ timeout: 60000 },
			);
		} else {
		...
		logs.push(
			`[ZIP] Created ${zipName} (${(stats.size / 1024).toFixed(0)}KB) — ${zipPath}`,
		);
		logs.push(
			"[ZIP] Ready to download — extract and run `npm install && npm run build`",
		);

		return { success: true, target: "zip", logs, zipPath };
	} catch (err) {
		logs.push(
			`[ZIP] Could not create archive: ${err instanceof Error ? err.message.slice(0, 100) : "unknown"}`,
		);
