...
async function fetchAvailableModels(): Promise<string[]> {
	if (cachedModels && Date.now() - cacheTimestamp < CACHE_TTL_MS) {
		logger.debug(CTX, "Using cached model list", {
	...
	try {
		...
		if (!res.ok) {
			throw new Error(
				`Ollama /api/tags returned ${res.status}: ${await res.text().catch(() => "")}`,
			);
		}
		...
		logger.info(CTX, `Detected ${cachedModels.length} Ollama models`, {
			models: cachedModels.slice(0, 10),
		});
		return cachedModels;
	} catch (err) {
		logger.warn(CTX, "Failed to fetch Ollama models", err);
		throw new Error(
			`Ollama is unreachable at ${OLLAMA_HOST}. Make sure Ollama is running.`,
		);
	}
}
...
export async function generateWithOllama(
	...
	try {
		...
		logger.info(CTX, `Ollama response received`, {
			model: data.model,
