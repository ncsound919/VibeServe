...
import type { GeneratedFile, TechStack } from "../types";
...
const reactTsViteTemplate: TemplateDefinition = {
	...
	getFiles: () => [
		...
		},
		{
			path: "src/main.tsx",
			content:
				"import React from 'react';import{createRoot}from'react-dom/client';import{App}from'./App';const r=document.getElementById('root');if(r)createRoot(r).render(<App/>);",
		},
		{
			path: "src/App.tsx",
...
const fullstackTemplate: TemplateDefinition = {
	...
	description:
		"A full-stack React + Vite frontend with Express API and SQLite persistence",
	defaultTechStack: {
		frontend: "react",
		backend: "express",
		database: "sqlite",
	...
	scaffold: async (appRoot: string, spec: AppSpecLike) => {
		...
		// Client package.json
		writeFileSync(
			path.join(appRoot, "client", "package.json"),
			JSON.stringify(
				{
					name: "nexus-client",
					version: "1.0.0",
		...
		writeFileSync(
			path.join(appRoot, "client", "vite.config.ts"),
			`import { defineConfig } from 'vite';
import react from '@vitejs/plugin-react';
export default defineConfig({ plugins: [react()], server: { port: 5173, proxy: { '/api': 'http://localhost:3000' } } });`,
		...
		// Server package.json
		writeFileSync(
			path.join(appRoot, "server", "package.json"),
			JSON.stringify(
				{
					name: "nexus-server",
					version: "1.0.0",
 ...
 */
export function getTemplateForDescription(
	description: string,
): TemplateDefinition {
