...
export async function generatePlan(spec: AppSpec): Promise<ArchitecturePlan> {
	const tpl = (() => {
		...
		if (d.includes("api") && d.includes("fullstack")) return "fullstack";
		if (d.includes("express") || d.includes("rest")) return "express-api";
		if (d.includes("fullstack") || d.includes("saas")) return "fullstack";
		if (d.includes("node") && !d.includes("react")) return "node-ai";
	...
	try {
		...
		if (i >= 0 && j > i) {
			const p = JSON.parse(c.slice(i, j + 1)) as ArchitecturePlan;
			return { ...p, appType: p.appType || tpl };
		}
	} catch {}
	return {
		appType: tpl,
		...
		files,
		dependencies:
			tpl === "react-ts-vite" ? ["react", "react-dom"] : ["express"],
