...
import { type CodingAgentService, codingAgent } from "./codingAgentService";
...
export class RealPipeline {
	...
	async run(spec: AppSpec): Promise<PipelineRunResult> {
		...
		logger.info(
			CTX,
			`Starting real pipeline for: ${spec.description.slice(0, 60)}`,
		);
		...
		if (!phases[0].success) {
			...
			return {
				...
				phases,
				totalDuration,
				summary: "Fatal: Environment setup failed. Cannot continue.",
			};
		}
	...
	private async phaseEnvironmentSetup(appDir: string): Promise<PhaseResult> {
		...
		if (existsSync(appDir)) {
			...
			logs.push(
				`[ENV] Directory contents: ${readdirSync(appDir).length} items`,
			);
		} else {
	...
	private async phaseDependencyResolution(
		appDir: string,
	): Promise<PhaseResult> {
		const logs: string[] = [];
		...
		try {
			...
			logs.push(
				`[DEPS] package.json: ${depCount} deps, ${devDepCount} devDeps`,
			);
			...
			try {
				...
				logs.push(
					...lines.slice(-3).map((l) => `[DEPS] ${l.trim().slice(0, 100)}`),
				);
			...
			logs.push(
				`[DEPS] Error: ${err instanceof Error ? err.message.slice(0, 100) : "unknown"}`,
			);
		}
	...
	private async phaseRAGSync(_appDir: string): Promise<PhaseResult> {
		...
		try {
			const { searchWikiForPhase } = await import(
				"../../services/wikiRetrievalService"
			);
			const wikiContext = searchWikiForPhase("Code Generation");
			if (wikiContext.relevantPages?.length > 0) {
				logs.push(
					`[RAG] Found ${wikiContext.relevantPages.length} relevant wiki page(s)`,
				);
			} else {
	...
	private async phaseMCPResolution(): Promise<PhaseResult> {
		...
		try {
			const { integrationHub: hub } = await import(
	...
	private async phaseStaticAnalysis(appDir: string): Promise<PhaseResult> {
		...
		try {
			const { stdout: output } = await execAsync(
				"npx biome check --no-errors-on-unmatch 2>&1 || true",
				{
					cwd: appDir,
					timeout: 30000,
					encoding: "utf-8",
			...
			try {
				const { stdout: tsOutput } = await execAsync(
	...
	private async phaseBuild(appDir: string): Promise<PhaseResult> {
		...
		if (buildResult.success) {
			...
			if (existsSync(buildResult.outputPath)) {
				...
				logs.push(
					`[BUILD] Output: ${files.length} files, ${(totalSize / 1024).toFixed(0)}KB`,
				);
			}
			return {
				phase: "Build & Compile",
		...
		try {
			...
			logs.push(
				`[AUTOFIX] Diagnosis: ${diagnosis.fixStrategy} — ${diagnosis.fixDescription}`,
			);
			...
			if (fixResult.attempts.length > 0) {
				logs.push(
					`[AUTOFIX] ${fixResult.attempts.length} fix attempt(s) attempted`,
				);
				for (const attempt of fixResult.attempts) {
					logs.push(
						`[AUTOFIX] Attempt #${attempt.attemptNumber}: ${attempt.fixDescription} — ${attempt.success ? "SUCCESS" : "FAILED"}`,
					);
				}
			}
			...
			logs.push(
				`[AUTOFIX] Auto-fix error: ${err instanceof Error ? err.message.slice(0, 150) : String(err)}`,
			);
		}
	...
	private async phaseSecurityAudit(appDir: string): Promise<PhaseResult> {
		...
		try {
			...
			try {
				...
				logs.push(
					`[SECURITY] npm audit: ${counts.critical} critical, ${counts.high} high, ${counts.moderate} mod, ${counts.low} low`,
				);
			} catch {
	...
	private async phaseE2ETesting(appDir: string): Promise<PhaseResult> {
		const logs: string[] = [];
		const hasPlaywright =
			existsSync(path.join(appDir, "playwright.config.ts")) ||
		...
		if (hasPlaywright) {
			try {
				...
				const lines = output.split("\n").filter((l) => l.trim());
				const passed = lines.filter(
					(l) => l.includes("✓") || l.includes("pass"),
				).length;
				const failed = lines.filter(
					(l) => l.includes("✗") || l.includes("fail"),
				).length;
			...
			try {
				...
				if (existsSync(assetsDir)) {
					...
					for (const f of jsFiles)
						jsSize += statSync(path.join(assetsDir, f)).size;
					for (const f of cssFiles)
						cssSize += statSync(path.join(assetsDir, f)).size;

					logs.push(
						`[OPTIMIZE] JS: ${jsFiles.length} files, ${(jsSize / 1024).toFixed(0)}KB`,
					);
					logs.push(
						`[OPTIMIZE] CSS: ${cssFiles.length} files, ${(cssSize / 1024).toFixed(0)}KB`,
					);
					logs.push(
						`[OPTIMIZE] Total: ${((jsSize + cssSize) / 1024).toFixed(0)}KB`,
					);

					if (jsSize > 500 * 1024)
						logs.push(
							"[OPTIMIZE] WARNING: JS bundle over 500KB — consider code splitting",
						);
					if (cssSize > 100 * 1024)
						logs.push(
							"[OPTIMIZE] WARNING: CSS bundle over 100KB — consider purging unused styles",
						);
				} else {
					...
					logs.push(
						`[OPTIMIZE] dist/: ${allFiles.length} files, ${(totalSize / 1024).toFixed(0)}KB`,
					);
				}
			} catch {
	...
	private async phaseFinalizing(
		...
		try {
			const { ingestWikiLearning } = await import(
				"../../services/wikiRetrievalService"
			);
		...
		try {
			...
			logs.push(
				"[DEPLOY] Docker: " + (avail.docker ? "available" : "config only"),
			);
			logs.push(
				"[DEPLOY] Vercel: " + (avail.vercel ? "available" : "config only"),
			);
			logs.push(
				"[DEPLOY] Netlify: " + (avail.netlify ? "available" : "config only"),
			);
