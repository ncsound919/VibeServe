...
export function KeybindingEditor() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    setBindings(b => b.map(bi => bi.command === recording ? { ...bi, key } : bi));
    setRecording(null);
  };

  return (
    <div className="p-3 text-xs" onKeyDown={handleKeyDown} tabIndex={0}>
      <div className="text-[11px] font-semibold uppercase mb-2" style={{ color: 'var(--text-secondary)' }}>Keyboard Shortcuts</div>
      {bindings.map(b => (
        <div key={b.command} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <button
            ...
            style={{ background: recording === b.command ? 'var(--accent)' : 'var(--bg-tertiary)', color: recording === b.command ? 'var(--text-on-accent)' : 'var(--text-muted)' }}
          >
