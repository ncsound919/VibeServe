...
export function MarketplacePanel() {
	...
	const handleInstall = async (id: string) => {
		...
		setExtensions((prev) =>
			prev.map((e) => (e.id === id ? { ...e, scanStatus: "scanning" } : e)),
		);
    ...
    `;
		...
		if (!scanResult.isSafe) {
			setExtensions((prev) =>
				prev.map((e) =>
					e.id === id
						? { ...e, scanStatus: "unsafe", scanWarnings: scanResult.errors }
						: e,
				),
			);
			return;
		...
		setExtensions((prev) =>
			prev.map((e) =>
				e.id === id
					? { ...e, scanStatus: "safe", scanWarnings: scanResult.warnings }
					: e,
			),
		);
		...
		<div className="h-full flex flex-col bg-[#1e1e1e] text-gray-300">
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-3">
				{filtered.map((ext) => (
					<div
						key={ext.id}
						className="bg-[#252526] rounded-lg p-4 hover:bg-[#2a2a2a] transition-colors"
					>
