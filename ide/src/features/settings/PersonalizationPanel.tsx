...
export function PersonalizationPanel() {
	...
	const [activeTab, setActiveTab] = useState<
		"appearance" | "editor" | "behavior" | "shortcuts"
	>("appearance");
	...
	return (
		<div className="h-full flex flex-col bg-[#1e1e1e] text-gray-300">
			...
			<div className="flex border-b border-gray-800">
				{(["appearance", "editor", "behavior", "shortcuts"] as const).map(
					(tab) => (
						<button
							key={tab}
						...
						{prefs.simpleMode && (
							<div className="p-3 bg-purple-900/20 border border-purple-800 rounded">
								<p className="text-sm text-purple-300">
									Simple mode is enabled. Some options are hidden for a simpler
									experience.
								</p>
								<button
					...
					<div className="space-y-4">
						...
						<div>
							...
							<div className="flex items-center gap-3 mt-1">
								<input
									...
									onChange={(e) =>
										updatePref("fontSize", parseInt(e.target.value))
									}
									className="flex-1"
								/>
						...
						<div>
							...
							<select
								value={prefs.tabSize}
								onChange={(e) =>
									updatePref("tabSize", parseInt(e.target.value))
								}
								className="w-full mt-1 bg-[#252526] border border-gray-700 rounded px-3 py-2 text-sm text-white"
							>
						...
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.showMinimap ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.wordWrap ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
					</div>
				...
				{activeTab === "behavior" && (
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.autoSave ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
						...
						{prefs.autoSave && (
							<div>
								...
								<select
									...
									onChange={(e) =>
										updatePref("autoSaveDelay", parseInt(e.target.value))
									}
									className="w-full mt-1 bg-[#252526] border border-gray-700 rounded px-3 py-2 text-sm text-white"
								>
						...
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.notificationsEnabled ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.soundEnabled ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							>
								<div
									className={`w-4 h-4 bg-white rounded-full transition-transform ${prefs.simpleMode ? "translate-x-5" : "translate-x-0.5"}`}
								/>
							</button>
						</div>
					</div>
						...
						<div className="p-3 bg-[#252526] rounded">
							...
							<div className="space-y-2 text-xs">
								...
								<div className="flex justify-between">
									...
									<kbd className="px-2 py-0.5 bg-gray-800 rounded">
										Ctrl+Shift+P
									</kbd>
								</div>
								<div className="flex justify-between">
