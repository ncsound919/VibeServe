...
import { CheckCircle2, Circle, Plus, ScrollText, Trash2 } from "lucide-react";
import { useState } from "react";
import { ProjectRule, useRulesStore } from "../../services/project-rules";
	...
	return (
		<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] p-5 rounded-xl">
						...
						{rules.map((rule) => (
							<div
								...
								className={`p-4 rounded-lg border transition-all ${
									rule.enabled
										? "border-[#2d2e32] bg-[#0a0a0c]/50"
										: "border-[#1a1b1e] bg-[#0a0a0c]/20 opacity-60"
								}`}
							>
								<div className="flex items-start justify-between mb-2">
									<div className="flex items-center gap-2">
										...
										<h3
											className={`text-[11px] font-bold ${rule.enabled ? "text-white" : "text-[#4a4b50]"}`}
										>
											{rule.name}
										</h3>
									</div>
			...
			<div className="mt-4 flex items-center gap-2 text-[9px] font-mono text-[#4a4b50]">
				...
				<span>
					Rules are automatically synchronized with the Serena context engine.
				</span>
			</div>
		</div>
	);
