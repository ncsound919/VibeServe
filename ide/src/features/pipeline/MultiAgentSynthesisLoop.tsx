...
import { Cpu, Share2, Terminal, Users } from "lucide-react";
...
export const MultiAgentSynthesisLoop = ({
	execution,
}: MultiAgentSynthesisLoopProps) => {
	...
	const stages = [
		{
			label: "Agent Collab",
			icon: Users,
			status: execution.progress > 20 ? "COMPLETE" : "ORCHESTRATING",
		},
		{
			label: "Context Sharding",
			icon: Share2,
			status: execution.progress > 40 ? "STABILIZED" : "FRAGMENTING",
		},
		{
			label: "Model Optimization",
			icon: Cpu,
			status: execution.progress > 60 ? "OPTIMIZED" : "PROCESSING",
		},
		{
			label: "MCP Handshake",
			icon: Terminal,
			status: execution.progress > 80 ? "ESTABLISHED" : "NEGOTIATING",
		},
	];

	const isDone = (s: string) =>
		["COMPLETE", "STABILIZED", "OPTIMIZED", "ESTABLISHED"].includes(s);
		...
		>
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				{stages.map((item, i) => (
					<div
						key={i}
						className="bg-[#0a0a0c]/50 p-3 rounded border border-[#2d2e32] flex flex-col gap-2"
					>
						<div
							className={cn(
						...
						<div className="space-y-1">
							<div className="text-[9px] font-mono text-[#8E9299] uppercase">
								{item.label}
							</div>
							<div
								className={cn(
