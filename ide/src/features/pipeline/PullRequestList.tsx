		...
		<div className="mt-8 space-y-4">
			...
			<div className="grid grid-cols-1 gap-3">
				{prs.map((pr) => (
					<div
						key={pr.id}
						className="bg-[#151619] border border-[#2d2e32] rounded-lg overflow-hidden hover:border-purple-500/30 transition-all group"
					>
						<div
							...
							<div className="flex justify-between items-start">
								...
								<div className="flex gap-2">
									<div className="text-[10px] font-mono px-2 py-1 bg-white/5 rounded text-white/40 uppercase">
										{pr.hunks.filter((h) => h.status === "approved").length}/
										{pr.hunks.length} Approved
									</div>
								</div>
							</div>
						</div>
						...
						{expandedPr === pr.id && (
							<div className="border-t border-[#2d2e32] bg-[#0c0d0f] p-4 space-y-4">
								...
								{pr.hunks.map((hunk) => (
									<div
										key={hunk.id}
										className="border border-[#2d2e32] rounded bg-[#151619] overflow-hidden"
									>
										<div className="bg-[#1a1b1e] px-3 py-1.5 flex justify-between items-center border-b border-[#2d2e32]">
											<div className="text-[10px] font-mono text-white/60 truncate max-w-[250px]">
												{hunk.file}
											</div>
											<div className="flex gap-2">
												{hunk.status === "pending" ? (
													<>
														<button
															onClick={(e) => {
																e.stopPropagation();
																handleHunkAction(pr.id, hunk.id, "rejected");
															}}
														...
														<button
															onClick={(e) => {
																e.stopPropagation();
																handleHunkAction(pr.id, hunk.id, "approved");
															}}
													...
													<div
														className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded flex items-center gap-1 ${hunk.status === "approved" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
													>
