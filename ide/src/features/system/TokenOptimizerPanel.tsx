...
import {
	getCostRouter,
	useCostRouter,
} from "../../core/agents/runtime/costRouter";
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="bg-[#252526] rounded-lg p-4">
								...
								<div className="text-2xl font-bold text-white">
									{totalTokenSavings.toLocaleString()}
								</div>
								<div className="text-xs text-gray-500">
									~${estimatedCostSavings.toFixed(4)} value
								</div>
							</div>
						...
						<div className="bg-[#252526] rounded-lg p-4">
							...
							<div className="space-y-2">
								...
								<div className="flex items-center justify-between">
									...
									<span className="text-emerald-400">
										{stats.toonCompressionCount} uses
									</span>
								</div>
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-2">
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<span className="text-white">
										${costRouter.metrics.totalSpend.toFixed(2)}
									</span>
								</div>
								<div className="flex items-center justify-between">
									...
									<span className="text-white">
										{costRouter.metrics.tokensUsed.input.toLocaleString()} /{" "}
										{costRouter.metrics.tokensUsed.output.toLocaleString()}
									</span>
								</div>
								<div className="flex items-center justify-between">
									...
									<span
										className={`px-2 py-0.5 text-xs rounded ${
											costRouter.metrics.budgetMode === "normal"
												? "bg-emerald-900 text-emerald-300"
												: costRouter.metrics.budgetMode === "throttle"
					...
					<div className="space-y-4">
						<div className="bg-[#252526] rounded-lg p-4">
							...
							<p className="text-sm text-gray-400">
								Uses predefined templates to generate boilerplate code (CRUD,
								React components, TypeScript interfaces, tests, etc.) without
								calling LLM. Saves 100% of tokens for applicable patterns.
							</p>
						</div>
						...
						<div className="bg-[#252526] rounded-lg p-4">
							...
							<p className="text-sm text-gray-400">
								Compresses JSON data by replacing verbose keys with
								abbreviations (e.g., "description" → "d", "createdAt" → "ca")
								and using YAML-like format. Reduces token count by 30-60%.
							</p>
						</div>
						...
						<div className="bg-[#252526] rounded-lg p-4">
							...
							<p className="text-sm text-gray-400">
								Instead of reading entire files, queries a compact knowledge
								graph to understand codebase structure. For 1000 LOC, reduces
								from ~500 tokens to ~7 tokens using graph queries.
							</p>
						</div>
						...
						<div className="bg-[#252526] rounded-lg p-4">
							...
							<p className="text-sm text-gray-400">
								Stores responses to repeated prompts in an LRU cache. Subsequent
								identical prompts return cached responses at 0 token cost.
							</p>
						</div>
					</div>
				)}
...
function ToggleSetting({
	...
	return (
		<div className="flex items-center justify-between">
			...
			>
				<div
					className={`w-4 h-4 bg-white rounded-full transition-transform ${enabled ? "translate-x-5" : "translate-x-0.5"}`}
				/>
			</button>
		</div>
	);
