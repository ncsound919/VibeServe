...
export function AgentStatusPanel() {
	...
	const getStatusIcon = (running: boolean, paused: boolean) => {
		...
		if (running)
			return <Activity className="w-3 h-3 text-emerald-400 animate-pulse" />;
		...
		<div className="h-full flex flex-col bg-[#1e1e1e] text-gray-300">
			<div className="flex items-center justify-between px-4 py-2 bg-[#252526] border-b border-gray-800">
				<div className="flex items-center gap-2">
					...
					<span
						className={`px-1.5 py-0.5 text-xs rounded ${agentRuntime.isRunning ? "bg-emerald-900 text-emerald-300" : "bg-gray-700"}`}
					>
					...
					{activeSession && (
						<div className="bg-[#252526] rounded p-3">
							...
							<div className="text-xs text-gray-400">
								...
								{activeSession.toolCalls.length} tool calls
							</div>
						</div>
					)}
				</div>
