...
import {
	...
	useParallelOrchestrator,
} from "../../services/parallelAgentOrchestrator";

const WORKFLOW_TEMPLATES = [
	{
		name: "Multi-Agent Research",
		mode: "fan-out" as ExecutionMode,
		description: "Parallel research across web, code, and docs",
	},
	{
		name: "Build Pipeline",
		mode: "sequential" as ExecutionMode,
		description: "Sequential build: setup → code → test → deploy",
	},
	{
		name: "Code Review",
		mode: "fan-in" as ExecutionMode,
		description: "Parallel review + aggregation",
	},
	{
		name: "Parallel Analysis",
		mode: "parallel" as ExecutionMode,
		description: "Independent parallel tasks with dependencies",
	},
];
...
export function ParallelAgentPanel() {
	...
	const [newWorkflowMode, setNewWorkflowMode] =
		useState<ExecutionMode>("parallel");
	...
	const activeWorkflow = selectedWorkflow
		? orchestrator.getWorkflow(selectedWorkflow)
		: null;
								...
								workflows.map((wf) => (
									...
									>
										<div className="flex items-center justify-between">
											...
											<span
												className={`text-xs px-1.5 py-0.5 rounded ${getStatusColor(wf.status)}`}
											>
												{wf.status}
											</span>
										</div>
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<div className="flex items-center gap-3 mt-1">
										<span
											className={`text-sm px-2 py-0.5 rounded ${getStatusColor(activeWorkflow.status)}`}
										>
								...
								<div className="flex items-center gap-2">
									{activeWorkflow.status === "pending" && (
										<button
											onClick={() =>
												orchestrator.executeWorkflow(activeWorkflow.id)
											}
									...
									{activeWorkflow.status === "running" && (
										<>
											<button
												onClick={() =>
													orchestrator.pauseWorkflow(activeWorkflow.id)
												}
											...
											<button
												onClick={() =>
													orchestrator.cancelWorkflow(activeWorkflow.id)
												}
									...
									{activeWorkflow.status === "paused" && (
										<button
											onClick={() =>
												orchestrator.resumeWorkflow(activeWorkflow.id)
											}
											className="flex items-center gap-1 px-3 py-1.5 bg-emerald-900 text-emerald-300 rounded text-sm hover:bg-emerald-800"
										>
											<Play className="w-4 h-4" />
							...
							<div className="bg-[#252526] rounded-lg p-3">
								...
								<div className="space-y-2">
									{activeWorkflow.tasks.map((task, i) => (
										<div
											key={task.id}
											className="flex items-center gap-3 p-3 bg-[#1e1e1e] rounded"
										>
						...
						<div className="flex items-center justify-center h-full text-gray-500">
							<div className="text-center">
								...
								<p className="text-xs text-gray-600 mt-1">
									Or use a template to create one
								</p>
							</div>
						</div>
					)}
					...
					<div className="bg-[#1e1e1e] rounded-lg w-96 p-4">
						...
						<div className="mb-4">
							...
							<select
								...
								onChange={(e) =>
									setNewWorkflowMode(e.target.value as ExecutionMode)
								}
								className="w-full mt-1 bg-[#252526] border border-gray-700 rounded px-3 py-2 text-white"
							>
