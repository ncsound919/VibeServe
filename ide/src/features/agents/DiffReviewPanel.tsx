...
import {
	...
	type FileChange,
	type PlanStep,
} from "../../services/multi-file-planner";
...
export function DiffReviewPanel({
	...
	const renderDiffLine = (change: any, index: number) => {
		...
		return (
			<div
				key={index}
				className={`flex font-mono text-xs ${isAdd ? "text-emerald-400 bg-emerald-900/20" : isRemove ? "text-red-400 bg-red-900/20" : "text-gray-300"}`}
			>
		...
		<div className="h-full flex flex-col bg-[#1e1e1e] text-gray-300">
			<div className="flex items-center justify-between px-4 py-2 bg-[#252526] border-b border-gray-800">
				<div className="flex items-center gap-2">
					...
					<span className="px-1.5 py-0.5 bg-gray-700 text-xs rounded">
			...
			<div className="flex-1 overflow-y-auto p-3 space-y-2">
				{changes.length === 0 ? (
					...
					changes.map((change) => (
						<div
							key={change.id}
							className="bg-[#252526] rounded-lg overflow-hidden"
						>
							<div
								...
								<div className="px-3 pb-3 border-t border-gray-800">
									<div className="flex items-center gap-2 py-2">
										...
										<span className="text-xs text-gray-500 ml-auto">
											{selectedFiles[change.id]?.size || 0} /{" "}
											{change.files.length} selected
										</span>
									</div>
									...
									<div className="space-y-2">
										{change.files.map((file, idx) => {
											const isSelected =
												selectedFiles[change.id]?.has(file.path) ?? true;
											return (
												<div
													key={idx}
													className={`border rounded ${isSelected ? "border-emerald-700" : "border-gray-700"}`}
												>
													<div
														className="flex items-center justify-between px-2 py-1.5 bg-[#1e1e1e] cursor-pointer"
														onClick={() =>
															toggleFileSelection(change.id, file.path)
														}
													>
														<div className="flex items-center gap-2">
															<input
									...
									{onApply && (
										<button
											onClick={() =>
												onApply(
													change,
													selectedFiles[change.id] ||
														new Set(change.files.map((f) => f.path)),
												)
											}
											disabled={
												!(selectedFiles[change.id]?.size ?? change.files.length)
											}
...
export function useDiffReviewStore() {
	...
	const addChange = (
		change: Omit<PendingChange, "id" | "timestamp" | "selectedFiles">,
	) => {
