	...
	const ServiceIcon = ({
		...
		return (
			<div className="flex items-center gap-3 p-3 rounded-lg bg-[#1a1b1e]">
				<div
					className={`p-2 rounded-lg ${connected ? "bg-emerald-500/20" : hasDeepSeek ? "bg-blue-500/20" : "bg-red-500/20"}`}
				>
					<Icon
						size={20}
						className={
				...
				<div className="flex-1">
					...
					<div
						className={`text-xs ${color === "emerald" ? "text-emerald-400" : color === "blue" ? "text-blue-400" : "text-red-400"}`}
					>
						{label}
					</div>
				</div>
		...
		<div className="bg-[#151619] border border-[#2d2e32] rounded-xl p-6 space-y-6">
			...
			<div className="flex items-center justify-between">
				...
				>
					<RefreshCw
						size={18}
						className={`text-[#8b8b8d] ${loading ? "animate-spin" : ""}`}
					/>
				</button>
			</div>
