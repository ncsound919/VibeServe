...
export const VibeCoderDashboard = () => {
	...
	const checkMutation = useMutation({
		mutationFn: async () => {
			const res = await fetch("/api/vibe/check", {
				...
				body: JSON.stringify({ repoCount: 1 }),
			});
			if (!res.ok) throw new Error("Failed");
			return res.json() as Promise<BuildScore>;
		},
		onSuccess: () =>
			queryClient.invalidateQueries({ queryKey: ["vibe-history"] }),
	});
	...
	return (
		<div data-testid="vibe-dashboard" className="space-y-6">
			...
			{loadingHistory ? (
				<div className="text-center py-8 text-[#4a4b50] font-mono text-xs">
					Loading scores...
				</div>
			) : !score ? (
				<div className="bg-[#151619] border border-[#2d2e32] p-8 rounded-xl text-center">
					...
					<p className="text-xs text-[#8E9299] font-mono mb-4">
						No builds scored yet — run your first pipeline!
					</p>
					<button
				...
				<>
					...
					>
						<div className="flex items-start justify-between">
							<div>
								<div className="text-[10px] font-mono text-white/60 uppercase tracking-widest mb-2">
									VibeCoder Grade
								</div>
								<div
									data-testid="vibe-grade"
								...
								{trend !== 0 && (
									<div
										className={`text-[10px] font-mono mt-2 ${trend > 0 ? "text-emerald-400" : "text-red-400"}`}
									>
										{trend > 0 ? "↑" : "↓"} {Math.abs(trendPercent)}% from last
										build
									</div>
								)}
							</div>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
						{score.gates.map((gate, i) => {
							...
							const pct =
								gate.maxScore > 0
									? Math.round((gate.score / gate.maxScore) * 100)
									: 0;
							return (
								<motion.div
									key={gate.gate}
								...
								>
									<div className="flex items-center justify-between mb-2">
										<div className="flex items-center gap-2">
											<Icon
												size={14}
												className={
													gate.passed ? "text-emerald-400" : "text-[#4a4b50]"
												}
											/>
										...
										<span
											className={`text-xs font-mono font-bold ${gate.passed ? "text-emerald-400" : "text-[#4a4b50]"}`}
										>
									...
									{gate.details.slice(0, 2).map((d, j) => (
										<div
											key={j}
											className="text-[9px] font-mono text-[#4a4b50] mt-2"
										>
											{d}
										</div>
									))}
									{gate.warnings.slice(0, 1).map((w, j) => (
										<div
											key={`w${j}`}
											className="text-[9px] font-mono text-amber-400/80 mt-1 flex items-center gap-1"
										>
					...
					{score.insights.length > 0 && (
						<div className="bg-[#151619] border border-[#2d2e32] p-4 rounded-xl">
							<div className="flex items-center gap-2 mb-3">
								...
								<h4 className="text-[10px] font-mono text-[#4a4b50] uppercase tracking-widest">
							...
							<div className="space-y-2">
								{score.insights.map((insight, i) => (
									<div
										key={i}
										data-testid="vibe-insight"
										className="flex items-start gap-2 text-[10px] font-mono text-[#8E9299]"
									>
										<Sparkles
											size={10}
											className="text-purple-400/50 mt-0.5 flex-shrink-0"
										/>
										<span>{insight}</span>
									</div>
								))}
					...
					{history && history.totalBuilds > 1 && (
						<div className="grid grid-cols-3 gap-4">
							<div
								data-testid="vibe-stat"
								className="bg-[#151619] border border-[#2d2e32] p-3 rounded-xl text-center"
							>
								<Trophy size={14} className="text-amber-400 mx-auto mb-1" />
								<div className="text-sm font-bold text-white font-mono">
									{history.bestScore}/{maxScore}
								</div>
								<div className="text-[9px] text-[#4a4b50] font-mono">
									Best Score
								</div>
							</div>
							<div
							...
							>
								...
								<div className="text-[9px] text-[#4a4b50] font-mono">
									Streak (best: {history.longestStreak}x)
								</div>
							</div>
						</div>
					)}
					...
					{history && history.trends.length > 1 && (
						<div
							data-testid="vibe-trend"
							className="bg-[#151619] border border-[#2d2e32] p-4 rounded-xl"
						>
							<h4 className="text-[10px] font-mono text-[#4a4b50] uppercase tracking-widest mb-3">
								Score Trend
							</h4>
							<div className="flex items-end gap-1.5 h-24">
								{history.trends.map((t, i) => {
									const height = Math.max(
										4,
										Math.round((t.score / maxScore) * 100),
									);
									const isMax =
										t.score === Math.max(...history.trends.map((x) => x.score));
									return (
										<div
											key={i}
										...
										>
											...
											/>
											{i % 4 === 0 && (
												<span className="text-[7px] font-mono text-[#4a4b50] whitespace-nowrap">
													{t.label}
												</span>
											)}
										</div>
