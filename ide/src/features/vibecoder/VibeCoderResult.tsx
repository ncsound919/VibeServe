...
export const VibeCoderResult = ({
	success,
	appPath,
	summary,
	duration,
}: VibeCoderResultProps) => {
	...
	const handleDeploy = async (target: string) => {
		...
		try {
			...
			if (data.success || data.configPath) {
				...
				setDeployMessage(
					data.logs?.slice(-1)[0] || `Config generated for ${target}`,
				);
			} else {
				setDeployStatus("done");
				setDeployMessage(
					data.logs?.slice(-1)[0] ||
						"Config files generated. Run manually to deploy.",
				);
			}
		} catch (err) {
	...
	return (
		...
		>
			...
			<div className="mt-4 pt-4 border-t border-[#1a1b1e]">
				<p className="text-[10px] text-[#6B7280] font-mono uppercase tracking-widest mb-3">
					Deploy
				</p>
				<div className="flex flex-wrap gap-2">
					...
					<button
						...
						onClick={() =>
							window.open(`file://${appPath}`, "_blank", "noopener,noreferrer")
						}
