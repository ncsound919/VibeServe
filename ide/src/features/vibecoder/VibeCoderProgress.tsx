...
export const VibeCoderProgress = ({
	status,
	message,
	logs,
	appPath,
}: VibeCoderProgressProps) => {
	const [showDetails, setShowDetails] = useState(false);
	const currentPhaseIndex = PHASE_ORDER.indexOf(
		status as (typeof PHASE_ORDER)[number],
	);
		...
		<div className="w-full max-w-2xl mx-auto">
						...
						{i < PHASE_ORDER.length - 1 && (
							<div
								className={`w-2 h-2 rounded-full ${
									i < currentPhaseIndex ? "bg-emerald-500" : "bg-[#2d2e32]"
								}`}
							/>
						)}
					</div>
			...
			<div className="flex justify-between text-[10px] font-mono text-[#6B7280] uppercase tracking-widest mb-4">
				{PHASE_ORDER.map((phase, i) => (
					<span
						key={phase}
						className={i <= currentPhaseIndex ? "text-emerald-400" : ""}
					>
						{phase === "generating"
							? "Plan"
							: phase === "building"
			...
			{message && (
				<p className="text-xs font-mono text-[#9CA3AF] mb-4 text-center">
					{message}
				</p>
			)}
					...
					{showDetails && (
						...
						>
							{logs.map((log, i) => (
								<p
									key={i}
									className="text-[10px] font-mono text-[#6B7280] leading-relaxed"
								>
									{log}
								</p>
							))}
