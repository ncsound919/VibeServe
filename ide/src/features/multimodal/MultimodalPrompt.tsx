import { Code, Download, Eye, Image, Upload, Wand2, X } from "lucide-react";
...
export function MultimodalPrompt({ onGenerateCode }: MultimodalPromptProps) {
	...
	const simulateAnalysis = async (url: string) => {
		const analysis: ImageAnalysis = {
			...
			imageUrl: url,
			analysis:
				"Detected a modern UI design with card-based layout, hero section, and navigation bar. Color scheme appears to be dark mode with accent colors.",
			suggestedComponents: [
				"Hero Section",
				"Navigation Bar",
				"Feature Cards",
	...
	return (
		<div className="h-full flex flex-col bg-[#1e1e1e] text-gray-300">
				...
				{images.length === 0 ? (
					<div className="text-center py-8 text-gray-500">
						...
						<p className="text-xs text-gray-600">
							Upload a design mockup and I'll generate UI code
						</p>
					</div>
				) : (
			...
			{selectedImage && (
				...
				>
					<div
						className="bg-[#1e1e1e] rounded-lg max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto"
