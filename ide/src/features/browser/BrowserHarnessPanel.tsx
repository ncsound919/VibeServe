...
import {
	...
	getDefaultCommands,
	runBrowserCommand,
} from "../../services/browserHarnessClient";
...
export const BrowserHarnessPanel = () => {
	...
	return (
		<div className="bg-[#0a0a0c] border border-[#2d2e32] rounded-xl overflow-hidden flex flex-col h-full">
			...
			<div
				ref={scrollRef}
				className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2 font-mono text-[11px]"
			>
				{entries.length === 0 && (
				...
				<AnimatePresence initial={false}>
					{entries.map((entry) => (
						...
						>
							...
							>
								{entry.expanded ? (
									<ChevronDown size={10} className="text-[#4a4b50]" />
								) : (
									<ChevronRight size={10} className="text-[#4a4b50]" />
								)}
								...
								<span className="text-[#8E9299] truncate flex-1">
									$ {entry.command}
								</span>
								<span className="text-[8px] text-[#4a4b50]">
						...
						>
							{Object.entries(presets).map(([name, cmd]) => (
								...
								>
									...
									<span className="text-[9px] text-[#4a4b50] mr-2 uppercase">
										{name}:
									</span>
									{cmd.slice(0, 60)}
									{cmd.length > 60 ? "..." : ""}
