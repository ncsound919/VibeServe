...
interface WorkspaceStore {
	...
	// Projects
	createProject: (
		p: Omit<Project, "id" | "createdAt" | "updatedAt" | "buildHistory">,
	) => string;
...
export const useWorkspaceStore = create<WorkspaceStore>((set, get) => ({
	...
	addWebhook: (w) =>
		set((s) => ({
			webhooks: [...s.webhooks, { ...w, id: `wh-${Date.now()}` }],
		})),

	updateWebhook: (id, updates) =>
		set((s) => ({
			webhooks: s.webhooks.map((w) => (w.id === id ? { ...w, ...updates } : w)),
		})),
