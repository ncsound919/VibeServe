...
import type { FixAttempt, HookConfig, HookResult } from "../types/hooks";
		...
		const connect = () => {
			...
			ws.onmessage = (event) => {
				try {
					...
					const execData =
						msg.type === "pipeline:update" || msg.type === "pipeline_update"
							? msg.execution || msg.exec
							: null;
					if (execData) {
			...
			ws.onclose = () => {
				...
				wsReconnectTimer = setTimeout(
					...
					Math.min(1000 * 2 ** get().wsReconnectAttempts, 30000),
				);
			};
	...
	startPipeline: async (repos) => {
		...
		try {
				...
				set((s) => ({
					activeExecution: s.activeExecution
						? {
								...
								logs: [
									...s.activeExecution.logs,
									`API error ${response.status}: ${errData.slice(0, 200)}`,
								],
							}
						: null,
				}));
			...
			fetch("/api/nexus/track", {
				...
				body: JSON.stringify({
					event: "pipeline-run",
					data: { success: true, repoCount: repos.length },
				}),
			}).catch(() => {});
		} catch (err) {
			...
			set((s) => ({
				activeExecution: s.activeExecution
					? {
							...s.activeExecution,
							status: "failed" as const,
							logs: [...s.activeExecution.logs, `Connection error: ${msg}`],
						}
					: null,
			}));
		}
