...
export const usePipelineProgressStore = create<PipelineProgressState>(
	(set) => ({
		status: "idle",
		phase: "idle",
		progress: 0,
		...
		setPhase: (phase, remainingSteps) =>
			set({ phase, remainingSteps, progress: 0, eta: "" }),
		setProgress: (progress, eta) =>
			set((s) => ({ progress, eta: eta ?? s.eta })),
