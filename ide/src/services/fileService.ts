...
export const fileService = {
  ...
  async deleteFile(path: string): Promise<void> {
    await fetch(`${API_BASE}/delete`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path }) });
  },
  async rename(oldPath: string, newPath: string): Promise<void> {
    await fetch(`${API_BASE}/rename`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ oldPath, newPath }) });
  },
};
