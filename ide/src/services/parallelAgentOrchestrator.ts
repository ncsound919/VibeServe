...
import { type SandboxExecutionResult, sandboxService } from "./sandboxService";
...
export const useParallelOrchestrator = create<OrchestratorStore>(
	(set, get) => ({
		workflows: {},
		pools: {},
				...
				return {
					workflows: {
						...
						[workflowId]: {
							...
							tasks: workflow.tasks.map((t) =>
								t.status === "running"
									? { ...t, status: "failed", error: "Cancelled" }
									: t,
							),
						},
					},
 ...
 */
function getSandboxTypeForWorkflow(
...
async function executeParallel(
	...
	const runTask = async (task: AgentTask): Promise<AgentTask> => {
		updateTaskStatus(workflow.id, task.id, "running", {
			startedAt: Date.now(),
		});
...
export function createBuildPipelineWorkflow(projectName: string): string {
	...
	const stages = [
		{
			name: "Setup Agent",
			input: `Initialize project: ${projectName}`,
			checkpoint: "setup",
		},
