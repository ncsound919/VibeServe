...
const LEVEL_TITLES: Record<number, string> = {
	1: "Nexus Initiate",
	2: "Pipeline Apprentice",
	3: "Build Squire",
	4: "Code Knight",
	5: "Architecture Baron",
	6: "Integration Viscount",
	7: "Autonomy Earl",
	8: "Synthesis Marquis",
	9: "Nexus Duke",
	10: "Alpha Archon",
	15: "Singularity Sage",
	20: "Omniscient Oracle",
};
...
const ALL_ACHIEVEMENTS: Achievement[] = [
	{
		id: "first-pipeline",
		name: "First Light",
		description: "Launch your first pipeline run",
		icon: "Rocket",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	},
	{
		...
		description: "Complete 10 successful pipeline runs",
		icon: "Zap",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	{
		id: "pipeline-streak-5",
		name: "Unstoppable",
		description: "5 consecutive successful pipeline wins",
		icon: "Flame",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "wiki-scribe",
		name: "Wiki Scribe",
		description: "Compile 10 wiki knowledge pages",
		icon: "BookOpen",
		category: "wiki",
		unlocked: false,
		progress: 0,
	...
	},
	{
		...
		description: "Build 100 wiki pages",
		icon: "Library",
		category: "wiki",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "first-agent",
		name: "Agent Smith",
		description: "Deploy your first autonomous agent",
		icon: "Bot",
		category: "agent",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "agent-army",
		name: "Agent Army",
		description: "Deploy 5 active agents",
		icon: "Users",
		category: "agent",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "repo-scanner",
		name: "Trend Spotter",
		description: "Scan trending repos 10 times",
		icon: "Search",
		category: "learning",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "build-king",
		name: "Build King",
		description: "Successfully build 25 repos",
		icon: "Hammer",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "repo-synthesizer",
		name: "Synthesizer",
		description: "Cross-synthesize 3 repos in one pipeline",
		icon: "GitMerge",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "learning-loop",
		name: "Self-Learner",
		description: "Complete 10 autonomous learning iterations",
		icon: "RefreshCw",
		category: "learning",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "failure-phoenix",
		name: "Phoenix Rising",
		description: "Recover from a pipeline failure to win again",
		icon: "Flame",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	},
	{
		id: "failure-streak-3",
		name: "Trial by Fire",
		description: "Survive a 3-pipeline failure streak",
		icon: "AlertTriangle",
		category: "pipeline",
		unlocked: false,
		progress: 0,
	...
	{
		id: "failure-recoveries-5",
		name: "Comeback Kid",
		description: "Recover from 5 separate pipeline failures",
		icon: "RefreshCw",
		category: "pipeline",
		unlocked: false,
		progress: 0,
...
export function trackPipelineRun(
	...
	if (success) {
		...
		p.failureStreak += 1;
		if (p.failureStreak > p.longestFailureStreak)
			p.longestFailureStreak = p.failureStreak;
		p.recentWins.unshift({
			type: "failure",
			message: `Pipeline failed (${repoCount} repo(s))`,
			xp: 0,
			ts: new Date().toISOString(),
		});
		p.trendData.push({
...
function checkAchievements(p: Progression): Achievement[] {
	...
	const check = (ach: Achievement) => {
		...
		switch (ach.id) {
			case "first-pipeline":
				ach.progress = Math.min(p.pipelineRuns, ach.target);
				break;
			case "pipeline-master":
				ach.progress = Math.min(p.pipelineWins, ach.target);
				break;
			case "pipeline-streak-5":
				ach.progress = Math.min(p.pipelineStreak, ach.target);
				break;
			case "wiki-scribe":
				ach.progress = Math.min(p.wikiPages, ach.target);
				break;
			case "wiki-library":
				ach.progress = Math.min(p.wikiPages, ach.target);
				break;
			case "first-agent":
				ach.progress = Math.min(p.agentsDeployed, ach.target);
				break;
			case "agent-army":
				ach.progress = Math.min(p.agentsDeployed, ach.target);
				break;
			case "repo-scanner":
				ach.progress = Math.min(p.reposScanned, ach.target);
				break;
			case "build-king":
				ach.progress = Math.min(p.reposBuilt, ach.target);
				break;
			case "insight-generator":
				ach.progress = Math.min(p.insightsGenerated, ach.target);
				break;
			case "level-5":
				ach.progress = Math.min(p.level, ach.target);
				break;
			case "level-10":
				ach.progress = Math.min(p.level, ach.target);
				break;
			case "daily-streak-7":
				ach.progress = Math.min(p.currentStreak, ach.target);
				break;
			case "repo-synthesizer":
				ach.progress = p.reposBuilt >= 3 ? ach.target : 0;
				break;
			case "learning-loop":
				ach.progress = Math.min(p.learningIterations, ach.target);
				break;
			case "total-xp-1000":
				ach.progress = Math.min(p.totalXp, ach.target);
				break;
			case "total-xp-10000":
				ach.progress = Math.min(p.totalXp, ach.target);
				break;
			case "failure-phoenix":
				ach.progress = p.failureRecoveries >= 1 ? ach.target : 0;
				break;
			case "failure-streak-3":
				ach.progress = Math.min(p.longestFailureStreak, ach.target);
				break;
			case "failure-recoveries-5":
				ach.progress = Math.min(p.failureRecoveries, ach.target);
				break;
		}
...
export function getLearningInsights(): string[] {
	...
	if (p.pipelineWins === 0)
		insights.push("Run your first pipeline to begin learning");
	else if (p.pipelineWins < 5)
		insights.push(
			`Pipeline success rate: ${((p.pipelineWins / Math.max(p.pipelineRuns, 1)) * 100).toFixed(0)}% — keep building!`,
		);
	else
		insights.push(
			`Strong pipeline mastery: ${p.pipelineWins} wins out of ${p.pipelineRuns} runs`,
		);

	if (p.pipelineFailures > 0) {
		insights.push(
			`Pipeline failures: ${p.pipelineFailures} total, longest streak: ${p.longestFailureStreak}, recoveries: ${p.failureRecoveries}`,
		);
		if (p.failureStreak > 2)
			insights.push(
				`⚠️ Current failure streak: ${p.failureStreak} — consider checking integration status`,
			);
	}

	if (p.wikiPages < 5)
		insights.push(
			"Expand the wiki: compile more knowledge pages for deeper context",
		);
	else if (p.wikiPages < 25)
		insights.push(
			`Wiki growing: ${p.wikiPages} pages — knowledge compound effect active`,
		);
	else
		insights.push(
			`Massive wiki: ${p.wikiPages} pages forming a comprehensive knowledge graph`,
		);

	if (p.agentsDeployed === 0)
		insights.push("Deploy an agent to unlock autonomous capabilities");
	else
		insights.push(
			`${p.agentsDeployed} agent(s) active — autonomous coverage expanding`,
		);

	if (p.learningIterations > 0) {
		insights.push(
			`Self-learning active: ${p.learningIterations} loops, ${p.insightsGenerated} insights`,
		);
	}

	const recentWinRate = p.trendData.slice(-5).filter((t) => t.wins > 0).length;
	if (recentWinRate >= 4)
		insights.push("Recent performance: Excellent — on a hot streak!");
	else if (recentWinRate >= 2)
		insights.push("Recent performance: Steady — room to optimize");
