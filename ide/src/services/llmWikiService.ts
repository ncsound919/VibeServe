...
async function callDeepSeek(
	...
	if (!apiKey)
		return `[LLM WIKI] DeepSeek not configured. Simulated output for: ${user.slice(0, 80)}...`;
	try {
...
function parseWikiPage(
	...
	if (tagMatch)
		tags.push(...tagMatch[1].split(/,\s*/).map((t) => t.trim().toLowerCase()));
...
export async function compileWikiPage(
	...
	const llmOutput = await callDeepSeek(
		systemPrompt,
		`Compile into wiki page titled "${pageTitle}": ${rawContent.slice(0, 12000)}`,
	);
	const page = parseWikiPage(
		llmOutput,
		slug,
		`compiled from ${rawFiles.length} sources`,
	);
...
async function updateIndex(page: WikiPage) {
	...
	if (!(await acquireLock())) {
		console.warn("[LLM Wiki] Failed to acquire lock");
		return;
	}
	try {
		...
		if (!existing.includes(`](${page.slug}.md)`))
			fs.writeFileSync(INDEX_FILE, existing + entry, "utf-8");
	} finally {
		releaseLock();
	}
}
...
export async function lintWiki(): Promise<LintResult[]> {
	...
	for (const file of files) {
		...
		const stat = fs.existsSync(path.join(PAGES_DIR, file))
			? fs.statSync(path.join(PAGES_DIR, file))
			: null;
		if (stat) {
			...
			if (ageDays > 7)
				issues.push({
					severity: "warning",
					message: `Content is ${Math.round(ageDays)} days old`,
				});
		}
...
export async function getWikiStats(): Promise<{
	...
	await ensureDirs();
	return {
		pageCount: fs.readdirSync(PAGES_DIR).filter((f) => f.endsWith(".md"))
			.length,
