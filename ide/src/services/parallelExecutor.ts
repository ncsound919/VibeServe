...
export async function executeInParallel<T>(
	...
	logger.info(
		"ParallelExecutor",
		`Starting parallel job "${job.name}" with ${job.workUnits.length} units (max ${maxParallel} parallel)`,
	);
			...
			const workPromise = (async () => {
				try {
					const timeoutPromise = new Promise<never>((_, reject) =>
						setTimeout(
							() => reject(new Error(`Timeout: ${unit.name}`)),
							timeoutMs,
						),
					);
					...
					unit.durationMs =
						new Date(unit.endTime).getTime() -
						new Date(unit.startTime!).getTime();
					completed.push(unit);
	...
	job.status =
		failed.length > 0 && completed.length === 0 ? "failed" : "completed";
	job.completedAt = new Date().toISOString();

	logger.info(
		"ParallelExecutor",
		`Job "${job.name}" done: ${completed.length} completed, ${failed.length} failed`,
	);
...
export function topologicalSort(units: WorkUnit[]): WorkUnit[][] {
