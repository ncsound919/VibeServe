...
async function runCommand(
  ...
  return new Promise((resolve) => {
    execFile(cmd, args, { cwd, timeout, encoding: 'utf-8' }, (err, stdout, stderr) => {
      if (err) {
        const logs: string[] = [];
...
export async function verifySuggestion(
  ...
  const logs: string[] = [`[verifySuggestion] Verifying ${filePath} in ${repoName}`];

  if (!repoRoot) {
    return {
      suggestionId,
      status: 'failing',
      logs: [...logs, 'Could not find project root (no package.json or pyproject.toml)'],
      formatPassed: false,
  ...
  if (isTS) {
    ...
    const fmtResult = await runCommand('npx', ['biome', 'format', '--write', '.'], repoRoot);
    ...
    if (!safePattern) {
      ...
      const testResult = await runCommand(
        'npx', ['vitest', 'run', safePattern, '--passWithNoTests'], repoRoot
      );
    ...
    const fmtResult = await runCommand('python', ['-m', 'ruff', 'format', '--check', '.'], repoRoot);
    formatPassed = fmtResult.passed;
    logs.push(`[format] ${fmtResult.passed ? 'PASS' : 'FAIL'}`);
    ...
    if (!safeTestName || !safeTestDir) {
      ...
      const testResult = await runCommand(
        'python', ['-m', 'pytest', `${safeTestDir}/tests/`, '-k', safeTestName, '--no-cov', '-q'], repoRoot
      );
      testsPassed = testResult.passed;
      logs.push(`[test] ${testResult.passed ? 'PASS' : 'FAIL'}`);
      if (!testResult.passed) logs.push(...testResult.logs.slice(-5));
  ...
  const status = (formatPassed && typecheckPassed && testsPassed) ? 'passing' : 'failing';
