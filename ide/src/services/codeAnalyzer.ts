...
export async function buildCodeGraph(
					...
					if (stats.isDirectory()) {
						...
						const analysis: FileAnalysis = {
							...
							complexity:
								Math.round(
									lines.length *
										(symbols.length / Math.max(1, lines.length)) *
										10,
								) / 10,
						};
	...
	logger.info(
		"CodeAnalyzer",
		`Graph built: ${files.length} files, ${symbolIndex.size} symbols, ${orphanFiles.length} orphans`,
	);
...
export function analyzeComplexity(codeGraph: CodeGraph): ComplexityReport {
	...
	const totalComplexity = complexFiles.reduce(
		(s, f) => s + (f.complexity || 0),
		0,
	);
	const avgComplexity =
	...
	const totalSymbols = codeGraph.files.reduce(
		(s, f) => s + f.symbols.length,
		0,
	);
	...
	if (complexFiles[0]?.complexity && complexFiles[0].complexity > 20) {
		recommendations.push(
			`High complexity in ${complexFiles[0].file} (${complexFiles[0].complexity}) — consider splitting`,
		);
	}
	if (codeGraph.orphanFiles.length > 5) {
		recommendations.push(
			`${codeGraph.orphanFiles.length} orphan files — may indicate dead code`,
		);
	}
