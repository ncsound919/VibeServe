...
export const useRulesStore = create<RulesStore>((set, get) => ({
	rules: [
		...
		},
		{
			...
			name: "Error Handling",
			content:
				"All async functions must have try-catch blocks. Never swallow errors.",
			enabled: true,
			priority: 3,
		},
		{
			...
			content:
				"Use camelCase for variables, PascalCase for classes/interfaces, UPPER_SNAKE for constants.",
			enabled: true,
			priority: 4,
		},
	],
	...
	saveRulesToDisk: async (rootPath) => {
		...
		try {
			...
			writeFileSync(
				join(nexusDir, "rules.json"),
				JSON.stringify(get().rules, null, 2),
			);
		} catch (e) {
...
export function checkCodeAgainstRules(
	code: string,
	rules: ProjectRule[],
): Array<{ rule: string; line: number; message: string }> {
