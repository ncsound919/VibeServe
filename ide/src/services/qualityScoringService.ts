...
export function scorePipelineQuality(exec: PipelineExecution): PipelineQualityScore {
  ...
  const errorLogs = exec.logs.filter(l =>
  ...
  const buildPassed = exec.logs.some(l => l.includes('[BUILD] Build succeeded'));
  const outputQuality = Math.round((e2eRate * 0.6 + (buildPassed ? 0.4 : 0)) * 100);
...
export function scoreReviewQuality(exec: PipelineExecution): ReviewQualityScore {
  // Parse review/audit findings from logs
  const securityLogs = exec.logs.filter(l => l.includes('[SECURITY]'));
  const lintLogs = exec.logs.filter(l => l.includes('[STATIC]') || l.includes('Biome'));
  ...
  const hasStatic = exec.steps.some(s => s.phase === 'Static Analysis' && s.status === 'completed');
  const hasSecurity = exec.steps.some(s => s.phase === 'Security Audit' && s.status === 'completed');
  const coverage = ((hasStatic ? 50 : 0) + (hasSecurity ? 50 : 0));

  // Severity — parse vulnerability counts
  const vulnMatch = securityLogs.find(l => l.includes('2 vulns') || l.includes('1 vuln') || l.includes('0 vuln'));
  const vulnCount = vulnMatch ? parseInt(vulnMatch.match(/\d+/)?.[0] ?? '0') : 0;
  ...
  const completeness = exec.steps
    ...
    .every(s => s.status === 'completed') ? 100 : 50;

  // Consensus — did multiple analysis passes agree?
  const lintPassed = !exec.logs.some(l => l.includes('[STATIC]') && l.includes('error'));
  ...
  if (vulnCount > 0) findings.push({ type: 'vulnerability', count: vulnCount, severity: vulnCount > 1 ? 'high' : 'medium' });
...
export function scoreAuditQuality(exec: PipelineExecution): AuditQualityScore {
  ...
  // Lint pass
  const lintErrors = exec.logs.filter(l => l.includes('[STATIC]') && l.includes('error')).length;
  ...
  if (!buildPassed) recommendations.push('Fix build errors — check vite.config.ts and dependencies');
  if (testPass < 100) recommendations.push(`Improve test coverage — ${e2ePassed}/${e2eTotal} passing`);
  if (vulnCount > 0) recommendations.push(`Address ${vulnCount} security vulnerabilities`);
...
export function generateQualityReport(exec: PipelineExecution): QualityReport {
  ...
  if (pipeline.score < 70) insights.push('Pipeline completion rate needs improvement');
  if (review.score < 70) insights.push('Code review coverage should be expanded');
  if (audit.score < 70) insights.push('Audit quality below threshold — address vulnerabilities');
  if (pipeline.dimensions.resilience < 50) insights.push('High error rate — improve error handling');
  if (pipeline.dimensions.outputQuality < 50) insights.push('Build or test output quality needs attention');
  if (!insights.length) insights.push('All systems nominal — quality above thresholds');
...
export function getQualityTrends(): {
  ...
  if (runs.length < 2) {
    return { pipelineTrend: 'stable', reviewTrend: 'stable', auditTrend: 'stable', averageScore: runs[0]?.report.overall ?? 0, runCount: runs.length };
  }

  const avgScore = Math.round(runs.reduce((s, r) => s + r.report.overall, 0) / runs.length);
  const first = runs[0].report;
  const last = runs[runs.length - 1].report;

  return {
    pipelineTrend: last.pipeline.score > first.pipeline.score ? 'improving' : last.pipeline.score < first.pipeline.score ? 'declining' : 'stable',
    reviewTrend: last.review.score > first.review.score ? 'improving' : last.review.score < first.review.score ? 'declining' : 'stable',
    auditTrend: last.audit.score > first.audit.score ? 'improving' : last.audit.score < first.audit.score ? 'declining' : 'stable',
