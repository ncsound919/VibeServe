...
import {
	runBrowserHarness,
	runDeterministicBrain,
} from "../server/brainToolService";
...
import type { FixAttempt, HookResult } from "../types/hooks";
import {
...
import {
	...
	consultBrainForPhase,
	endBrainSession,
	startBrainSession,
} from "./brainOrchestratorService";
...
import { buildGraph, getGraphSummary, queryGraph } from "./graphifyService";
...
import { generatePrDiff, openPullRequest } from "./prAgentService";
...
import { ingestWikiLearning, searchWikiForPhase } from "./wikiRetrievalService";
...
async function executeWithRetry<T>(
	...
	while (attempts < config.maxRetries) {
		...
		try {
			...
			const backoff = Math.min(
				config.backoffMs * 2 ** (attempts - 1),
				config.maxBackoffMs,
			);
			logger.warn(
				"PipelineRetry",
				`Operation "${operationName}" failed (attempt ${attempts}/${config.maxRetries}). Retrying in ${backoff}ms`,
				{ error: err.message },
			);
			await sleep(backoff);
		}
	...
	throw (
		lastError ??
		new Error(
			`Operation "${operationName}" failed after ${config.maxRetries} attempts`,
		)
	);
}
...
async function executePhaseWithRetry(
	...
	try {
		...
		console.warn(
			`[PIPELINE] Phase "${phaseName}" failed after retries:`,
...
`;
		...
		const brainInsights = await consultBrainForPhase(
			`remediation-${error.type}`,
			[remediationPrompt, ...(execution.sourceRepos || [])],
		);
		const insight =
			brainInsights[0]?.response ||
...
const mcpProbeBreaker = new CircuitBreaker(probeIntegrations, breakerOptions);
const e2eProbeBreaker = new CircuitBreaker(probeIntegrations, breakerOptions);
...
async function runRAGSync(ctx: PhaseContext): Promise<void> {
	...
	const filename = await ingestRaw(`pipeline-${execution.id}`, wikiEntry, {
		pipelineId: execution.id,
	});
	execution.logs = [
		...execution.logs,
		`[WIKI] Ingested pipeline context → raw/${filename}`,
	];
}
...
async function runMCPSync(ctx: PhaseContext): Promise<void> {
	...
	execution.logs = [
		...execution.logs,
		"[MCP] Probing all integration services...",
	];
	const integrations = await mcpProbeBreaker.fire();
	for (const svc of integrations) {
		execution.logs = [
			...execution.logs,
			`[MCP] ${svc.name}: ${svc.status} (${svc.latency}ms)`,
		];
	}
	execution.logs = [
		...execution.logs,
		`[MCP] Integration probe complete (${integrations.filter((i) => i.status === "online" || i.status === "configured").length}/${integrations.length} available)`,
	];

	const pages = await compileAllRaw();
	execution.logs = [
		...execution.logs,
		`[WIKI] Compiled ${pages.length} wiki page(s) from raw sources`,
	];
	for (const p of pages.slice(0, 3)) {
		execution.logs = [
			...execution.logs,
			`[WIKI]   → ${p.title} (${p.slug}.md, ${p.tags.join(", ") || "no tags"})`,
		];
	}
}
...
async function runE2ETests(ctx: PhaseContext): Promise<void> {
	...
	try {
		...
		if (!passed) {
			execution.logs = [
				...execution.logs,
				`[TEST] Test suite failed with code ${testResult.code}`,
			];
		} else {
			execution.logs = [
				...execution.logs,
				`[TEST] Test suite passed successfully.`,
			];
		}
	} catch (error) {
		execution.e2eResults = [
			{
				testName: "Real Test Suite Error",
				status: "failed",
				duration: 0,
	...
	const integrations = await e2eProbeBreaker.fire();
	for (const svc of integrations) {
		let status: "passed" | "failed" | "skipped" = "skipped";
		let detail = "Integration not configured — skipped";
		if (svc.status === "online") {
			status = "passed";
			detail = "Integration reachable";
		} else if (svc.status === "configured") {
			status = "passed";
			detail = "API key configured";
		} else if (svc.status.startsWith("error")) {
			status = "failed";
			detail = `Integration error: ${svc.status}`;
		} else {
			status = "skipped";
			detail = `Integration ${svc.status} — skipped (expected in local dev)`;
		}
		execution.e2eResults = [
			...execution.e2eResults,
			{
				...
				logs: [
					`${svc.name} health check: ${svc.status} (${svc.latency}ms)`,
					detail,
				],
			},
		];
...
async function runLearningIteration(ctx: PhaseContext): Promise<void> {
	...
	execution.logs = [
		...execution.logs,
		`[LEARNING] Invoking VibeServe Self-Learning Loop Iteration...`,
	];
	try {
		...
		if (resp.ok) {
			execution.logs = [
				...execution.logs,
				`[LEARNING] CritiqueLoop iteration complete. Memory updated.`,
			];
		} else {
			execution.logs = [
				...execution.logs,
				`[LEARNING] CritiqueLoop fallback: Synthesizing current context...`,
			];
		}
	} catch (err) {
		execution.logs = [
			...execution.logs,
			`[LEARNING] CritiqueLoop iteration skipped: ${err}`,
		];
	}
}
...
async function runOptimizationPhase(ctx: PhaseContext): Promise<void> {
	...
	if (lintResults.length > 0) {
		execution.logs = [
			...execution.logs,
			`[WIKI] Wiki lint found ${lintResults.length} page(s) with issues`,
		];
		for (const r of lintResults.slice(0, 3)) {
			for (const issue of r.issues.slice(0, 2)) {
				execution.logs = [
					...execution.logs,
					`[WIKI]   ${r.page}: [${issue.severity}] ${issue.message}`,
				];
			}
		}
	} else {
		execution.logs = [
			...execution.logs,
			`[WIKI] Wiki lint passed — no broken links or stale content`,
		];
	}
}
...
async function runFinalizingPhase(ctx: PhaseContext): Promise<void> {
	...
	if (
		execution.logs.some((l) => l.includes("[COMPOSER]") || l.includes("[FIX]"))
	) {
		execution.logs.push(
			"[PR] Detected changes — generating Pull Request artifact...",
		);
		try {
			...
			execution.logs.push(
				`[PR] Pull Request generated: ${pr.id} (${pr.diffPath})`,
			);
		} catch (err) {
			execution.logs.push(
				`[PR] Failed to generate PR: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
	}
	...
	try {
		const learnings = [
			...
			"## Key Observations",
			...execution.logs
				.filter((l) => l.includes("[LEARNING]") || l.includes("[BRAIN]"))
				.map((l) => `- ${l}`),
			"",
			"## Automated Fixes & Changes",
			...execution.logs
				.filter((l) => l.includes("[PR]") || l.includes("[FIX]"))
				.map((l) => `- ${l}`),
			"",
			"## Performance Metrics",
		...
		const learningFile = await ingestRaw(
			`learning-${execution.id}`,
			learnings,
			{
				pipelineId: execution.id,
		...
		execution.logs.push(
			`[MEMORY] Synthesis saved to wiki: raw/${learningFile}`,
		);
	} catch (err) {
...
async function probeIntegrations(): Promise<
...
> {
	...
	try {
		...
		for (const [displayName, key] of entries) {
			...
			const envKey =
				key === "nanobot"
					? "ANTHROPIC_API_KEY"
					: key === "Qdrant" || key === "qdrant"
						? "QDRANT_URL"
						: `${key.toUpperCase()}_API_KEY`;
			const isConfigured =
				!!process.env[envKey as string] ||
				(key === "qdrant" && !!process.env.QDRANT_URL);
			results.push({
				name: displayName,
				status: connected
					? "online"
	...
	const probes = [
		{
			name: "Qdrant",
			url: process.env.QDRANT_URL || "http://localhost:6333/collections",
		},
...
function detectSourceTypes(repos: string[]): {
	isFolder: boolean;
	sources: string[];
	targetPaths: string[];
} {
...
export async function runAutomatedPipeline(
	...
	const allPhases: BuildStep[] = [
		{
			...
			status: "pending",
			details: "Spinning up worker nodes",
		},
		{
			id: "2",
			phase: "Dependency Resolution",
			status: "pending",
			details: "Installing required modules",
		},
		{
			id: "rag",
			phase: "RAG Context Sync",
			status: "pending",
			details: "Retrieving build intelligence",
		},
		{
			id: "dc",
			phase: "Dead Code Check",
			status: "pending",
			details: "Scanning for dead code and unused dependencies",
		},
		{
			id: "mcp",
			phase: "MCP Context Resolution",
			status: "pending",
			details: "Handshaking with MCP host",
		},
		{
			id: "3",
			phase: "Static Analysis",
			status: "pending",
			details: "Scanning for code smells",
		},
		{
			id: "4",
			phase: "Build & Compile",
			status: "pending",
			details: "Generating optimized bundle",
		},
		{
			id: "5",
			phase: "Security Audit",
			status: "pending",
			details: "Running CVE scan",
		},
		{
			id: "6",
			phase: "E2E Testing",
			status: "pending",
			details: "Validating critical paths",
		},
		{
			id: "learning",
			phase: "Self-Learning Iteration",
			status: "pending",
			details: "Running VibeServe CritiqueLoop",
		},
		{
			id: "7",
			phase: "Optimization",
			status: "pending",
			details: "Edge distribution preparation",
		},
		{
			id: "8",
			phase: "Finalizing",
			status: "pending",
			details: "Completing deployment cycle",
		},
	];
	...
	const execution: PipelineExecution = {
		...
		logs: [
			...
			sourceInfo.isFolder
				? `[SYSTEM] Detected local folder sources — processing uploaded codebase`
				: `[AUTH] Verifying permissions for user cluster...`,
	...
	if (incrementalBuild) {
		execution.logs.push(
			"[SYSTEM] Running incremental build — reduced phase set",
		);
	}
	...
	try {
		...
		execution.logs = [
			...execution.logs,
			"[BRAIN] Orchestration session started",
		];
	} catch {
		execution.logs = [
			...execution.logs,
			"[BRAIN] Orchestration session unavailable",
		];
	}
	...
	try {
		...
		if (unlocked.length > 0) {
			execution.logs.push(
				`[ACHIEVEMENTS] Pipeline features: ${unlocked.join(", ")}`,
			);
		}
		...
		if (suggestions.length > 0) {
			...
			for (const s of suggestions.slice(0, 3)) {
				execution.logs.push(
					`[LEARNING]   • ${s.benchmarkName}: ${s.actionable}`,
				);
			}
		}
	} catch {
	...
	try {
		...
		for (let i = 0; i < steps.length; i++) {
			...
			try {
				...
				const preHooks = await runHooksForPhase(
					`${step.phase}:pre`,
					hooks || [],
					ctx,
				);
				for (const hr of preHooks) {
					execution.logs.push(
						`[HOOK] ${hr.success ? "PASS" : "FAIL"} Pre-hook: ${hr.hookId}`,
					);
					if (hr.output)
						execution.logs.push(
							`[HOOK] Output: ${hr.output.substring(0, 500)}`,
						);
				}
			} catch (err) {
			...
			if (step.phase === "RAG Context Sync") await runRAGSync(ctx);
			else if (step.phase === "Dead Code Check")
				await runDeadCodeCheckPhase(ctx);
			else if (step.phase === "MCP Context Resolution") await runMCPSync(ctx);
			else if (step.phase === "E2E Testing") await runE2ETests(ctx);
			else if (step.phase === "Self-Learning Iteration")
				await runLearningIteration(ctx);
			else if (step.phase === "Optimization") await runOptimizationPhase(ctx);
			else if (step.phase === "Finalizing") await runFinalizingPhase(ctx);
			else if (step.phase === "Security Audit")
				await runSecurityAuditPhase({
					execution,
					sourceRepos,
					targetPath: sourceInfo.targetPaths[0],
				});
			else if (step.phase === "Static Analysis")
				await runStaticAnalysisPhase({
					execution,
					sourceRepos,
					targetPath: sourceInfo.targetPaths[0],
				});
				...
				execution.logs.push(
					"[SYSTEM] Provisioning isolated execution environment...",
				);
				broadcastService.broadcast({
					type: "pipeline:log",
					executionId: execution.id,
					log: "[SYSTEM] Provisioning isolated execution environment...",
				});
			} else if (step.phase === "Dependency Resolution") {
				execution.logs.push(
					"[SYSTEM] Analyzing package.json and resolving dependency graph...",
				);
				broadcastService.broadcast({
					type: "pipeline:log",
					executionId: execution.id,
					log: "[SYSTEM] Analyzing package.json and resolving dependency graph...",
				});
				...
				execution.logs.push(
					buildOk
						...
						: `[BUILD] Build failed (code ${buildResult.code}).`,
					...stdOut,
					...stdErr,
				);
				stdOut.concat(stdErr).forEach((l) =>
					broadcastService.broadcast({
						type: "pipeline:log",
						executionId: execution.id,
						log: l,
				...
				// Fallback to logs
				const templates =
					LOG_TEMPLATES[step.phase as keyof typeof LOG_TEMPLATES];
				if (templates) {
			...
			try {
				const hooks = getHooks(); // Fix Issue #3: Server-side access
				const postHooks = await runHooksForPhase(
					`${step.phase}:post`,
					hooks || [],
					ctx,
				);
				for (const hr of postHooks) {
					execution.logs.push(
						`[HOOK] ${hr.success ? "PASS" : "FAIL"} Post-hook: ${hr.hookId}`,
					);
					if (hr.output)
						execution.logs.push(
							`[HOOK] Output: ${hr.output.substring(0, 500)}`,
				...
				console.warn(
					`[HOOKS] Error running post-hooks for ${step.phase}:`,
					err,
				);
			}
			...
			saveExecutionLog(execution.id, execution.logs).catch((err) =>
				console.warn("[PIPELINE] Failed to persist execution log:", err),
			);
		}
...
export async function runRealTool<T = Record<string, unknown>>(
	tool: "build" | "audit" | "lint" | "test",
): Promise<T> {
	...
	if (!resp.ok) {
		...
		throw new Error(
			`Real tool failed: ${err?.error ?? resp.statusText ?? "unknown error"}`,
		);
	}
