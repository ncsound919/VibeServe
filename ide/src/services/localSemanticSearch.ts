	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (language === "typescript" || language === "javascript") {
			if (trimmed.match(/^(export\s+)?(async\s+)?function\s+\w+/))
				detectedKind = "function";
			else if (trimmed.match(/^(export\s+)?class\s+\w+/))
				detectedKind = "class";
			else if (trimmed.match(/^(export\s+)?(async\s+)?const\s+\w+\s*=/))
				detectedKind = "function";
			...
			else if (trimmed.match(/^(pub\s+)?enum\s+\w+/))
				detectedKind = "interface";
		} else if (language === "go") {
		...
		if (detectedKind) {
			...
			const symbolMatch = trimmed.match(
				/(?:export\s+|async\s+)?(?:function|class|const|def|fn|struct|enum|type)\s+(\w+)/,
			);
			currentChunk = {
				filePath: "",
				language,
		...
		(set, get) => ({
				...
				try {
					...
					set({
						stats: {
							...
							totalSymbols: allChunks.filter(
								(c) => c.symbolKind === "function" || c.symbolKind === "class",
							).length,
			...
			findSymbol: async (name: string, kind?: string) => {
				...
				return allChunks.filter((chunk) => {
					const nameMatch = chunk.symbolName
						.toLowerCase()
			...
			findReferences: async (symbolId: string) => {
				...
				return allChunks.filter(
					(chunk) => chunk.content.includes(symbol) && chunk.id !== symbolId,
				);
			},
