...
export const vitalsService = {
	...
	async getAppReport(appId: string): Promise<{
		...
		const vitals = await this.profileUrl(
			`http://localhost:3000/preview/${appId}`,
		);

		const recommendations = [];
		if (vitals.lcp > 2.0)
			recommendations.push("Optimize hero image loading with priority hints");
		if (vitals.ttfb > 0.5)
			recommendations.push("Enable edge caching for static assets");
		if (vitals.score < 90)
			recommendations.push(
				"Enable Gzip/Brotli compression in the production server",
			);
