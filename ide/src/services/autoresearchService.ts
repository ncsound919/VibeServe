...
import { compileAllRaw, ingestRaw } from "./llmWikiService";
...
async function evaluateWithDeepSeek(
	...
	if (!apiKey) {
		...
		const passCount = exec.e2eResults.filter(
			(r) => r.status === "passed",
		).length;
	...
	const summary = {
		...
		e2eResults: exec.e2eResults.map((r) => ({
			name: r.testName,
			status: r.status,
		})),
	...
	try {
		...
		return {
			...
			improvement: parsed.improvement || "No suggestion",
		};
	} catch {
		const passCount = exec.e2eResults.filter(
			(r) => r.status === "passed",
		...
		return {
			...
			improvement: "Review E2E test configuration for stability",
		};
	}
}
...
export async function runAutonomousLoop(
	...
	for (let i = state.currentIteration; i < state.maxIterations; i++) {
		...
		const resultMd = [
			...
			"## E2E Results",
			...exec.e2eResults.map(
				(r) => `- ${r.testName}: ${r.status} (${r.duration}ms)`,
			),
			"",
			"## Logs",
			...exec.logs.slice(-15).map((l) => `> ${l}`),
		].join("\n");
		ingestRaw(`pipeline-iteration-${i + 1}`, resultMd, {
			iteration: i + 1,
			status: exec.status,
		});
