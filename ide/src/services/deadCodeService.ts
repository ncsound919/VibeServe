...
export async function runKnipCheck(targetPath?: string): DeadCodeReport {
	const cwd = targetPath || process.cwd();
	try {
		const { stdout } = await execAsync("npx --yes knip --production", {
			encoding: "utf-8",
			timeout: 120000,
			cwd,
		});
		const lines = stdout.trim().split("\n").filter(Boolean);
		const unusedDeps = lines
			.filter((l) => l.includes("Unused dependencies") || l.includes("unused"))
			.slice(0, 15);
		const unusedExports = lines
			.filter((l) => l.includes("Unused exports") || l.includes("unreferenced"))
			.slice(0, 15);
		return {
			unusedDeps,
			unusedExports,
		...
		return {
			...
			unusedExports: [],
			totalIssues: 0,
			summary: "Knip not available — skipped",
		};
	}
}

export async function runDeadCodeCheckPhase(ctx: {
	execution: any;
	sourceRepos: string[];
	targetPath?: string;
}): Promise<void> {
	const { execution } = ctx;
	const report = await runKnipCheck(ctx.targetPath);
	...
	if (report.unusedDeps.length > 0) {
		execution.logs = [
			...execution.logs,
			`[DEADCODE] Unused dependencies: ${report.unusedDeps.length}`,
		];
	...
	if (report.unusedExports.length > 0) {
		execution.logs = [
			...execution.logs,
			`[DEADCODE] Unused exports: ${report.unusedExports.length}`,
		];
	...
	if (
		...
		report.summary !== "Knip not available — skipped"
	) {
