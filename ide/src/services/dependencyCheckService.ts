...
export async function runDependencyCheck(
	...
	if (!hasPackageJson) {
		return {
			...
			remediationSuggestions: [
				"No package.json found — skipping dependency check",
			],
		};
	}
	...
	try {
		...
		if (audit.metadata && audit.metadata.vulnerabilities) {
			...
			result.moderateVulnerabilities =
		...
		result.remediationSuggestions.push(
			"npm audit failed — may be network or permissions issue",
		);
	}
	...
	if (result.vulnerabilities > 0) {
		if (result.criticalVulnerabilities > 0) {
			result.remediationSuggestions.push(
				`Run "npm audit fix --force" to fix ${result.criticalVulnerabilities} critical vulnerabilities`,
			);
		}
		if (result.highVulnerabilities > 0) {
			result.remediationSuggestions.push(
				`Run "npm audit fix" to fix ${result.highVulnerabilities} high vulnerabilities`,
			);
		}
	}
	if (result.outdated > 0) {
		result.remediationSuggestions.push(
			`${result.outdated} packages outdated — consider "npm update" or "npx npm-check-updates"`,
		);
	}
	...
	logger.info(
		"DependencyCheck",
		`Check complete: ${result.totalPackages} packages, ${result.vulnerabilities} vulns, ${result.outdated} outdated`,
	);
	return result;
}
