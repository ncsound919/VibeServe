...
function getDefaultRecovery(): RecoveryPattern[] {
	return [
		{
			id: "network-timeout",
			errorPattern: "ECONNREFUSED|ETIMEDOUT|ENOTFOUND",
			category: "network",
			action: "retry",
			successRate: 0,
			successCount: 0,
		...
		{
			id: "npm-install-fail",
			errorPattern: "npm.*install|MODULE_NOT_FOUND",
			category: "dependency",
			action: "retry",
			successRate: 0,
			successCount: 0,
		...
		{
			id: "build-fail",
			errorPattern: "Build failed|compilation.*error",
			category: "build",
			action: "repair",
			successRate: 0,
			successCount: 0,
		...
		{
			id: "security-scan-fail",
			errorPattern: "security.*scan|vulnerability",
			category: "security",
			action: "skip",
			successRate: 0,
			successCount: 0,
		...
		{
			id: "file-not-found",
			errorPattern: "ENOENT|file.*not.*found",
			category: "system",
			action: "repair",
			successRate: 0,
			successCount: 0,
...
export function trackError(
	...
	saveErrors(errors);
	logger.error("ErrorTracker", `[${phase}] ${tracked.message}`, {
		category,
		severity,
		id: tracked.id,
	});
...
export function attemptRecovery(errorId: string): RecoveryAction {
	...
	const pattern = recovery.find((p) =>
		new RegExp(p.errorPattern, "i").test(error.message),
	);
...
function updateRecoveryPattern(
	message: string,
	category: ErrorCategory,
	action: RecoveryAction,
	success: boolean,
	...
	const recovery = loadRecovery();
	const pattern = recovery.find((p) =>
		new RegExp(p.errorPattern, "i").test(message),
	);
	...
	if (pattern) {
		if (success) {
			...
			pattern.successRate =
				(pattern.successCount / pattern.totalAttempts) * 100;
		}
		saveRecovery(recovery);
	}
}
...
export function getErrorStats(): ErrorStats {
	...
	errors.forEach((e) => {
		...
		if (new Date(e.timestamp).getTime() > dayAgo && !e.resolved)
