...
import { compileAllRaw, getWikiStats } from "./llmWikiService";
...
export async function runAutoDreamConsolidation() {
	...
	try {
		...
		logger.info(
			"AutoDream",
			`Pre-consolidation memory: ${statsBefore.pageCount} pages, ${statsBefore.rawCount} raw snippets.`,
		);
		...
		logger.info(
			"AutoDream",
			`Consolidated ${pages.length} memory pages into structured knowledge graphs.`,
		);

		const statsAfter = getWikiStats();
		logger.info(
			"AutoDream",
			`Post-consolidation memory: ${statsAfter.pageCount} optimized pages.`,
		);
