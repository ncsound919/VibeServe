...
function parseGeminiResponse(text: string): GeminiErrorAnalysisResponse | null {
	...
	for (const line of lines) {
		...
		if (/error\s*type/i.test(l))
			result.errorType = l.split(/:\s*/)[1]?.trim() || "";
		else if (/root\s*cause/i.test(l))
			result.rootCause = l.split(/:\s*/)[1]?.trim() || "";
		else if (/file/i.test(l) && !result.affectedFile)
			result.affectedFile = l.match(/[^\s]+\.[a-z]+/i)?.[0];
		else if (/line/i.test(l) && !result.affectedLine)
			result.affectedLine = parseInt(l.match(/\d+/)?.[0] || "0") || undefined;
		else if (/fix|strategy/i.test(l))
			result.fixStrategy = l.split(/:\s*/)[1]?.trim().toLowerCase() || "manual";
		else if (/code|patch/i.test(l) && l.includes("```")) result.fixCode = l;
		else if (/alternative/i.test(l))
			result.alternatives.push(l.split(/:\s*/)[1]?.trim() || l);
		else if (/confidence/i.test(l))
			result.confidence = parseFloat(l.match(/[\d.]+/)?.[0] || "0.5");
	}
  ...
  "confidence": 0.0 to 1.0
		...
		if (parsed) {
			return {
				...
				fixDescription:
					parsed.explanation ||
					`Fix ${category} error in ${phase || "pipeline"}`,
		...
		logger.warn(
			"ErrorAnalyzer",
			`Gemini analysis failed: ${err instanceof Error ? err.message : String(err)}, using fallback`,
		);
	}
...
function mapFixStrategy(strategy: string): ErrorDiagnosis["fixStrategy"] {
	...
	if (s.includes("command") || s.includes("run") || s.includes("exec"))
		return "command";
	if (s.includes("config") || s.includes("setup")) return "config";
	if (s.includes("depend") || s.includes("install") || s.includes("package"))
		return "dependency";
	return "manual";
}
...
function fallbackDiagnosis(
	...
	if (
		lower.includes("type") &&
		(lower.includes("error") || lower.includes("tsc"))
	) {
		return {
			...
			confidence: 0.4,
			alternativeFixes: [
				"Run tsc --noEmit to see full errors",
