...
function parseGraphJson(graphPath: string): KnowledgeGraph | null {
	...
	try {
		...
		const languages = [
			...new Set(nodes.map((n: GraphNode) => n.language).filter(Boolean)),
		];
		return {
			nodes,
			edges,
...
export async function buildGraph(
	...
	if (!graphifyAvailable()) {
		logger.warn(
			"Graphify",
			"graphify not installed — run: pip install graphifyy && graphify install",
		);
		return {
			graph: null,
			wikiPath: "",
	...
	writeFileSync(
		path.join(GRAPH_CACHE, "latest.json"),
	...
	logger.info(
		"Graphify",
		`Graph built: ${nodeCount} nodes, ${edgeCount} edges, ${communityCount} communities`,
	);
...
export function getGraphSummary(): string {
	...
	const topNodes = [...connMap.entries()]
		.sort((a, b) => b[1] - a[1])
		.slice(0, 10);
	for (const [nodeId, count] of topNodes) {
		const node = graph.nodes.find((n) => n.id === nodeId);
		lines.push(
			`  - ${node?.label || nodeId} (${count} connections)${node?.type ? ` [${node.type}]` : ""}`,
		);
	}
