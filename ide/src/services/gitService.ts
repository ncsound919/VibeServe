...
export interface GitStatus {
  path: string;
  status: 'modified' | 'deleted' | 'added' | 'untracked' | 'renamed' | 'unmodified';
  oldPath?: string;
}
...
export const gitService = {
  ...
  async push(): Promise<void> {
    await git.push({ fs, http, dir: WORKSPACE_ROOT, onAuth: () => ({ username: 'token' }) });
  },

  async pull(): Promise<void> {
    await git.pull({
      fs, http, dir: WORKSPACE_ROOT,
      author: { name: 'VibeServe', email: 'vibeserve@local' },
  ...
  async currentBranch(): Promise<string> {
    const branch = await git.currentBranch({ fs, dir: WORKSPACE_ROOT, fullname: false });
    return branch ?? 'HEAD';
  },
  ...
  async diff(oldRef?: string, newRef?: string): Promise<string> {
    ...
    for (const [filepath, HEAD, WORKDIR, STAGE] of matrix) {
      ...
      if (s === 'modified') {
        let oldContent = '';
        try { const b = await git.readBlob({ fs, dir: WORKSPACE_ROOT, oid: await resolveHeadOid(filepath) }); oldContent = Buffer.from(b.blob).toString('utf8'); } catch {}
        let newContent = '';
  ...
  async clone(url: string, targetDir: string): Promise<void> {
    await git.clone({ fs, http, dir: targetDir, url, singleBranch: true, depth: 1 });
  },
...
async function resolveHeadOid(filepath: string): Promise<string> {
  const headOid = await git.resolveRef({ fs, dir: WORKSPACE_ROOT, ref: 'HEAD' });
  const { tree } = await git.readTree({ fs, dir: WORKSPACE_ROOT, oid: headOid });
...
async function diffRefs(oldRef?: string, newRef?: string): Promise<string> {
  ...
  const tree1 = (await git.readTree({ fs, dir: WORKSPACE_ROOT, oid: oid1 })).tree;
  const tree2 = (await git.readTree({ fs, dir: WORKSPACE_ROOT, oid: oid2 })).tree;
  ...
  for (const filepath of allFiles) {
    ...
    if (!o1 && o2) { lines.push(`ADDED ${filepath}`); continue; }
    if (o1 && !o2) { lines.push(`DELETED ${filepath}`); continue; }
    if (o1 !== o2) {
      const b1 = Buffer.from((await git.readBlob({ fs, dir: WORKSPACE_ROOT, oid: o1! })).blob).toString('utf8');
      const b2 = Buffer.from((await git.readBlob({ fs, dir: WORKSPACE_ROOT, oid: o2! })).blob).toString('utf8');
...
function generateUnifiedDiff(filepath: string, oldText: string, newText: string): string[] {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    else { lines.push(`-${o}`); lines.push(`+${n}`); }
  }
  return lines;
}
