...
import { getGraph, graphAvailable, queryGraph } from "./graphifyService";
import { useSerenaStore } from "./serenaService";
import { compressJson, encodeToToon, getToonStats } from "./toonService";
...
function extractGraphifyContext(prompt: string): string | null {
	...
	const needsContext = contextKeywords.some((kw) =>
	...
	try {
		...
		const contextParts = relevantNodes
			.slice(0, 10)
			.map((node) => `${node.type}: ${node.label} (${node.file}:${node.line})`);
...
export async function optimizePrompt(prompt: string): Promise<OptimizedPrompt> {
	...
	if (config.enableAutocoder) {
		...
		if (pattern) {
			...
			if (result.success && result.files.length > 0) {
				...
				logger.info(
					"TokenOptimizer",
					`Cheetah generated ${pattern} — ~${estimatedSavings} tokens saved via ${result.engine}`,
				);
				return {
					original: prompt,
					...
					tokenCount: {
						...
						optimized: Math.ceil((result.files[0]?.content || "").length / 4),
					},
					cached: false,
	...
	if (config.enableGraphify) {
		// Reusing Graphify flag for now or adding a new one
		...
		if (enriched !== optimized) {
			...
			logger.info(
				"TokenOptimizer",
				`Serena enriched prompt with codebase context`,
			);
		}
	}
	...
	if (config.enableToon && isJsonLike(prompt)) {
		...
		if (toonResult.applied) {
			...
			logger.info(
				"TokenOptimizer",
				`Toon compressed: ${toonResult.savingsPercent}% savings`,
			);
		}
	}
...
export function setOptimizerConfig(
	newConfig: Partial<TokenOptimizerConfig>,
): void {
...
export { getGraph, graphAvailable, queryGraph } from "./graphifyService";
