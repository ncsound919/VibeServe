	...
	challenges: [
		{
			...
			description:
				"Build a generic event emitter with typed events and handlers.",
			difficulty: "medium",
			testFile: "eventEmitter.test.ts",
			prompt:
				"Create a type-safe event emitter class that allows registering handlers for specific event types with proper TypeScript generics.",
		},
		{
			id: "challenge-2",
			...
			prompt:
				"Build an LRU (Least Recently Used) cache with get/set operations and optional TTL per entry.",
		},
		{
			id: "challenge-3",
			name: "Promise Pool",
			description:
				"Control concurrency of async operations with a promise pool.",
			difficulty: "hard",
			testFile: "promisePool.test.ts",
			prompt:
				"Implement a promise pool that limits concurrent promise execution to N at a time.",
		},
	],
	results: [],
	isEvaluating: false,
...
const getScoreboard = () => {
	const scores: Record<
		string,
		{ total: number; passed: number; passRate: number; avgTime: number }
	> = {};
	...
	state.results.forEach((result) => {
		...
		scores[result.agentId].avgTime =
			(scores[result.agentId].avgTime * (scores[result.agentId].total - 1) +
				result.executionTimeMs) /
			scores[result.agentId].total;
	});

	Object.keys(scores).forEach((agentId) => {
		scores[agentId].passRate =
			(scores[agentId].passed / scores[agentId].total) * 100;
	});
