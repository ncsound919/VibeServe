...
export class SandboxService {
	...
	async createSandbox(
		...
		if (IS_BROWSER) {
			return {
				...
				exitCode: 1,
				stdout: "",
				stderr: "Sandbox not available in browser",
				durationMs: 0,
			};
		}
	...
	private async executeWithoutSandbox(
		...
		try {
			const res = await safeExecLegacy(command, {
				cwd: workDir,
	...
	private async executeWithDocker(
		...
		if (typeof window !== "undefined")
			return {
				...
				exitCode: 1,
				stdout: "",
				stderr: "Not available in browser",
				durationMs: 0,
				containerId: "",
			};
		if (!this.isDockerAvailable()) {
			throw new Error(
				'Docker is not available. Please install Docker or use sandbox type "none"',
			);
		}
