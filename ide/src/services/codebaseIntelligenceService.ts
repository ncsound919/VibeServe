...
interface CodebaseIntelligenceStore {
	...
	getSemanticDiff: (gitRef: string) => Promise<{
		oldSymbols: Symbol[];
		newSymbols: Symbol[];
		changed: Symbol[];
	}>;
...
function findReferencesInContent(
	content: string,
	symbolName: string,
): string[] {
	const lines = content.split("\n");
...
export const useCodebaseIntelligence = create<CodebaseIntelligenceStore>()(
	(set, get) => ({
		symbols: new Map(),
		dependencies: [],
		callGraph: new Map(),
						...
						if (entry.isDirectory()) {
							...
							if (CODE_EXTS.includes(ext)) {
								...
								for (const symbol of symbols) {
									...
									const existing = newCallGraph.get(symbol.id) || {
										symbolId: symbol.id,
										calls: [],
										calledBy: [],
									};
									...
									if (safeName) {
										const calledSymbols = content.match(
											new RegExp(`${safeName}\\s*\\(`, "g"),
										);
										if (calledSymbols) {
		...
		getExports: (filePath) => {
			const { symbols } = get();
			return Array.from(symbols.values()).filter(
				(s) => s.filePath === filePath && s.kind !== "variable",
			);
		},
		...
		analyzeChangeImpact: async (changedFiles) => {
			...
			for (const file of changedFiles) {
				const fileSymbols = Array.from(symbols.values()).filter(
					(s) => s.filePath === file,
				);
			...
			const breakingChanges = downstreamDependents.filter(
