...
function getDefaultToolImpacts(): ToolImpact[] {
	return [
		{
			toolName: "Biome",
			benchmarksAffected: ["LINT_SCORE", "PERF_SCORE"],
			estimatedImprovement: 80,
			confidence: 0.9,
		},
		{
			toolName: "Trivy",
			benchmarksAffected: ["SECURITY"],
		...
		{
			toolName: "Knip",
			benchmarksAffected: ["DEPS_AUDIT", "BUNDLE_SIZE"],
			estimatedImprovement: 60,
			confidence: 0.85,
		},
		{
			toolName: "Graphify",
			benchmarksAffected: ["TOKEN_EFFICIENCY"],
		...
		{
			toolName: "Biome static analysis",
			benchmarksAffected: ["LINT_SCORE"],
			estimatedImprovement: 30,
			confidence: 0.85,
		},
		{
			toolName: "E2E Playwright",
			benchmarksAffected: ["TEST_COVERAGE"],
			estimatedImprovement: 50,
			confidence: 0.9,
		},
		{
			toolName: "Sequential-Thinking-MCP",
			benchmarksAffected: ["TYPE_SAFETY", "PERF_SCORE"],
...
function calculateOverallScore(benchmarks: Benchmark[]): number {
	...
	const weightedSum = benchmarks.reduce((sum, b) => {
		const normalized =
			b.targetScore > 0 ? Math.min(b.currentScore / b.targetScore, 1) : 0;
...
export function applyBenchmarkToGamification(
	...
	if (!previousGrade) {
		const snapshots = loadSnapshots();
		previousGrade =
			snapshots.length >= 2 ? snapshots[snapshots.length - 2].grade : "F";
	}
