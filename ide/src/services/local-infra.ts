...
function persist() {
	...
	for (const [name, col] of collections) {
		data[name] = {
			name: col.name,
			vectors: col.vectors,
			points: Array.from(col.points.values()),
		};
	}
	writeFileSync(
		path.join(DATA_DIR, "qdrant.json"),
...
async function handleQdrantRequest(
	...
	try {
		...
		if (
			...
			if (!col) {
				json(res, 404, { status: "error", error: "Collection not found" });
				return;
			}
			...
			json(res, 200, {
				result: { operation_id: Date.now(), status: "completed" },
				status: "ok",
				time: Date.now(),
			});
			persist();
			...
			json(res, 200, {
				result: {
					...
					config: {
						params: {
							vectors: {
								size: col.vectors.size,
								distance: col.vectors.distance,
							},
						},
					},
				},
	...
	return new Promise((resolve, reject) => {
		...
		server.on("error", (err: NodeJS.ErrnoException) => {
			if (err.code === "EADDRINUSE") {
				logger.warn(
					"LocalInfra",
					`Port ${port} in use — skipping (likely Docker running)`,
				);
				resolve(server);
			} else {
				reject(err);
...
export async function start(): Promise<LocalInfraStatus> {
	...
	try {
		...
		logger.info(
			"LocalInfra",
			"Local infrastructure started (mock services on standard ports)",
		);
	} catch (e: any) {
		logger.error("LocalInfra", "Failed to start local infrastructure", {
			error: e.message,
		});
	}
...
export function getStatus(): LocalInfraStatus {
	...
	const totalVectors = Array.from(collections.values()).reduce(
		(sum, col) => sum + col.points.size,
		0,
	);
