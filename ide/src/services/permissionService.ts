...
export class PermissionService {
	...
	checkPermission(ctx: PermissionContext): PermissionCheckResult {
		...
		const enabledRules = this.rules.filter(
			(r) => r.enabled && r.scope === ctx.requestedScope,
		);
...
export const permissionService = new PermissionService([
	{
		...
		action: "allow",
		enabled: true,
		description: "Allow read by default",
	},
	{
		id: "default-edit",
		scope: "edit",
		action: "ask",
		enabled: true,
		description: "Always ask for edits",
	},
	{
		id: "default-bash",
		scope: "bash",
		action: "ask",
		enabled: true,
		description: "Always ask for shell commands",
	},
	{
		id: "default-network",
		scope: "network",
		action: "deny",
		enabled: true,
		description: "Deny external network calls by default",
	},
]);
