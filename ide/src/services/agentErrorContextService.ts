...
`;
		...
		if (error.type === "network") {
			...
			confidence = 0.8;
			suggestedFix =
				"Verify network connectivity and service availability. Consider retry with exponential backoff.";
		}
		if (error.type === "dependency") {
			relatedPatterns.push("npm-cache-clear", "lockfile-regenerate");
			confidence = 0.85;
			suggestedFix =
				"Clear npm cache and reinstall dependencies: rm -rf node_modules && npm install";
		}
		...
		if (error.type === "security") {
			...
			confidence = 0.9;
			suggestedFix =
				"Address security vulnerability. Run npm audit fix for automated fixes.";
		}
	} catch {
		rootCause = error.message;
		suggestedFix =
			"Unable to diagnose automatically. Review logs and error details.";
		confidence = 0.3;
	}
...
export function buildErrorFromLogEntry(
	...
	const category =
		type || (categorizeError(message).category as BuildError["type"]);
