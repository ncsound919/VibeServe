...
const SKILL_PATTERNS: Record<
	string,
	{ skills: string[]; phase: string; type: string }
> = {
	".ts": {
...
export function assessAgentFolder(folderPath: string): AgentAssessment {
	...
	const topExt =
		Object.entries(extCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "";
	const typeInfo = SKILL_PATTERNS[topExt] || {
		skills: ["General"],
		phase: "Environment Setup",
		type: "Unknown",
	};
	...
	const integrationPotential = [
		{
			pipelinePhase: typeInfo.phase,
			score: 8,
			reason: `Primary extension: ${topExt}`,
		},
		{
			pipelinePhase: "Static Analysis",
			score: hasTsconfig || jsFiles.length > 0 ? 7 : 3,
			reason: "Code quality assessment",
		},
		{
			pipelinePhase: "RAG Context Sync",
			score: hasReadme ? 7 : 2,
			reason: hasReadme ? "Has documentation" : "No documentation",
		},
		{
			pipelinePhase: "E2E Testing",
			score: skills.includes("Testing") ? 9 : 1,
			reason: skills.includes("Testing") ? "Has test files" : "No tests found",
		},
		{
			pipelinePhase: "Security Audit",
			score: typeSafety >= 7 ? 6 : 3,
			reason: "Security assessment potential",
		},
	...
	try {
		logger.info(
			"AgentAssessment",
			`Assessed "${name}": type=${typeInfo.type} grade=${overall} assignment=${best?.pipelinePhase ?? "Review"}`,
		);
	} catch {}
	...
	return {
		...
		recommendedAssignment:
			best?.score && best.score >= 5 ? best.pipelinePhase : "Review",
