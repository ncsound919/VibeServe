...
Analyze what phases are needed and respond with JSON:
{
  ...
  "summary": "1-2 sentence plan overview"
		...
		try {
			...
			parsed = {
				title: lines[0]?.replace(/^[#*]+\s*/, "") || "Pipeline Plan",
				description: lines[1] || "",
				phases: [],
			};
		}
		...
		const plan: PipelinePlan = {
			...
			description: String(
				parsed.description || `Plan for ${sourceRepos.join(", ")}`,
			),
			sourceRepos,
			phases: phases.length > 0 ? phases : getDefaultPhases(sourceRepos),
			overallRisk:
				(parsed.overallRisk as PipelinePlan["overallRisk"]) || "medium",
		...
		logger.info(
			"PlanAgent",
			`Plan generated: ${plan.title} (${plan.phases.length} phases)`,
		);
		...
		logger.warn(
			"PlanAgent",
			`Plan generation failed: ${err instanceof Error ? err.message : String(err)}, using default`,
		);
		...
		return {
			...
			warnings: [
				"AI-generated plan unavailable — using standard phase template",
			],
			confidence: 0.3,
		};
	}
	...
	const phases: PlanPhase[] = [
		{
			...
			description: "Initialize build environment and verify prerequisites",
			dependencies: [],
			estimatedDuration: "30s",
			risks: ["Node.js version mismatch", "Missing environment variables"],
			expectedOutput: "Ready build environment",
			validationCriteria: "All env vars present, Node.js v20+",
		},
		{
			id: "2",
			name: "Dependency Resolution",
			description: "Install and verify all project dependencies",
			dependencies: ["1"],
			...
			expectedOutput: "Installed node_modules with verified integrity",
			validationCriteria: "npm ci success, no audit issues",
		},
		{
			id: "3",
			name: "Static Analysis",
			description: "Run TypeScript type checking and lint rules",
			dependencies: ["2"],
			estimatedDuration: "30s",
			risks: ["Type errors in source code", "Unused variables"],
			expectedOutput: "Type-checked and linted codebase",
			validationCriteria: "tsc --noEmit passes, biome reports clean",
		},
		{
			id: "4",
			name: "Build & Compile",
			description: "Bundle the application for production",
			dependencies: ["3"],
			estimatedDuration: "1m",
			risks: ["Bundle size exceeds threshold", "Chunk splitting issues"],
			expectedOutput: "Optimized production bundle in dist/",
			validationCriteria: "vite build succeeds, bundle < 2MB",
		},
		{
			id: "5",
			name: "Security Audit",
			...
			validationCriteria: "0 critical/high vulnerabilities",
		},
		{
			id: "6",
			name: "E2E Testing",
			...
			expectedOutput: "Test report with pass/fail",
			validationCriteria: "All critical tests pass",
		},
		{
			id: "7",
			name: "Finalizing",
			description: "Generate reports and sync artifacts",
			dependencies: ["5", "6"],
			estimatedDuration: "30s",
			risks: ["Artifact upload failure"],
			expectedOutput: "Complete pipeline report",
			validationCriteria: "All artifacts synced",
		},
	];
	return phases;
}
...
export function rejectPlan(
	...
	logger.info(
		"PlanAgent",
		`Plan rejected: ${plan.title}${reason ? ` — ${reason}` : ""}`,
	);
	return plan;
}
...
export function listPlans(): PipelinePlan[] {
	...
	const files = readdirSync(DATA_DIR).filter((f: string) =>
		f.endsWith(".json"),
	);
	return files
		.map((f: string) => {
			try {
				return JSON.parse(readFileSync(path.join(DATA_DIR, f), "utf-8"));
			} catch {
				return null;
			}
		})
...
export async function validatePlan(plan: PipelinePlan): Promise<{
	...
	if (plan.phases.length > 20)
		issues.push("Too many phases — consider grouping");

	for (const phase of plan.phases) {
		if (!phase.name) issues.push(`Phase ${phase.id} has no name`);
		if (!phase.validationCriteria)
			issues.push(`Phase "${phase.name}" has no validation criteria`);
