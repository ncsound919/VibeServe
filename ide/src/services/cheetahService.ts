...
async function detectPythonEngine(): Promise<{
	available: boolean;
	scriptPath?: string;
}> {
	if (_pythonAvailable !== null) {
		return {
			available: _pythonAvailable,
			scriptPath: _cheetahScriptPath ?? undefined,
		};
	}
	...
	logger.info(
		"CheetahService",
		"Python engine not found — using TypeScript fallback",
	);
...
function getOutputPath(
	pattern: CheetahPattern,
	name: string,
	opts?: CheetahRequest["options"],
): string {
	const pascal = toPascalCase(name);
	const camel = toCamelCase(name);
	switch (pattern) {
		case "hono-route":
			return `src/server/routes/${camel}Routes.ts`;
		case "crud-api":
			return `src/routes/${camel}.ts`;
		case "react-component":
			return `src/components/${pascal}/${pascal}.tsx`;
		case "typescript-interface":
			return `src/types/${pascal}.ts`;
		case "database-schema":
			return `database/${toCamelCase(name)}.sql`;
		case "supabase-table":
			return `database/supabase_${toSnakeCase(name)}.sql`;
		case "test-file":
			return `tests/${pascal}.test.ts`;
		case "hook":
			return `src/hooks/use${pascal}.ts`;
		case "service":
			return `src/services/${pascal}Service.ts`;
		case "model":
			return `src/models/${pascal}.ts`;
		case "middleware":
			return `src/middleware/${camel}.ts`;
		case "config-file":
			return opts?.language === "yaml" ? "config.yaml" : "config.json";
		default:
			return `src/${name}.ts`;
	}
}
...
export async function generateWithCheetah(
	request: CheetahRequest,
): Promise<CheetahResult> {
	const t0 = Date.now();
	...
	try {
		...
		if (available && scriptPath) {
			...
			try {
				...
				if (request.options?.incremental === false)
				...
				if (existsSync(reportFile)) {
					const report: CheetahBuildReport = JSON.parse(
					...
					logger.info(
						"CheetahService",
						`[python] Generated ${files.length} files for task "${request.taskId}"`,
					);
				...
				logger.warn(
					"CheetahService",
					`Python engine failed, falling back to TS: ${pyErr}`,
				);
			}
		}
		...
		logger.info(
			"CheetahService",
			`[ts-fallback] Generated ${request.pattern} for "${request.name}"`,
		);
...
export async function getCheetahStatus(): Promise<{
	...
	const done = existsSync(TASK_DIR)
		? readdirSync(TASK_DIR).filter((f) => f.endsWith(".done"))
		: [];
