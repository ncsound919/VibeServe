...
export function addHook(config: Omit<HookConfig, "id">): HookConfig {
	...
	logger.info(
		"HookEngine",
		`Registered hook: ${hook.name} [${hook.phase}:${hook.pipelinePhase}]`,
	);
	return hook;
}
...
export async function executeHook(
	...
	if (hook.script) {
		...
		try {
			const output = await safeExecLegacy(hook.script, {
				timeout: 30000,
				stdio: "pipe",
			});
			logs.push(
				`[HOOK:${hook.id}] Script output: ${output.trim().substring(0, 200)}`,
			);
		} catch (err: unknown) {
	...
	if (hook.condition) {
		...
		try {
			await safeExecLegacy(hook.condition, {
				timeout: 15000,
				stdio: "pipe",
			});
			...
			if (hook.action === "block") {
				...
				warnings.push(
					`[WARN] Hook ${hook.name}: condition not met for phase "${pipelinePhase}"`,
				);
			}
		}
	}
	...
	if (blocked) {
		...
		logger.warn(
			"HookEngine",
			`Phase "${pipelinePhase}" blocked by hook "${hook.name}"`,
		);
	}
...
export function getHookStats(): {
	...
	for (const h of hooks) {
		if (h.enabled) {
			...
			byPipelinePhase[h.pipelinePhase] =
				(byPipelinePhase[h.pipelinePhase] || 0) + 1;
		}
	}
