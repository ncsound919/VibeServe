...
const PATTERNS: ErrorPattern[] = [
	...
	// ── pytest failure block ──────────────────────────────────────────────────
	{
		...
		detect: /FAILED (.+?) - (.+?)(?:\n|$)/,
		title: (m) =>
			`Test failed: ${m[1].trim().slice(0, 80)} — ${m[2].trim().slice(0, 60)}`,
		location: /([^\s]+\.py)(?::(\d+))?/,
	},
...
export class ErrorContextService {
	...
	ingestChunk(chunk: string): void {
		...
		if (this.buffer.length > ROLLING_BUFFER_CHARS) {
			this.buffer = this.buffer.slice(
				this.buffer.length - ROLLING_BUFFER_CHARS,
			);
		}
	...
	dismissError(id: string): void {
		...
		if (idx !== -1)
			this.errorQueue[idx] = { ...this.errorQueue[idx], dismissed: true };
	}
	...
	private _scan(): void {
		for (const pattern of PATTERNS) {
			...
			const rawWindow = this.buffer.slice(
				windowStart,
				matchIndex + Math.min(m[0].length + 2_000, MAX_TRACE_CHARS),
			);
