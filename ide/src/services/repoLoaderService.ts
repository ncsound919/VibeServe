...
export async function cloneRepo(githubUrl: string): Promise<ClonedRepo> {
	...
	try {
		...
		const entries = readdirSync(targetDir, { withFileTypes: true }).filter(
			(e) => e.isDirectory(),
		);
		const sourceDir =
			entries.length > 0 ? path.join(targetDir, entries[0].name) : targetDir;
		...
		logger.info(
			"RepoLoader",
			`Cloned ${name}: ${files.length} files, ${totalSizeKB} KB, ${languages.length} languages`,
		);
...
export function getRepoPath(owner: string, repoName: string): string | null {
	...
	for (const entry of entries) {
		if (entry.name.startsWith(`${owner}_${repoName}_`)) {
			const subdirs = readdirSync(path.join(dir, entry.name), {
				withFileTypes: true,
			}).filter((e) => e.isDirectory());
			if (subdirs.length > 0)
				return path.join(dir, entry.name, subdirs[0].name);
