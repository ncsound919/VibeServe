...
export function runBiomeCheck(targetPath?: string): LintReport {
	...
	try {
		const result = execSync(
			"npx --yes biome check --fix --max-diagnostics=50 .",
			{
				encoding: "utf-8",
				timeout: 60000,
				cwd,
		...
		return {
			errors: 0,
			warnings: 0,
			summary: "Biome not available — skipped",
			details: [],
		};
	}
}

export function runDepCheck(targetPath?: string): {
	unused: string[];
	missing: string[];
	summary: string;
} {
	const cwd = targetPath || process.cwd();
	try {
		const result = execSync("npx --yes depcheck --json", {
			encoding: "utf-8",
			timeout: 60000,
			cwd,
		});
		...
		return {
			unused: [],
			missing: [],
			summary: "Depcheck not available — skipped",
		};
	}
}
...
export function getDebtRadar(content: string): DebtRadarReport {
	...
	const complexityScore =
		(content.match(/if |while |for |&& |\|\| /g) || []).length /
		(lineCount / 10);

	const untypedExports = (content.match(/export (const|let|var) \w+ =/g) || [])
		.length;
