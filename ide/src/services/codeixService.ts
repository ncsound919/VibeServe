		...
		for (const pattern of langPatterns) {
			...
			if (match) {
				...
				if (pattern.source.includes("class")) kind = "class";
					...
					pattern.source.includes("variable")
				)
					kind = "variable";
...
export const useCodeixStore = create<CodeixStore>()((set, get) => ({
	...
	getExports: (scope) => {
		...
		return index.entries.filter(
			(e) => e.scope === scope && e.visibility === "public",
		);
	},
	...
	mergeIndex: (other) => {
		...
		Object.keys(mergedSymbolIndex).forEach((name) =>
			addToBloom(mergedBloom, name),
		);
