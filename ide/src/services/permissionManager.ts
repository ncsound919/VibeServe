...
function getDefaultRules(): PermissionRule[] {
	return [
		...
		},
		{
			id: "ask-edit-config",
			scope: "edit",
			pattern: "**/*.{json,yaml,toml,env*}",
			action: "ask",
			priority: 10,
			enabled: true,
			description: "Ask before editing config files",
		},
		{
			id: "allow-edit-src",
			scope: "edit",
		...
		{
			id: "deny-bash-dangerous",
			scope: "bash",
			pattern: "rm -rf *|git push --force*|git reset --hard*",
			action: "deny",
			priority: 200,
			enabled: true,
		...
		},
		{
			...
			priority: 10,
			enabled: true,
			description: "Ask before deleting source files",
		},
		{
			id: "deny-execute-arbitrary",
			scope: "execute",
			pattern: "**/*",
			action: "deny",
			priority: 100,
			enabled: true,
			description: "Deny arbitrary execution",
		},
	];
}
...
export function addRule(rule: Omit<PermissionRule, "id">): PermissionRule {
	...
	logger.info(
		"PermissionManager",
		`Added rule: ${newRule.scope}:${newRule.pattern} → ${newRule.action}`,
	);
	return newRule;
}
...
export function getPermissionsForScope(
	...
	return loadRules().filter(
		(r) => r.enabled && (r.scope === scope || r.scope === "all"),
	);
}
