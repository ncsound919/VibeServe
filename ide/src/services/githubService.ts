...
export async function triggerWorkflow(
	...
	if (!res.ok)
		throw new Error(
			`GitHub workflow trigger failed: ${res.status} ${res.statusText}`,
		);
}
...
export async function getWorkflowRuns(
	...
	const res = await fetch(`${path}?per_page=10`, {
		headers: getHeaders(token),
	});
...
export async function getSecurityAdvisories(
	...
	{ ghsa_id: string; summary: string; severity: string; published_at: string }[]
> {
	const res = await fetch(
...
export async function createRepo(
	...
	if (!res.ok)
		throw new Error(
			`GitHub create repo failed: ${res.status} ${res.statusText}`,
		);
	return res.json();
}
...
export async function createOrUpdateFile(
	...
	if (!res.ok)
		throw new Error(
			`GitHub create/update file failed: ${res.status} ${res.statusText}`,
		);
	return res.json();
}
...
export async function pushFilesToRepo(
	...
	const refRes = await fetch(`${baseUrl}/git/refs/heads/${branch}`, {
		headers: getHeaders(token),
	});
	...
	if (!newCommitRes.ok)
		throw new Error(`Commit creation failed: ${newCommitRes.status}`);
	...
	if (!updateRes.ok)
		throw new Error(`Branch update failed: ${updateRes.status}`);
}
