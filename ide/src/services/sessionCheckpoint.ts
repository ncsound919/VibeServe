	...
	try {
		...
		if (index.checkpoints.length > 20) {
			...
			for (const old of oldest) {
				try {
					unlinkSync(getCheckpointPath(old.id));
				} catch {}
			}
		}
		saveIndex(index);
		...
		logger.info(
			"SessionCheckpoint",
			`Checkpoint "${label}" created: ${checkpointId} (${Object.keys(files).length} files)`,
		);
		return checkpoint;
	} catch (err) {
		logger.error(
...
export function restoreCheckpoint(checkpointId: string): boolean {
	try {
		...
		logger.info(
			"SessionCheckpoint",
			`Checkpoint "${checkpoint.label}" restored: ${restored} files`,
		);
		return true;
	} catch (err) {
		logger.error(
...
export function createPrePhaseCheckpoint(
	executionId: string,
	phase: string,
): Checkpoint | null {
...
export function createPostPhaseCheckpoint(
	executionId: string,
	phase: string,
): Checkpoint | null {
