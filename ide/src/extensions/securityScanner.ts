...
export class ExtensionSecurityScanner {
	...
	public static scan(
		...
		for (const pattern of ExtensionSecurityScanner.DISALLOWED_PATTERNS) {
			if (pattern.test(extensionCode)) {
				result.errors.push(
					`Disallowed code pattern found: ${pattern.toString()}`,
				);
				result.isSafe = false;
			}
		}
		...
		if (declaredPermissions.includes("fs") && !extensionCode.includes("fs")) {
			result.warnings.push(
				'Manifest requests "fs" permission but it is not used.',
			);
		}

		// 3. Scan for potential network exfiltration if no network permission
		if (
			!declaredPermissions.includes("network") &&
			/fetch\s*\(|XMLHttpRequest|WebSocket/i.test(extensionCode)
		) {
			result.errors.push(
				'Extension contains network calls but lacks "network" permission in manifest.',
			);
			result.isSafe = false;
		}
