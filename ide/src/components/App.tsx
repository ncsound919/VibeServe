...
import { type DiffChange, InlineDiffOverlay } from "./InlineDiffOverlay";
...
const CommandCenterTab = lazy(() =>
	import("./views/CommandCenterTab").then((m) => ({
		default: m.CommandCenterTab,
	})),
);
const PipelineTab = lazy(() =>
	import("./views/PipelineTab").then((m) => ({ default: m.PipelineTab })),
);
const SettingsTab = lazy(() =>
	import("./views/SettingsTab").then((m) => ({ default: m.SettingsTab })),
);
const ActivityTab = lazy(() =>
	import("./views/ActivityTab").then((m) => ({ default: m.ActivityTab })),
);
const HistoryTab = lazy(() =>
	import("./views/HistoryTab").then((m) => ({ default: m.HistoryTab })),
);
const AuditTab = lazy(() =>
	import("./views/AuditTab").then((m) => ({ default: m.AuditTab })),
);
...
const PreviewTab = lazy(() =>
	import("../features/preview/MultimodalPreview").then((m) => ({
		default: m.MultimodalPreview,
	})),
);
const ExtensionsTab = lazy(() =>
	import("../features/extensions/ExtensionsPanel").then((m) => ({
		default: m.ExtensionsPanel,
	})),
);
const SystemTab = lazy(() =>
	import("../features/system/SystemPanel").then((m) => ({
		default: m.SystemPanel,
	})),
);
const AgentEvalTab = lazy(() =>
	import("./views/AgentEvalTab").then((m) => ({ default: m.AgentEvalTab })),
);
const MagicTab = lazy(() =>
	import("../features/composer/MagicComposer").then((m) => ({
		default: m.MagicComposer,
	})),
);
...
export default function App() {
	...
	const handleDiffAcceptFile = useCallback(
	...
	if (appLicensed === false)
		return <LicenseGate onActivate={() => refetch()} />;

	return (
		<div
			className="min-h-screen bg-[#0d1117] flex flex-col"
			role="application"
			aria-label="VibeServe IDE"
		>
			{/* Top glow decoration */}
