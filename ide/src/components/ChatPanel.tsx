...
import { gatherChatContext, formatContextForPrompt } from '../services/chatContextService';
import { InlineDiffOverlay, computeDiff, type DiffChange } from './InlineDiffOverlay';
...
export function ChatPanel({ isOpen, onToggle }: ChatPanelProps) {
    ...
    try {
      ...
      const res = await fetch('/api/integrations/agent/chat', {
        ...
        body: JSON.stringify({
          ...
          symbols: context.openFile?.symbols.map((s) => `${s.kind}:${s.name}`).join(', '),
        }),
      });
      ...
      const assistantMsg: ChatMessage = {
        ...
        codeBlocks: data.codeBlocks ?? extractCodeBlocksFromText(data.response ?? ''),
        timestamp: Date.now(),
      };
  ...
  const handleApplyCode = async (codeBlock: CodeBlock) => {
    ...
    try {
      ...
      let oldContent = '';
      try {
        const res = await fetch(`/api/editor/file?path=${encodeURIComponent(fileName)}`);
      ...
      setMessages((prev) => [
        ...prev,
        { id: uid(), role: 'system', content: 'Failed to prepare diff.', timestamp: Date.now() },
      ]);
      setActiveCodeBlock(null);
    }
  };
  ...
  const handleDiffAcceptFile = async (fileName: string) => {
    ...
    try {
      ...
      const writeRes = await fetch('/api/editor/file', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: fileName, content: diffChange.newContent }),
      });
      ...
      try {
        const verifyRes = await fetch('/api/pipeline/suggestions/apply', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ filePath: fileName, content: diffChange.newContent }),
        });
        const verifyData = await verifyRes.json();
        verificationStatus = verifyData.verification?.status === 'failing' ? 'failing' : 'passing';
      } catch {
        // Verification failed, but file was written
      }

      setDiffChange((prev) =>
        prev ? { ...prev, applied: true, verifying: false, verified: true, verificationStatus } : prev,
      );
      ...
      setMessages((prev) => [
        ...prev,
        { id: uid(), role: 'system', content: `Failed to apply ${fileName}.`, timestamp: Date.now() },
      ]);
    }
  };
    ...
    <div className="w-80 lg:w-96 border-l border-[#21262d] bg-[#0d1117] flex flex-col h-full">
        ...
        <AnimatePresence>
          {messages.length === 0 && (
            ...
            >
              ...
              <h3 className="text-sm font-semibold text-[#c9d1d9] mb-1">Ask anything</h3>
              <p className="text-xs text-[#7d8590] max-w-[200px]">
                Generate code, explain logic, refactor, or debug with full codebase context.
              </p>
            </motion.div>
          )}
                ...
                {msg.codeBlocks?.map((block, i) => (
                  <div key={i} className="mt-2">
                    ...
                    <pre className="bg-[#0d1117] border border-[#30363d] rounded-b p-2 overflow-x-auto">
                      <code className="text-xs font-mono text-[#c9d1d9]">{block.code}</code>
                    </pre>
      ...
      <div className="p-4 border-t border-[#21262d] bg-[#161b22]">
        ...
        <div className="mt-2 flex items-center justify-between text-[10px] text-[#484f58]">
          <span>
            <kbd className="px-1 py-0.5 bg-[#0d1117] border border-[#30363d] rounded font-mono">Enter</kbd> to send
          </span>
          <span>
            <kbd className="px-1 py-0.5 bg-[#0d1117] border border-[#30363d] rounded font-mono">Shift+Enter</kbd> new line
          </span>
        </div>
      </div>
    </div>
