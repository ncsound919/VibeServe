import { Bug, Circle, Eye, FileEdit, Pause, Type, Users } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { type PresenceUser, usePresenceStore } from "../stores/presenceStore";
...
export function PresenceBar() {
	...
	const getActivityIcon = (activity?: PresenceUser["activity"]) => {
		...
		switch (activity.type) {
			case "editing":
				return <FileEdit className="w-3 h-3 text-emerald-400" />;
			case "viewing":
			...
			<div className="flex items-center gap-2 px-3 py-1.5 bg-slate-800/50 border-b border-slate-700">
				...
				<div className="flex -space-x-2">
					...
					{onlineUsers.length > 5 && (
						<div
							className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center text-xs text-slate-300 border-2 border-slate-800 cursor-pointer"
							onClick={() => setShowDetails(!showDetails)}
						>
							+{onlineUsers.length - 5}
						</div>
				...
				<div className="ml-auto flex items-center gap-1">
					<select
						...
						onChange={(e) =>
							updateUserStatus(
								currentUserId,
								e.target.value as PresenceUser["status"],
							)
						}
					>
							...
							>
								...
								<div className="flex-1 min-w-0">
									...
									<div className="flex items-center gap-3 text-xs text-slate-400">
										...
										{user.cursor && <span>Line {user.cursor.line}</span>}
									</div>
									...
									{user.selections && user.selections.length > 0 && (
										<div className="text-xs text-slate-500 mt-1">
											{user.selections.length} selection
											{user.selections.length > 1 ? "s" : ""}
										</div>
									)}
								</div>
							</div>
...
function PresenceAvatar({
	...
	return (
		...
		>
			...
			<div
				className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-slate-800 ${statusColor}`}
			/>
