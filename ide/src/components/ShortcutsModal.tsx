...
const SHORTCUTS: ShortcutEntry[] = [
  { keys: 'Ctrl+I', action: 'Agent mode — describe & edit code', category: 'Agent' },
  ...
  { keys: 'Ctrl+1..6', action: 'Navigate tabs (Composer..Changes)', category: 'Navigation' },
  { keys: 'Ctrl+,', action: 'Open settings', category: 'Navigation' },
  { keys: 'Tab', action: 'Accept ghost-text completion', category: 'Editor' },
  { keys: 'Escape', action: 'Close modals / exit vim insert', category: 'General' },
  { keys: 'Ctrl+Shift+O', action: 'Open dashboard overview', category: 'Navigation' },
];

const CATEGORIES = ['Agent', 'Editor', 'Files', 'View', 'Navigation', 'Pipeline', 'General'];
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#21262d]">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-semibold text-[#c9d1d9]">Keyboard Shortcuts</h2>
          </div>
          <button onClick={onClose} className="p-1.5 rounded hover:bg-[#21262d] text-[#7d8590] hover:text-[#c9d1d9]">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-3 border-t border-[#21262d] bg-[#0d1117]/50">
          <p className="text-[10px] text-[#484f58] text-center">
            Press <kbd className="px-1 py-0.5 bg-[#161b22] border border-[#30363d] rounded font-mono">Ctrl+K Ctrl+S</kbd> anytime to see this cheatsheet
          </p>
        </div>
      </motion.div>
    </motion.div>
