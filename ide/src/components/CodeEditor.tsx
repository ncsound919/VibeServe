...
import { extractSymbols, useContextIndex } from "../services/contextIndex";
import {
...
import {
	...
	memoryCache,
	symbolCache,
} from "../services/lru-cache";
import {
	checkCodeAgainstRules,
	useRulesStore,
} from "../services/project-rules";
import { useAIStore } from "../stores/useAIStore";
import { useIDEStore } from "../stores/useIDEStore";
import { CmdKEditPalette } from "./CmdKEditPalette";
...
export function CodeEditor({
	...
	// Global Cmd+K listener — open palette with current selection or whole file
	useEffect(() => {
		const onKey = (e: KeyboardEvent) => {
			if ((e.metaKey || e.ctrlKey) && e.key === "k") {
				// Don't hijack Cmd+K if the palette is already open
				if (showCmdK) return;
				const target = e.target as HTMLElement | null;
				// Only handle when not typing in a normal text field
				if (
					target &&
				...
				setContextMenu(null);
			} else if (e.key === "Escape" && showCmdK) {
				setShowCmdK(false);
			} else if (e.key === "Escape" && showExplain) {
				setShowExplain(false);
			}
		};
		window.addEventListener("keydown", onKey);
		return () => window.removeEventListener("keydown", onKey);
	}, [value, selectionRange, contextMenu, showCmdK, showExplain]);
	...
	const toggleVim = useCallback(async () => {
		...
		if (!vimEnabled) {
			try {
				...
				vimStatusRef.current = initVimMode(
					editor,
				...
				const checkMode = setInterval(() => {
					...
					if (el) {
						...
						else if (text.includes("-- NORMAL --") || !text)
							setVimMode("NORMAL");
					}
				}, 200);
	...
	const toggleCollab = useCallback(() => {
		if (collabEnabled) {
			...
			session.onConnectionChange = (connected: boolean) =>
		...
		(monaco: Monaco) => {
			...
			monaco.languages.registerCodeActionProvider("typescript", {
				provideCodeActions: (
					...
					violations.forEach((v: any) => {
						if (
							v.line >= range.startLineNumber &&
							v.line <= range.endLineNumber
						) {
							if (
								v.rule === "No console.log" &&
			...
			monaco.languages.registerCompletionItemProvider("typescript", {
				provideCompletionItems: (
					...
					const cached = completionCache.get(cacheKey) as
	...
	const handleMount = useCallback(
		(
			...
			// Register Cmd+K (Ctrl+K) to open the AI edit palette
			editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyK, () => {
				...
				if (!text) {
					// No selection: open with whole-file context
			...
			// Selection change → persist the current selection for explain/copy
			...
			const ctxMenuHandler = editor.onContextMenu((e) => {
				const model = editor.getModel();
				if (!model) return;
				const sel = editor.getSelection();
				...
				const disposable = monaco.languages.registerInlineCompletionsProvider(
					lang,
					{
						provideInlineCompletions: async (
							...
							const localCompletion = getLocalInlineCompletion(
								code,
								lineNumber,
								column,
								lang,
			...
			const prev = (editor as any).__vibeserve_disposables as
	...
	const handleCopy = () => {
		const text = selectionRange?.text ?? value;
		navigator.clipboard.writeText(text);
	};

	const applyEditToSelection = useCallback((newText: string) => {
		const editor = editorRef.current;
		const sel = editor?.getSelection();
		if (!editor || !sel) {
	...
	const openInPreview = useCallback(() => {
		if (
			!["html", "htm", "tsx", "jsx", "svg", "md"].some((ext) =>
		...
		addMessage({
			role: "assistant",
			content: `👁 Opened ${fileName} in new tab`,
		});
	}, [value, fileName, addMessage]);
	...
	}, [selectionRange, value, currentLanguage, addMessage]);

	const openExplain = useCallback(() => {
		if (selectionRange?.text) setShowExplain(true);
	}, [selectionRange]);
	...
	return (
		<div className="flex flex-col h-full bg-[#0d1117] rounded-xl overflow-hidden border border-[#21262d]">
			...
			<div className="flex items-center justify-between px-3 py-2 bg-[#161b22] border-b border-[#21262d]">
				...
				<div className="flex items-center gap-1">
					...
					<button
						...
						className="flex items-center gap-1 px-2 py-1 text-xs text-[#7d8590] hover:text-[#d2a8ff] hover:bg-[#21262d] rounded transition-colors"
						title="Edit with AI (Cmd+K)"
					>
					...
					<button
						onClick={openExplain}
						disabled={!selectionRange?.text}
						className="flex items-center gap-1 px-2 py-1 text-xs text-[#7d8590] hover:text-[#79c0ff] hover:bg-[#21262d] rounded transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
					...
					<div className="relative">
						...
						>
							{LANGUAGE_OPTIONS.find((l) => l.value === currentLanguage)
			...
			{contextMenu && (
				<div
					className="fixed z-50 min-w-[220px] rounded-lg shadow-2xl py-1"
					style={{
						left: contextMenu.x,
						top: contextMenu.y,
						background: "#161b22",
				...
				>
					<button
						onClick={() => {
							setSelectionRange({
								...
								startLine: 1,
								endLine: contextMenu.text.split("\n").length,
							});
							setContextMenu(null);
							setShowCmdK(true);
						}}
						className="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs hover:bg-[#1f6feb]/20 text-[#c9d1d9]"
					>
						<Sparkles className="w-3 h-3 text-[#d2a8ff]" /> Edit with AI…
						<kbd className="ml-auto text-[9px] font-mono opacity-60">⌘K</kbd>
					</button>
					<button
						onClick={() => {
							setSelectionRange({
						...
						className="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs hover:bg-[#1f6feb]/20 text-[#c9d1d9]"
					>
						<HelpCircle className="w-3 h-3 text-[#79c0ff]" /> Explain this code
					</button>
					<button
						onClick={() => {
					...
					<button
						...
						className="flex items-center gap-2 w-full text-left px-3 py-1.5 text-xs hover:bg-[#1f6feb]/20 text-[#c9d1d9]"
					>
						<Sparkles className="w-3 h-3 text-[#d2a8ff]" /> Send to Composer
					</button>
				</div>
			)}
			...
			<CmdKEditPalette
				...
				selectedText={selectionRange?.text ?? value}
				language={currentLanguage}
				fileName={fileName}
				onApply={applyEditToSelection}
				onExplain={(text) => {
					setShowExplain(false);
			...
			<ExplainCodePanel
				...
				code={selectionRange?.text ?? value}
				language={currentLanguage}
				fileName={fileName}
			/>
		</div>
