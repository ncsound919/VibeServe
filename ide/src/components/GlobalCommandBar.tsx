...
export const GlobalCommandBar = () => {
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if (
				(mod && e.key === "k" && !e.shiftKey && !e.altKey) ||
		...
		(item: (typeof flatItems)[0]) => {
			if (item.type === "file") {
				...
				window.dispatchEvent(
					new CustomEvent("vibeserve:open-file", { detail: item.file.path }),
				);
			} else {
				item.command.action();
			}
				...
				<>
					...
					>
						<div className="rounded-xl overflow-hidden shadow-2xl border border-[#30363d] bg-[#161b22]">
							...
							{/* Results */}
							<div
								className="max-h-[360px] overflow-y-auto p-1 custom-scrollbar"
								role="listbox"
							>
											...
											<div key={category}>
												...
												{cmds.map((cmd) => {
													const globalIdx = flatItems.findIndex(
														(i) =>
															i.type === "command" && i.command.id === cmd.id,
													);
													return (
														<button
															key={cmd.id}
															onClick={() =>
																handleSelect({ type: "command", command: cmd })
															}
							...
							<div className="px-4 py-2 border-t border-[#21262d] flex items-center justify-between text-[10px] text-[#7d8590]">
								...
								<span className="font-mono text-[#58a6ff]/60">
									{loadingFiles ? "Indexing..." : `${files.length} files`}
								</span>
							</div>
						</div>
					</motion.div>
