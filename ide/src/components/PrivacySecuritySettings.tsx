...
export function PrivacySecuritySettings() {
	...
	const testOllamaConnection = async () => {
		...
		try {
			const response = await fetch(`${privacy.ollamaEndpoint}/api/tags`, {
				method: "GET",
			});
			if (response.ok) {
						...
						<div className="flex items-center justify-between mb-3">
							<div>
								...
								<div className="text-xs text-gray-500">
									Process code locally without cloud APIs
								</div>
							</div>
							<button
								onClick={() =>
									updatePrivacy("localFirstMode", !privacy.localFirstMode)
								}
								...
								<div>
									...
									<div className="flex gap-2">
										<input
											...
											onChange={(e) =>
												updatePrivacy("ollamaEndpoint", e.target.value)
											}
						...
						<div className="space-y-3">
							<label className="flex items-center justify-between cursor-pointer">
								...
								<input
									...
									onChange={(e) =>
										updatePrivacy("storeCodeLocally", e.target.checked)
									}
							...
							<label className="flex items-center justify-between cursor-pointer">
								<div>
									...
									<div className="text-xs text-gray-500">
										Clean up generated files after session
									</div>
								</div>
								<input
									type="checkbox"
									...
									onChange={(e) =>
										updatePrivacy("autoDeleteTempFiles", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
							...
							<label className="flex items-center justify-between cursor-pointer">
								<div>
									...
									<div className="text-xs text-gray-500">
										Detect API keys, passwords in generated code
									</div>
								</div>
								<input
									type="checkbox"
									...
									onChange={(e) =>
										updatePrivacy("sensitiveDataScanning", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
				...
				<div className="space-y-4">
						...
						<div className="space-y-3">
							<label className="flex items-center justify-between cursor-pointer">
								...
								<input
									...
									onChange={(e) =>
										updateSecurity("apiKeyMasked", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
							...
							<label className="flex items-center justify-between cursor-pointer">
								...
								<input
									...
									onChange={(e) =>
										updateSecurity("scanGeneratedCode", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
							...
							<label className="flex items-center justify-between cursor-pointer">
								...
								<input
									...
									onChange={(e) =>
										updateSecurity("verifyBeforeApply", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
							...
							<label className="flex items-center justify-between cursor-pointer">
								...
								<input
									...
									onChange={(e) =>
										updateSecurity("auditLogging", e.target.checked)
									}
									className="w-4 h-4 accent-emerald-500"
								/>
							</label>
					...
					<div className="p-4 bg-gray-900/50 rounded-lg border border-gray-800">
						...
						<input
							...
							onChange={(e) =>
								updateSecurity("secretRotationDays", parseInt(e.target.value))
							}
						...
						<div className="flex justify-between text-xs text-gray-500 mt-1">
							...
							<span className="text-white">
								{security.secretRotationDays} days
							</span>
							<span>180</span>
						</div>
					</div>
						...
						<div className="flex items-start gap-2">
							...
							<div>
								...
								<div className="text-xs text-gray-400 mt-1">
									Always verify generated code before applying. Review changes
