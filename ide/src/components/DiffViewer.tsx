		...
		hunks: [
			{
				...
				lines: [
					...
					{
						type: "add",
						content: "interface SettingsStore {",
						oldLineNumber: undefined,
						newLineNumber: 3,
					},
					{
						type: "add",
						content: "  localFirstMode: boolean;",
						oldLineNumber: undefined,
						newLineNumber: 4,
					},
					{
		...
		hunks: [
			{
				...
				lines: [
					{
						type: "remove",
						content: "export default function OldTab() {",
						oldLineNumber: 1,
						newLineNumber: undefined,
					},
					{
						type: "remove",
						content: "  return <div>Old Content</div>;",
						oldLineNumber: 2,
						newLineNumber: undefined,
					},
					{
...
export function DiffViewer({
	...
	const getStatusIcon = (status: DiffFile["status"]) => {
		switch (status) {
			...
			case "modified":
				return <GitCompare className="w-3 h-3 text-amber-400" />;
		}
	};
					...
					<div className="p-2 space-y-1">
						{diff.map((file) => (
							...
							>
								...
								<span
									className={`text-[10px] px-1.5 py-0.5 rounded ${getStatusBadge(file.status)}`}
								>
									{file.status}
								</span>
							</button>
					...
					{activeFile && (
						<div className="flex-1 overflow-auto">
							<div className="sticky top-0 px-4 py-2 bg-[#1a1a1c] border-b border-gray-800 flex items-center gap-2">
								...
								<span
									className={`text-[10px] px-1.5 py-0.5 rounded ${getStatusBadge(activeFile.status)}`}
								>
									{activeFile.status}
								</span>
							</div>
					...
					<div className="space-y-2 text-xs">
						...
						<div className="flex justify-between text-gray-400">
							...
							<span className="text-emerald-400">
								{activeFile?.hunks.reduce(
									(acc, h) =>
										acc + h.lines.filter((l) => l.type === "add").length,
									0,
								) || 0}
							</span>
						</div>
						<div className="flex justify-between text-gray-400">
							...
							<span className="text-red-400">
								{activeFile?.hunks.reduce(
									(acc, h) =>
										acc + h.lines.filter((l) => l.type === "remove").length,
									0,
								) || 0}
							</span>
						</div>
