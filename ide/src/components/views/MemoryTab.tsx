...
import type { Memory, MemoryTier } from "../../core/agents/memory/memoryTypes";
	...
	return (
		<div className="flex h-full">
					...
					{tiers.map((tier) => (
						<button
							...
							className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm mb-1 transition-colors ${
								selectedTier === tier
									? tierColors[tier].split(" ")[0] + " bg-opacity-20"
									: "text-gray-400 hover:bg-gray-800"
							}`}
							style={{
								backgroundColor:
			...
			<div
				...
				role="tabpanel"
				aria-label="Memory Inspector View"
			>
					...
					{searchResults.length === 0 ? (
						<div className="flex flex-col items-center justify-center h-full text-gray-500">
							...
							<p className="text-xs mt-1">
								Start using the agent to build memory
							</p>
						</div>
					) : (
