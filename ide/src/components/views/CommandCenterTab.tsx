...
import type { CLIStateData, DashboardData } from "../../types";
...
export const CommandCenterTab = ({
	data = {
		cliState: { activeProvider: "opencode", output: [] },
		customAgents: [],
		mcpStatus: null,
	},
	onCommand = () => {},
			...
			<section>
				<div className="flex justify-between items-center mb-6">
					...
					<div className="text-[10px] font-mono text-emerald-400 flex items-center gap-2">
						...
						BRIDGE_ACTIVE:{" "}
						{data.cliState?.activeProvider?.toUpperCase() || "OFFLINE"}
					</div>
				</div>
				<CLITerminal
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div className="bg-[#151619] border border-[#2d2e32] rounded-xl p-6 flex flex-col justify-center">
							...
							<p className="text-[11px] text-[#8E9299] font-mono leading-relaxed">
								L3 connection verified between DeepSeek-V4 and Gemini-2.0.
								Cross-model reasoning enabled via MCP protocol v1.0.0.
							</p>
						</div>
					</div>
				</section>
		...
		<div className="space-y-8">
			...
			<section className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-6">
				...
				<p className="text-xs text-[#8E9299] font-mono leading-relaxed mb-4">
					DeepSeek-V4 and DeepSeek-R1 (Reasoning) are currently ACTIVE.
					Ultra-low latency semantic scans are routed through the Nexus-Alpha
					gateway.
				</p>
