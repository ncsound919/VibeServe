...
export const HistoryTab = () => {
		...
		try {
			...
			if (res.ok) {
				...
				setErrorStats({
					...
					recentErrors: Array.isArray(stats.errors)
						? stats.errors
	...
	const resolvedPercent = errorStats
		? errorStats.total > 0
			...
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="text-[11px] text-[#9CA3AF] font-mono mt-1">
						Build scores, fix history, and error tracking
					</p>
				</div>
				<button
						...
						{vibeState === "error" && (
							<div className="flex flex-col items-center justify-center py-8 text-center">
								...
								<button
									...
									onClick={fetchAll}
									className="mt-3 text-[10px] font-mono text-emerald-400 hover:text-emerald-300 focus-visible:ring-2 focus-visible:ring-emerald-500/50 transition-all"
								>
									Retry
								</button>
							</div>
						...
						{vibeState === "empty" && (
							<div className="flex flex-col items-center justify-center py-10 text-center bg-[#1a1b1e]/30 border border-[#2d2e32] rounded-xl">
								...
								<p className="text-[10px] text-[#6B7280] font-mono mt-1">
									Generate an app to populate scores
								</p>
							</div>
						)}
							...
							vibeHistory.map((entry, i) => (
								<motion.div
									...
									className={cn(
										...
										getGradeBg(entry.grade),
									)}
								>
									<div className="flex items-center justify-between mb-1">
										<p className="text-[11px] font-mono text-white truncate pr-2">
											{entry.description}
										</p>
										<span
											className={cn(
					...
					<div className="space-y-3">
						{fixHistory.length === 0 && (
							<div className="flex flex-col items-center justify-center py-10 text-center bg-[#1a1b1e]/30 border border-[#2d2e32] rounded-xl">
								...
								<p className="text-[11px] text-[#9CA3AF] font-mono">
									No fix history recorded
								</p>
								<p className="text-[10px] text-[#6B7280] font-mono mt-1">
									Pipeline auto-fixes will appear here
								</p>
							</div>
						)}
				...
				<section className="xl:col-span-1">
					...
					{errorState === "error" && (
						<div className="flex flex-col items-center justify-center py-8 text-center">
							...
							<button
								aria-label="Retry loading errors"
								onClick={fetchAll}
								className="mt-3 text-[10px] font-mono text-emerald-400 hover:text-emerald-300 focus-visible:ring-2 focus-visible:ring-emerald-500/50 transition-all"
							>
								Retry
						...
						<div className="space-y-4">
							<div className="grid grid-cols-3 gap-3">
								<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] rounded-xl p-4 text-center">
									...
									<div className="text-[9px] text-[#6B7280] font-mono mt-0.5">
										Total
									</div>
								</div>
								<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] rounded-xl p-4 text-center">
							...
							<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] rounded-xl p-4">
								...
								<div className="flex justify-between mt-1">
									...
									<span className="text-[9px] font-mono text-[#6B7280]">
										{errorStats.resolved}/{errorStats.total}
									</span>
								</div>
							</div>
