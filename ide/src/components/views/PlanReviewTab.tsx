...
export default function PlanReviewTab() {
	...
	useEffect(() => {
		...
		const mockPlan: MultiFilePlan = {
			...
			summary:
				"Add JWT verification to all sensitive routes and implement a secure token refresh logic.",
			steps: [
				{
					id: "1",
					file: "src/server/hono.ts",
					action: "modify",
					purpose: "Inject requireRole middleware into /api/proxy routes",
					status: "pending",
				},
				{
					id: "2",
					...
					action: "create",
					purpose: "New authentication utility for token validation",
					status: "pending",
				},
				{
					id: "3",
					...
					action: "modify",
					purpose: "Auto-attach Bearer tokens to outbound requests",
					status: "pending",
				},
			],
		};
	...
	return (
		<div className="p-6 h-[calc(100vh-120px)] flex flex-col gap-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-gray-500">
						Approve and execute multi-file implementation plans
					</p>
				</div>
			...
			<div
				...
				role="tabpanel"
				aria-label="Plan Review View"
			>
				{/* Left: Step List */}
				<div className="w-80 flex flex-col gap-4">
					<div className="glass-card rounded-2xl p-4 border-white/5">
						<h3 className="text-xs font-bold uppercase tracking-widest text-indigo-300 mb-4">
							Plan Overview
						</h3>
								...
								>
									<div className="mt-0.5">
										{step.status === "completed" ? (
											...
											<Circle
												className={`w-4 h-4 ${selectedStepId === step.id ? "text-indigo-400" : "text-gray-600"}`}
											/>
										)}
									</div>
									<div className="min-w-0">
										<div
											className={`text-xs font-bold truncate ${selectedStepId === step.id ? "text-white" : "text-gray-400"}`}
										>
											{step.file.split("/").pop()}
										</div>
					...
					<div className="glass-card rounded-2xl p-4 border-white/5 flex items-center gap-3">
						...
						<div>
							...
							<div className="text-[9px] text-gray-500">
								Plan passed 12 automated guardrails
							</div>
						</div>
					</div>
				</div>
				...
				<div className="flex-1 flex flex-col gap-4 min-w-0">
					{selectedStep ? (
						<>
							<div className="glass-card rounded-2xl p-4 border-white/5 flex items-center justify-between">
								...
								<div className="flex gap-2">
									<span className="px-2 py-1 bg-white/5 rounded-lg text-[10px] font-mono text-gray-400 uppercase">
										{selectedStep.action}
									</span>
								</div>
							</div>
						...
						<div className="flex-1 flex flex-col items-center justify-center opacity-30 select-none">
							...
							<p className="text-sm font-mono uppercase tracking-[0.2em] text-indigo-300">
								Select a step to review
							</p>
						</div>
					)}
				</div>
