	...
	const fetchAppDetail = async (app: GeneratedApp) => {
		...
		try {
			const res = await fetch(
				`/api/coding-agent/apps/${encodeURIComponent(app.name)}`,
			);
			if (res.ok) {
				const data = await res.json();
				setSelectedApp(data);
			...
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="text-[11px] text-[#9CA3AF] font-mono mt-1">
						Generated apps and recent pipeline runs
					</p>
				</div>
				<button
				...
				>
					<RefreshCcw
						size={12}
						className={appsState === "loading" ? "animate-spin" : ""}
					/>
					Refresh
				</button>
			</div>
					...
					<div className="space-y-3">
						...
						{appsState === "error" && (
							<div className="flex flex-col items-center justify-center py-12 text-center">
								...
								<p className="text-[10px] text-[#6B7280] font-mono mb-4">
									{appsError}
								</p>
								<button
						...
						{appsState === "empty" && (
							<div className="flex flex-col items-center justify-center py-12 text-center bg-[#1a1b1e]/30 border border-[#2d2e32] rounded-xl">
								...
								<p className="text-[10px] text-[#6B7280] font-mono mt-1">
									Use the VibeCoder to create your first app
								</p>
							</div>
						)}
							...
							apps.map((app, i) => (
								...
								>
									<div className="flex items-start justify-between">
										...
										<ChevronRight
											size={14}
											className="text-[#6B7280] group-hover:text-emerald-400 transition-colors shrink-0 mt-1"
										/>
									</div>
					...
					<div className="space-y-3">
						{!hasPipelineData && (
							<div className="flex flex-col items-center justify-center py-12 text-center bg-[#1a1b1e]/30 border border-[#2d2e32] rounded-xl">
								...
								<p className="text-[10px] text-[#6B7280] font-mono mt-1">
									Trigger a pipeline from repo analysis
								</p>
							</div>
						)}
						...
						{recentPipelineRuns.map((exec, i) => (
							...
							>
								<div className="flex items-start justify-between mb-2">
									<h4 className="text-xs font-mono font-medium text-white truncate">
										{exec.id}
									</h4>
									<span
										className={cn(
								...
								{exec.logs && exec.logs.length > 0 && (
									<div className="mt-2 pt-2 border-t border-[#2d2e32]">
										<p className="text-[9px] font-mono text-[#6B7280] truncate">
											{exec.logs[exec.logs.length - 1]}
										</p>
									</div>
								)}
							</motion.div>
