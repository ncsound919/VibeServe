	...
	const handleFileClick = async (filePath: string) => {
		...
		try {
			const res = await fetch(
				`/api/editor/file?path=${encodeURIComponent(filePath)}`,
			);
			const data = await res.json();
			...
			<div className="w-64 border-r border-white/5 flex flex-col bg-[#0D0D0E]">
				<div className="p-4 border-b border-white/5">
					<div className="flex items-center justify-between mb-4">
						...
						<button
							onClick={fetchApps}
							className="p-1 hover:bg-white/5 rounded text-gray-500 hover:text-indigo-400 transition-colors"
						>
						...
						{activeContent !== null ? (
							...
							<div className="flex flex-col items-center justify-center h-full opacity-30 select-none">
								...
								<p className="text-[10px] mt-2 text-gray-500">
									All writes are strictly guarded by Nexus RBAC
								</p>
							</div>
						)}
					</AnimatePresence>
