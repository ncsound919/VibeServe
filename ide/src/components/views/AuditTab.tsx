...
import {
	type AuditEvent,
	useAuditStore,
} from "../../core/agents/monitoring/auditStore";
				...
				<div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-2xl font-bold">
								{
									events.filter((e) => Date.now() - e.timestamp < 3600000)
										.length
								}
							</p>
						</div>
				...
				>
					...
					{agentIds.map((id) => (
						<option key={id} value={id}>
							{metrics[id]?.totalExecutions ? `Agent ${id.slice(0, 8)}` : id}
						</option>
					))}
				</select>
				...
				<div className="divide-y divide-slate-700/50">
					{filteredEvents.length === 0 ? (
						...
						filteredEvents.map((event) => (
							<div
								key={event.id}
								className="p-3 hover:bg-slate-800/30 flex items-start gap-3"
							>
