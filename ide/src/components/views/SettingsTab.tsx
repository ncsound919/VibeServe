import { Activity, Cpu, Save, Settings, Shield, Users } from "lucide-react";
...
function PrivacyModePanel() {
	...
	return (
		<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] p-5 rounded-xl">
			...
			<div
				className="p-4 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50"
				role="tabpanel"
			...
			>
				<div className="flex items-center justify-between">
					...
					>
						<span
							className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${privacyMode ? "translate-x-[18px]" : "translate-x-0.5"}`}
						/>
					</button>
				</div>
				...
				{privacyMode && (
					<div className="mt-3 p-3 rounded-lg border border-emerald-400/20 bg-emerald-400/5">
						<div className="flex items-center justify-between mb-2">
							...
							<span className="text-[8px] font-mono text-emerald-400/50 uppercase tracking-tighter">
								Connected
							</span>
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center gap-2">
								<label className="text-[9px] font-mono text-[#4a4b50] uppercase min-w-[60px]">
									Model
								</label>
								<select
									value={selectedModel}
							...
							<div className="text-[9px] text-[#4a4b50] space-y-1">
								<p>
									Zero API keys required — all processing runs on your hardware
								</p>
		...
		<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] p-5 rounded-xl">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50">
					<label className="text-[9px] font-mono text-[#4a4b50] uppercase">
						Build Timeout (s)
					</label>
					<input
						type="number"
				...
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50">
					...
					<input
						...
						onChange={(e) =>
							toggle("maxParallelPhases", Number(e.target.value))
						}
						className="w-full mt-0.5 text-[10px] font-mono bg-[#0a0a0c] border border-[#2d2e32] rounded px-2 py-1 text-white"
					/>
				</div>
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50 flex items-center justify-between">
					...
					>
						<span
							className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${local.autoRetry ? "translate-x-[18px]" : "translate-x-0.5"}`}
						/>
					</button>
				</div>
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50">
				...
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50 flex items-center justify-between">
					...
					>
						<span
							className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${local.wikiAutoCompile ? "translate-x-[18px]" : "translate-x-0.5"}`}
						/>
					</button>
				</div>
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50 flex items-center justify-between">
					...
					<button
						onClick={() =>
							toggle("graphifyAutoBuild", !local.graphifyAutoBuild)
						}
						className={`w-9 h-5 rounded-full transition-colors relative ${local.graphifyAutoBuild ? "bg-emerald-400" : "bg-[#2d2e32]"}`}
					>
						<span
							className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${local.graphifyAutoBuild ? "translate-x-[18px]" : "translate-x-0.5"}`}
						/>
					</button>
				</div>
				<div className="p-3 rounded-lg border border-[#2d2e32] bg-[#0a0a0c]/50 flex items-center justify-between">
					...
					>
						<span
							className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${local.toonAutoCompress ? "translate-x-[18px]" : "translate-x-0.5"}`}
						/>
					</button>
				</div>
			</div>
...
function AgentRegistryPanel() {
	...
	return (
		<div className="bg-[#1a1b1e]/50 border border-[#2d2e32] p-5 rounded-xl">
			...
			{agents.length === 0 ? (
				<p className="text-[10px] text-[#4a4b50] italic">
					No agents uploaded or assessed yet.
				</p>
			) : (
					...
					{agents.map((agent) => (
						...
						>
							<div className="flex items-center justify-between mb-1">
								...
								<span
									className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
										agent.qualityGrade === "A"
											? "text-emerald-400 bg-emerald-400/10"
											: agent.qualityGrade === "B"
							...
							<div className="text-[9px] text-[#4a4b50] space-y-0.5">
								<p>
									Type: <span className="text-white/70">{agent.type}</span>
								</p>
								<p>
									Phase:{" "}
									<span className="text-white/70">{agent.assignedPhase}</span>
								</p>
								<p>
									Skills:{" "}
	...
	return (
		<div className="p-6 space-y-6">
			<div className="flex items-center gap-3 mb-2">
				...
				{loading && (
					<span className="text-[10px] font-mono text-[#4a4b50] animate-pulse">
						Refreshing...
					</span>
				)}
			</div>
			...
			<IntegrationsPanel integrations={integrations} onRefresh={refreshAll} />
