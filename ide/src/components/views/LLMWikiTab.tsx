		...
		<div className="space-y-8 max-w-6xl">
			<div className="flex justify-between items-center">
				<div className="space-y-1">
					...
					<p className="text-[10px] font-mono text-[#4a4b50] pl-6 uppercase tracking-wider">
						Karpathy-style persistent knowledge base · {stats.pageCount} pages ·{" "}
						{stats.rawCount} raw sources
					</p>
				</div>
			...
			<div className="bg-[#151619] border border-[#2d2e32] p-4 rounded-xl">
				...
				{autoStatus && autoStatus.history.length > 0 && (
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
						{autoStatus.history.slice(-4).map((r) => (
							<div
								key={r.iteration}
								className="bg-[#0a0a0c] border border-[#1a1b1e] p-3 rounded-lg"
							>
								<div className="text-[9px] font-mono text-[#4a4b50] uppercase">
									Iteration {r.iteration}
								</div>
								<div className="text-xs font-mono text-emerald-400 mt-1">
									Score: {(r.evalScore * 100).toFixed(0)}%
								</div>
								<div className="text-[9px] text-[#8E9299] mt-1 truncate">
									{r.improvement.slice(0, 60)}
								</div>
							</div>
						))}
					</div>
				...
				{(!autoStatus || autoStatus.history.length === 0) && (
					<p className="text-[10px] font-mono text-[#4a4b50]">
						No loops run yet. Start an autonomous iteration to evaluate and
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
				...
				<div className="lg:col-span-1 bg-[#151619] border border-[#2d2e32] rounded-xl p-4 h-fit sticky top-24 max-h-[70vh] overflow-y-auto custom-scrollbar">
					<h3 className="text-[10px] font-mono uppercase text-[#4a4b50] tracking-widest mb-3">
						Wiki Index
					</h3>
					{pages.length === 0 && (
						<p className="text-[10px] font-mono text-[#4a4b50] italic">
							No wiki pages yet. Run Compile All to generate from pipeline
							results.
						</p>
					)}
					<div className="space-y-1">
						{pages.map((page) => (
							...
							>
								...
								<div className="text-[8px] text-[#4a4b50] mt-0.5">
					...
					{selectedPage ? (
						<div className="font-mono">
								...
								{selectedPage.content.split("\n").map((line, i) => {
									if (line.startsWith("# "))
										return (
											<h1
												key={i}
												className="text-lg font-bold text-white mb-4 mt-0"
											>
												{line.replace("# ", "")}
											</h1>
										);
									if (line.startsWith("## "))
										return (
											<h2
												key={i}
												className="text-base font-bold text-emerald-400 mt-6 mb-2"
											>
												{line.replace("## ", "")}
											</h2>
										);
									if (line.startsWith("### "))
										return (
											<h3
												key={i}
												className="text-sm font-bold text-[#8E9299] mt-4 mb-1"
											>
												{line.replace("### ", "")}
											</h3>
										);
									if (line.startsWith("- "))
										return (
											<li
												key={i}
												className="text-[11px] text-[#A1A1AA] ml-4 list-disc"
											>
												{line.replace("- ", "")}
											</li>
										);
									if (line.startsWith("> "))
										return (
											<blockquote
												key={i}
												className="text-[11px] text-[#4a4b50] italic border-l-2 border-[#2d2e32] pl-3 my-2"
											>
												{line.replace("> ", "")}
											</blockquote>
										);
							...
							{selectedPage.related.length > 0 && (
								<div className="mt-8 pt-4 border-t border-[#2d2e32]">
									<h4 className="text-[10px] font-mono uppercase text-[#4a4b50] tracking-widest mb-2">
										Related Pages
									</h4>
						...
						<div className="flex flex-col items-center justify-center h-full text-[#4a4b50]">
							...
							<p className="text-xs font-mono">
								Select a wiki page from the index
							</p>
							<p className="text-[10px] font-mono mt-2">
								Pages are compiled by DeepSeek from pipeline artifacts
							</p>
						</div>
					)}
				</div>
