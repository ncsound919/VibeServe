...
export const PipelineTab = ({
	...
	const [ingestedRepos, setIngestedRepos] = useState<
		Array<{ owner: string; repo: string; addedAt: string }>
	>([]);
	...
	const handleIngestRepo = async (urlOverride?: string) => {
		...
		const match =
			targetUrl.match(/github\.com\/([^/]+)\/([^/\s.#]+)/) ||
			targetUrl.match(/^([^/]+)\/([^/\s.#]+)$/);
		if (!match) return;

		// If it's just owner/repo format, reconstruct the URL
		const finalUrl = targetUrl.includes("github.com")
			? targetUrl
			: `https://github.com/${targetUrl}`;
		const owner = match[1];
		const repo = match[2];
		...
		try {
			...
			if (!cloneRes.ok)
				throw new Error(
					"Clone failed: Backend service unavailable or repository not found",
				);
			...
			const wikiEntry = `# Repository: ${cleanName}\n\n**URL:** ${finalUrl}\n**Files:** ${cloned.fileCount}\n**Languages:** ${cloned.languages?.join(", ") || "unknown"}\n**Status:** Cloned and ready for pipeline analysis\n\n## Processed Files\n${
				cloned.files
					?.slice(0, 50)
					.map(
						(f: any) => `- ${f.relativePath} (${Math.round(f.size / 1024)}KB)`,
					)
					.join("\n") || ""
			}\n`;
	...
	const handleFolderUpload = async (e: ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			setUploadedFolders((prev) => [
				...prev,
				{ id: data.folderId, name: data.folderName, fileCount: data.fileCount },
			]);
		...
		<div data-testid="pipeline-tab" className="max-w-5xl">
			...
			{activeRun ? (
				<ActivePipelineRun
					...
					onReset={onReset}
					onLaunch={onLaunch}
					activeAgent={customAgents.find(
						(a) => a.id === activeRun.assignedAgentId,
					)}
				/>
			) : (
				<div className="flex flex-col items-center justify-center py-24 bg-[#151619]/30 border border-dashed border-[#2d2e32] rounded-3xl">
					...
					<h3 className="text-white font-medium mb-2">
						No Active Pipeline Sessions
					</h3>
					<p className="text-[#8E9299] text-xs font-mono text-center max-w-xs px-8">
						Select a trending repository or upload a local folder to initialize
						an automated build and E2E testing sequence.
					</p>
					...
					<div className="mt-6 flex items-center gap-3 text-[10px] font-mono uppercase tracking-wider">
						<div
							className={`flex items-center gap-2 px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full ${!import.meta.env.VITE_DEEPSEEK_KEY ? "opacity-50 grayscale" : ""}`}
						>
							<div
								className={`w-2 h-2 rounded-full ${import.meta.env.VITE_DEEPSEEK_KEY ? "bg-blue-400 animate-pulse" : "bg-gray-600"} shadow-[0_0_8px_rgba(96,165,250,0.5)]`}
							/>
							<span
								className={
						...
						<div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
							...
							<span className="text-emerald-400">
								WIKI: {buildPipeline.length} PAGES
							</span>
						</div>
						<div className="flex items-center gap-2 px-3 py-1.5 bg-purple-500/10 border border-purple-500/20 rounded-full">
							...
							<span className="text-purple-400">
								MCP: {uploadedFolders.length + customAgents.length} AGENTS
							</span>
						</div>
					</div>
					...
					<div className="mt-8 w-full max-w-md mx-auto">
						...
						<p className="text-[8px] font-mono text-[#4a4b50] mt-2 text-center uppercase tracking-wider">
							Paste any GitHub repo → ingested into wiki + queued for pipeline
							analysis
						</p>
					</div>
					...
					{ingestedRepos.length > 0 && (
						<div className="mt-6 w-full max-w-md mx-auto">
							<h4 className="text-[10px] font-mono text-[#4a4b50] uppercase tracking-wider mb-2">
							...
							<div className="space-y-1.5">
								{ingestedRepos.map((r, i) => (
									<div
										key={i}
										className="flex items-center justify-between px-3 py-2 bg-[#0a0a0c] border border-[#1a1b1e] rounded-lg"
									>
										<span className="text-xs font-mono text-emerald-400">
											{r.owner}/<strong className="text-white">{r.repo}</strong>
										</span>
					...
					{trendingRepos.length > 0 && (
						<div className="mt-12 w-full max-w-4xl mx-auto">
							<SectionHeader
								title="Available Upgrades from Trending GitHub Repos"
								icon={Network}
							/>
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
								{trendingRepos.slice(0, 6).map((repo) => (
									<div
										key={repo.name}
										className="flex flex-col bg-[#0a0a0c] border border-[#1a1b1e] rounded-xl p-4 hover:border-blue-500/30 transition-all hover:-translate-y-1 shadow-lg shadow-black/50 group"
									>
										<div className="flex items-start justify-between mb-2">
											<h4
												className="text-sm font-bold text-white truncate pr-2 group-hover:text-blue-400 transition-colors"
												title={repo.name}
											>
												{repo.name.split("/").pop()}
											</h4>
							...
							>
								<div className="flex items-center gap-3 mb-3">
									...
									<div className="min-w-0">
										...
										<p className="text-[10px] font-mono text-[#4a4b50]">
										...
										<div className="flex items-center justify-between">
											...
											<span
												className={`text-[10px] font-bold font-mono ${
													folder.assessment.quality.overall === "A"
														? "text-emerald-400"
														: folder.assessment.quality.overall === "B"
															? "text-blue-400"
															: "text-amber-400"
												}`}
											>
												{folder.assessment.quality.overall} (str{" "}
												{folder.assessment.quality.structureScore} ts{" "}
												{folder.assessment.quality.typeSafety})
											</span>
										</div>
										<div className="flex items-center justify-between">
											<span className="text-[10px] font-mono text-[#4a4b50]">
											...
											<span className="text-[10px] font-mono text-purple-400">
												{folder.assessment.recommendedAssignment}
											</span>
										</div>
										<div className="flex items-center justify-between">
											<span className="text-[10px] font-mono text-[#4a4b50]">
											...
											<span className="text-[10px] font-mono text-[#8E9299]">
												{Math.round(folder.assessment.confidence * 100)}%
											</span>
										</div>
										{folder.assessment.skills.length > 0 && (
											<div className="pt-1.5 border-t border-[#1a1b1e]">
												...
												{folder.assessment.skills.slice(0, 4).map((sk, i) => (
													<span
														key={i}
														className="inline-block px-1.5 py-0.5 bg-purple-500/10 border border-purple-500/20 rounded text-[8px] font-mono text-purple-400 mr-1"
													>
														{sk}
													</span>
												))}
			...
			{!activeRun && (
				<div className="mt-12">
					...
					<PipelineControlPanel steps={buildPipeline} />
				</div>
			)}
		</div>
	);
