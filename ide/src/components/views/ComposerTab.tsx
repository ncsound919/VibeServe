...
export const ComposerTab = () => {
  const [templates, setTemplates] = useState<Array<{ appType: string; name: string; description: string }>>([]);
    ...
    fetch('/api/coding/templates')
      ...
      .then(data => {
        if (data.templates) {
          ...
          if (data.templates.length > 0) setSelectedTemplate(data.templates[0].appType);
        }
      });
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success) {
        setChanges((data.files || []).map((filePath: string, i: number) => ({
          id: `change-${i}`,
          filePath,
        ...
        setLore(`Nexus Engine: Synthesized ${data.templateId} artifact at ${data.appPath}.`);
    ...
    <div className="flex flex-col h-[calc(100vh-180px)] p-6 space-y-6">
      ...
      <div className="relative glass-card rounded-2xl p-4 flex flex-col gap-4">
        <div className="flex gap-4">
          ...
          >
            {templates.map(t => (
              <option key={t.appType} value={t.appType}>{t.name}</option>
            ))}
          </select>
          <input
            type="text"
        ...
        <div className="flex items-center justify-between mt-2">
          {isGenerating && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-indigo-400 animate-pulse">CHEETAH_ENGINE_ACTIVE</span>
