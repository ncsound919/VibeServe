...
export function InlineComposer({
	...
	const [phase, setPhase] = useState<"idle" | "planning" | "editing" | "done">(
		"idle",
	);
	...
	const handleSubmit = useCallback(async () => {
		...
		try {
			...
			let contextStr = "";
			try {
				const ctxRes = await fetch("/api/editor/files", {
			...
			for (const file of result.files) {
				...
				try {
					const fileRes = await fetch(
						`/api/editor/file?path=${encodeURIComponent(file.path)}`,
					);
					...
					{phase === "idle" && (
						<>
							<div className="flex items-center gap-3 px-4 py-3 border-b border-[#21262d]">
								...
								<button
									onClick={onClose}
									className="p-1.5 rounded hover:bg-[#21262d] text-[#7d8590]"
								>
									<X className="w-4 h-4" />
								</button>
							</div>
							<div className="px-4 py-2 text-[10px] text-[#484f58]">
								Agent will plan changes, search the codebase, and present diffs
								for your review
							</div>
						</>
					)}
