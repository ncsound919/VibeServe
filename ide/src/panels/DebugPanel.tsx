				...
				{(["variables", "watch", "callstack", "breakpoints"] as const).map(
					(tab) => (
						<button
							...
							style={{
								background:
									activeTab === tab ? "var(--accent)" : "var(--bg-tertiary)",
								color:
									activeTab === tab
										? "var(--text-on-accent)"
										: "var(--text-muted)",
				...
				{activeTab === "callstack" && (
					<div className="p-3 space-y-1">
						{stackFrames.length === 0 ? (
							...
							>
								{isRunning
									? "Paused state will show here."
									: "No call stack. Start debugging to see paused state."}
							</div>
						) : (
					...
					<div className="p-3 space-y-1">
						...
						{breakpoints.length === 0 ? (
							<div
								...
								style={{ color: "var(--text-muted)" }}
							>
								No breakpoints. Click the gutter in the editor to add
								breakpoints.
							</div>
						) : (
							breakpoints.map((bp) => (
								...
								>
									...
									<span
										style={{
											color: bp.enabled ? "var(--error)" : "var(--text-muted)",
										}}
									>
										●
									</span>
