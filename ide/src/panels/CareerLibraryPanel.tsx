  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ backgroundColor: 'var(--bg-secondary, #1e1e2e)' }}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        ...
        {loading && (
          <div className="text-xs" style={{ color: 'var(--text-muted, #6c7086)' }}>Searching across all indexed repos...</div>
        )}
        {!query && !results && (
          <div className="text-xs py-4 text-center" style={{ color: 'var(--text-muted, #6c7086)' }}>
            ...
            <br />
            <span className="mt-1 block">Start with a component name, function, or hook.</span>
          </div>
        )}
        {results && results.results.length === 0 && query && (
          <div className="text-xs py-4 text-center" style={{ color: 'var(--text-muted, #6c7086)' }}>
            ...
            <br />
            <span className="mt-1 block">Try indexing more repos or searching with a different term.</span>
          </div>
        )}
        {results && results.results.length > 0 && (
          <div className="space-y-1.5">
            <div className="text-xxs mb-2" style={{ color: 'var(--text-muted, #6c7086)' }}>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: (kindColors[s.kind] || '#6c7086') + '22', color: kindColors[s.kind] || '#6c7086' }}
                  >
                    {s.kind}
                  </span>
                </div>
                <div className="text-xxs mt-1 flex items-center gap-2" style={{ color: 'var(--text-muted, #6c7086)' }}>
                  <span style={{ color: 'var(--accent, #89b4fa)' }}>{s.repo_key}</span>
                  <span>{s.file_path}:{s.line}</span>
                  {s.exported && (
                    <span className="px-1 py-0.5 rounded" style={{ backgroundColor: 'var(--bg-primary, #1e1e2e)', fontSize: '0.6rem' }}>
                      exported
                    </span>
                  )}
                </div>
