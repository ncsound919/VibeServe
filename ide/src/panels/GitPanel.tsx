...
export function GitPanel() {
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', message: `Committed: ${message.slice(0, 40)}${message.length > 40 ? '...' : ''}` });
      setMessage('');
      setStaged(new Set());
      await load();
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="p-4 text-xs" style={{ color: 'var(--text-muted)' }}>Loading git status...</div>;
  }

  if (!isRepo) {
    return (
      <div className="flex flex-col h-full">
        <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Source Control</div>
        <div className="flex-1 flex flex-col items-center justify-center p-6 gap-4">
          <div className="text-xs text-center" style={{ color: 'var(--text-muted)' }}>
            No git repository found in this workspace.
          </div>
          <button
            onClick={async () => { await gitService.init(); await load(); addToast({ type: 'success', message: 'Git repository initialized' }); }}
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-3 py-2" style={{ borderBottom: '1px solid var(--border)' }}>
          <button onClick={() => setViewingDiff(false)} className="text-xs px-2 py-1 rounded" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
            ← Back
          </button>
          <span className="text-[11px] font-semibold uppercase" style={{ color: 'var(--text-muted)' }}>Diff</span>
          <div className="w-12" />
        </div>
        <div className="flex-1 overflow-auto p-3">
          <pre className="text-xs font-mono whitespace-pre-wrap" style={{ color: 'var(--text-secondary)', lineHeight: '1.6' }}>
            {diffText?.split('\n').map((line, i) => (
              <div
                key={i}
                style={{
                  color: line.startsWith('+') ? 'var(--success)' : line.startsWith('-') ? 'var(--error)' : line.startsWith('@@') ? 'var(--accent)' : 'var(--text-secondary)',
                }}
              >
                {line}
              </div>
  ...
  const statusColors: Record<string, string> = {
    modified: 'var(--git-modified)', added: 'var(--git-added)', deleted: 'var(--git-deleted)', untracked: 'var(--git-untracked)',
  };
  ...
  return (
    <div className="flex flex-col h-full text-xs">
      ...
      {statuses.length === 0 ? (
        <div className="flex-1 flex items-center justify-center p-4 text-xs" style={{ color: 'var(--text-muted)' }}>
          Working tree clean
        </div>
      ) : (
        <div className="flex-1 overflow-y-auto">
          ...
          {statuses.map((s) => (
            <div
              ...
              style={{ background: staged.has(s.path) ? 'rgba(83,109,254,0.1)' : 'transparent' }}
            >
              <input type="checkbox" checked={staged.has(s.path)} onChange={(e) => { e.stopPropagation(); handleStage(s.path); }} className="w-3 h-3" />
              ...
              <span className="shrink-0 text-[10px]" style={{ color: 'var(--text-muted)' }}>{s.status.charAt(0).toUpperCase()}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ borderTop: '1px solid var(--border)' }}>
        <div className="p-3">
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button onClick={handleViewDiff} className="flex-1 px-3 py-1.5 rounded text-xs font-medium" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
              View Diff
            </button>
          </div>
        </div>
