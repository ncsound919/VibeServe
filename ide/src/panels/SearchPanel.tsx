  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex gap-2">
          <button
            ...
            style={{
              ...
              color: mode === 'files' ? 'var(--text-on-accent)' : 'var(--text-muted)',
            }}
          >
            Files
          </button>
          <button
            ...
            className="flex-1 text-[11px] py-1 rounded font-medium"
            style={{
              background: mode === 'content' ? 'var(--accent)' : 'var(--bg-tertiary)',
              color: mode === 'content' ? 'var(--text-on-accent)' : 'var(--text-muted)',
            }}
          >
            Content
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-3">
        {loading && <div className="py-4 text-xs text-center" style={{ color: 'var(--text-muted)' }}>Searching...</div>}
        {error && <div className="py-4 text-xs text-center" style={{ color: 'var(--error)' }}>{error}</div>}
        {!loading && !error && results.length === 0 && query.trim() && (
          <div className="py-4 text-xs text-center" style={{ color: 'var(--text-muted)' }}>No results found</div>
        )}
        {results.map((r, i) => (
          <div
            key={i}
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') handleResultClick(r); }}
