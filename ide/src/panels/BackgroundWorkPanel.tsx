...
export function BackgroundWorkPanel() {
	...
	const loadSuggestions = useCallback(async () => {
		try {
			const res = await fetch(`${apiBase}/api/pipeline/suggestions/pending`, {
	...
	const loadAll = useCallback(async () => {
		...
		try {
			...
			const reposRes = await callMcp("list_indexed_repos").catch(() => ({
				repos: [],
			}));
	...
	useEffect(() => {
		...
		import("../ws-client").then(({ createWSClient }) => {
			const wsUrl = apiBase
				...
				: `ws://${window.location.hostname}:3002/ws`;
			const token = localStorage.getItem("nexus_token");
	...
	const handleApply = async (s: Suggestion, e: React.MouseEvent) => {
		...
		try {
				...
				setSuggestions((prev) =>
					prev.map((x) =>
						x.id === s.id
							? {
									...x,
									verificationStatus: result.verification?.status || "not-run",
								}
							: x,
					),
				);
			...
			import("../stores/toastStore").then(({ useToastStore }) => {
				useToastStore
					.getState()
					.addToast({ type: "error", message: `Apply error: ${err.message}` });
			});
		}
	};
	...
	const tabs: { id: Tab; label: string; count?: number }[] = [
		...
		},
		{
			id: "reuse",
			label: "Reuse",
			count: suggestions.filter((s) => s.type === "reuse").length,
		},
		{
			id: "test-gaps",
			label: "Test Gaps",
			count: suggestions.filter((s) => s.type === "test").length,
		},
		{
			id: "refactors",
			label: "Refactors",
			count: suggestions.filter((s) => s.type === "refactor").length,
		},
	];
				...
				{tabs.map((tab) => (
					...
					>
						...
						{tab.count !== undefined && tab.count > 0 && (
							<span
								className="ml-1 px-1 py-0.5 rounded-full text-xxs"
						...
						onClick={async () => {
							...
							try {
								const res = await fetch(
									`${apiBase}/api/pipeline/scheduler/trigger`,
									{
										method: "POST",
										headers: {
											"Content-Type": "application/json",
				...
				{!loading && suggestions.length > 0 && (
					...
					>
						...
						>
							...
							{suggestions.length !== 1 ? "s" : ""} while you were away
						</div>
						<div
						...
						>
							{(() => {
								const goalIds = new Set(
									suggestions.filter((s) => s.goalId).map((s) => s.goalId),
								);
						...
						{Object.entries(goalGroups).map(([goalKey, items]) => (
							<div key={goalKey}>
								{goalKey !== "unaligned" && items[0]?.goalTitle && (
									<div
										className="text-xxs font-semibold mb-1 px-1"
								...
								{items.map((s) => (
									<div
										...
										style={{
											backgroundColor: "var(--bg-tertiary, #313244)",
											borderLeft: `3px solid ${typeColors[s.type] || "#6c7086"}`,
										}}
									>
										<div className="flex items-start justify-between">
											<div className="flex-1">
												...
												<div className="flex items-center gap-1 mt-1">
													{s.goalId ? (
														<button
															onClick={(e) => openGoalInAgenda(s.goalId, e)}
											...
											<div className="flex flex-col items-end gap-1 ml-2">
												...
												>
													{s.verificationStatus === "passing"
														? "OK"
														: s.verificationStatus === "failing"
															? "FAIL"
