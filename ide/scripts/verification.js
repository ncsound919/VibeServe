...
try {
	...
	const buildContent = fs.readFileSync(
		"./dist/assets/index-CkCJmglp.js",
		"utf8",
	);
	if (
		buildContent.includes("executeAgent") &&
		buildContent.includes("addEpisodic")
	) {
...
try {
	...
	if (
		buildContent.includes("terminal") &&
		buildContent.includes("git") &&
		buildContent.includes("filesystem")
	) {
		console.log(
			"✓ Tool adapters (terminal, git, filesystem) detected in build",
		);
	} else {
...
try {
	...
	if (previewExists && extensionsExists && systemExists) {
		console.log(
			"✓ New UI components (Preview, Extensions, System) detected in build",
		);
	} else {
