...
ws.on("message", (data) => {
	...
	if (
		activeStep &&
		!shownPhases.has(activeStep.phase) &&
	...
	if (exec.status === "success" || exec.status === "failed") {
		...
		console.log(
			`║  Progress: ${Math.round(exec.progress)}%. Phases: ${shownPhases.size}/10. Logs: ${exec.logs.length}║`,
		);
		console.log(`╚══════════════════════════════════════════════╝\x1b[0m`);
		...
		if (exec.e2eResults && exec.e2eResults.length > 0) {
			...
			for (const r of exec.e2eResults) {
				...
				console.log(
					`  ${icon}\x1b[0m ${r.testName} (${r.duration}ms) - ${r.logs[0]}`,
				);
			}
		}
		...
		if (exec.status === "failed") {
			...
			if (failed)
				console.log(`\x1b[31m  Pipeline failed at: ${failed.testName}\x1b[0m`);
		}
