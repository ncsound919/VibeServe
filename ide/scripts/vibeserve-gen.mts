 ...
 */
import { initVibeServeClient, callMcpTool, listMcpTools, disconnectMcp } from '../src/server/mcpClient';
import fs from 'fs';
...
async function main() {
  ...
  for (const t of tools) {
    console.log(`  • ${t.name}: ${t.description?.substring(0, 80) ?? 'no description'}`);
  }
  ...
  try {
    const archResult = await callMcpTool('aethernexus_vibe_architect', {
      intent: 'Redesign Nexus Alpha IDE with progressive pipeline sidebar, dashboard, toast notifications, and professional polish',
      design_system: 'Custom dark theme (#0A0A0B background, emerald/amber/rose/indigo color system)',
      target_stack: 'React 18 + TypeScript + Tailwind CSS v4 + Zustand + framer-motion + lucide-react',
...
Controls: sliders, toggles, knobs with micro-interactions.`,
    ...
    console.log('Architect result:', JSON.stringify(archResult, null, 2).substring(0, 500));
  } catch (err: any) {
    console.log('Architect failed (expected — may need different tool):', err.message);
  }
  ...
  try {
    const specResult = await callMcpTool('aethernexus_generate_ui_spec', {
      project_intent: 'Professional pipeline sidebar + dashboard + toast notifications for Nexus Alpha IDE',
      target_stack: 'React + TypeScript + Tailwind CSS v4 + framer-motion',
      max_iterations: 3,
      description: SPEC,
    });
    console.log('UI Spec result:', JSON.stringify(specResult, null, 2).substring(0, 1000));
  ...
  try {
    const codeResult = await callMcpTool('aethernexus_vibe_code', {
      architecture_plan: JSON.stringify({
        ...
        styling: 'Tailwind CSS v4, dark theme (#0A0A0B base), emerald/amber/rose/indigo color system',
    ...
    console.log('Code result:', JSON.stringify(codeResult, null, 2).substring(0, 2000));
  ...
  try {
    ...
    console.log('Validate result:', JSON.stringify(validateResult, null, 2).substring(0, 500));
  } catch (err: any) {
...
function extractAndWriteFiles(result: any) {
  ...
  if (fileCount === 0) {
    ...
    console.log('  No structured files found. Saved raw output to .planning/vibeserve-raw-output.txt');
  }
}
