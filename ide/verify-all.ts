...
async function verifyAll() {
  ...
  // Primary tabs
  const primary = ['Composer', 'Editor', 'Review', 'Magic', 'Memory', 'Preview'];
  ...
  const advanced = ['Overview', 'Pipeline', 'Activity', 'History', 'Audit', 'Mission Control', 'Changes', 'Command Center', 'Settings', 'Extensions', 'System', 'Agent Eval'];
  console.log('\n=== ADVANCED TABS ===');
  for (const name of advanced) {
    ...
    const hasError = content.toLowerCase().includes('failed to load') && content.length < 200;
