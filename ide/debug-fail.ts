...
async function debugFailures() {
  ...
  const failing = ['#nav-item-mission-control', '#nav-item-changes', '#nav-item-command-center', '#nav-item-settings'];
  ...
  for (const tabId of failing) {
    ...
    console.log(`Errors: ${jsErrors.map(e => e.substring(0,60)).join(', ')}`);
  }
