...
async function checkFailing() {
  ...
  for (const tab of tabs) {
    ...
    const hasError = errors.length > 0 || content.toLowerCase().includes('failed');
    console.log(`${tab.name}: ${hasError ? 'FAIL' : 'OK'} - ${errors[0]?.substring(0,60) || content.substring(0,40)}`);
  }
