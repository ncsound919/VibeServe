...
async function testAllTabs() {
  ...
  const results: { tab: string; status: string; content: string; errors: string[] }[] = [];
  ...
  for (const tab of ALL_TABS) {
    ...
    if (exists === 0) {
      ...
      results.push({ tab: tab.name, status: 'BUTTON NOT FOUND', content: '', errors: [] });
    ...
    const hasError = mainContent.includes('failed to load') || mainContent.includes('Error');
    
    page.off('console', handler);
    
    if (!hasContent || hasError) {
      console.log(`  ❌ No content or error`);
      results.push({ tab: tab.name, status: 'FAILED', content: mainContent.substring(0, 100), errors: tabErrors });
    } else {
      console.log(`  ✅ OK (${mainContent.length} chars)`);
      results.push({ tab: tab.name, status: 'OK', content: mainContent.substring(0, 100), errors: tabErrors.slice(0, 3) });
    }
  }
