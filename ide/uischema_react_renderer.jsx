...
export class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    if (this.props.onError) {
      ...
      console.error('UISpecRenderer ErrorBoundary caught an error:', error, errorInfo);
    }
  }
 ...
 */
export const ComponentRenderer = ({ component, tokens, onInteraction, componentMap }) => {
  ...
  const baseStyles = useMemo(() => {
    const typo = component.visual?.size
      ? {
          fontSize: `calc(1rem * ${
            component.visual.size === 'lg' ? 1.5 : component.visual.size === 'sm' ? 0.875 : 1
          })`,
        }
      : {};
  ...
  const interactiveStyles = useMemo(() => {
    let styles = { ...baseStyles, ...animationStyles };
    if (state.hovered) {
      return {
        ...styles,
        opacity: 0.9,
  ...
  const shouldBeTabbable = isInteractive && !component.accessibility?.aria_hidden;
  ...
  switch (component.type) {
    ...
    case 'card': {
      ...
      return (
        ...
        >
          {component.label && (
            <div style={{ marginBottom: tokens.getSpacing('sm'), fontWeight: 600 }}>
              {component.label}
            </div>
          )}
...
export const LayoutRenderer = ({ layout, components, tokens, onInteraction }) => {
  const gridStyle = useMemo(() => {
    const bp =
      layout.breakpoints?.desktop || layout.breakpoints?.tablet || layout.breakpoints?.mobile;
 ...
 */
export const UISpecRenderer = ({ spec, onInteraction, debug = false, loading = false }) => {
  const tokenManager = useMemo(() => createTokenManager(spec.design_system), [spec.design_system]);
  ...
  if (loading) {
    return (
      ...
      >
        <ComponentRenderer
          component={{ type: 'spinner', label: 'Loading\u2026', id: 'loading-fallback' }}
          tokens={tokenManager}
        />
      </div>
    );
          ...
          <div key={idx}>
            {layout.regions?.map((region, regionIdx) => {
              const regionComponents = effectiveComponents.filter((c) => {
                ...
                if (c.regions && Array.isArray(c.regions)) return c.regions.includes(region.id);
                return false;
              });
              return (
                <section
