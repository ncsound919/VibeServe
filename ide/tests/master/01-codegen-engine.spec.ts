...
import { API_BASE, AUTH_HDR, EXAMPLE_PROMPTS, expect, test } from "./fixtures";
...
test.describe("VibeCoder Prompt Interface", () => {
	...
	test("Generate button is disabled when textarea is empty", async ({
		page,
	}) => {
	...
	test("Generate button enables after typing at least 3 characters", async ({
		page,
	}) => {
	...
	test("Enter key in textarea triggers generation transition", async ({
		page,
	}) => {
		const ta = page.getByLabel("App description");
...
test.describe("Coding Agent API Contract", () => {
	test("GET /api/coding-agent/apps returns typed array", async ({
		request,
	}) => {
	...
	test("POST /api/pipeline/generate requires description field", async ({
		request,
	}) => {
	...
	test("POST /api/pipeline/generate accepts valid payload", async ({
		request,
	}) => {
		const res = await request.post(`${API_BASE}/api/pipeline/generate`, {
			headers: AUTH_HDR,
	...
	test("generation job status endpoint tracks progress 0–100", async ({
		request,
	}) => {
		...
		if (!startRes.ok()) {
			test.skip(true, "Gen endpoint N/A");
			return;
		}
		...
		if (!statusRes.ok()) {
			test.skip(true, "Status endpoint N/A");
			return;
		}
	...
	test("generated app history entries have required shape", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/coding-agent/apps`, {
			headers: AUTH_HDR,
	...
	for (const { prompt, expected } of cases) {
		test(`"${prompt}" → template: ${expected}`, async ({ request }) => {
			const res = await request.post(
				`${API_BASE}/api/coding-agent/template-select`,
				{
					headers: AUTH_HDR,
			...
			if (!res.ok()) {
				test.skip(true, "Template-select N/A");
				return;
			}
			const body = await res.json();
...
test.describe("VibeCoder Quality Gates", () => {
	test("POST /api/vibe/check runs all gates and returns letter grade", async ({
		request,
	}) => {
	...
	test("Project Structure gate passes (required baseline)", async ({
		request,
	}) => {
		const res = await request.post(`${API_BASE}/api/vibe/check`, {
			headers: AUTH_HDR,
		...
		const g = gates.find((g: { gate: string }) =>
			g.gate.toLowerCase().includes("structure"),
		);
	...
	test("score history accumulates across builds", async ({ request }) => {
		...
		const histRes = await request.get(`${API_BASE}/api/vibe/history`, {
			headers: AUTH_HDR,
		});
...
test.describe("Deployment Availability", () => {
	test("GET /api/deploy/availability returns deployment flags", async ({
		request,
	}) => {
	...
	test("zip deployment is always available (no external dep)", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/deploy/availability`, {
			headers: AUTH_HDR,
