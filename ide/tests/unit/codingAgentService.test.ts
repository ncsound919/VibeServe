...
describe("CodingAgentService — BOLA Guards & Safe File Writing", () => {
	...
	it("accepts descriptions of 3+ characters", async () => {
		...
		assert.strictEqual(
			result.success,
			true,
			result.message || "Generation failed unexpectedly",
		);
	...
	it("path traversal is prevented (directory confinement guard)", () => {
		...
		assert.strictEqual(
			isConfined,
			true,
			"paths inside OUT_DIR should start with OUT_DIR prefix",
		);
		...
		assert.strictEqual(
			isOutOfBounds,
			false,
			"path traversal attempts must not start with OUT_DIR prefix",
		);
	});
	...
	it("falls back to description-based template when templateId not found", async () => {
		...
		if (result.success) {
			assert.ok(
				result.templateId,
				"With no matching templateId, should fall back to description-matched template",
			);
			assert.ok(result.appPath);
		}
	});
