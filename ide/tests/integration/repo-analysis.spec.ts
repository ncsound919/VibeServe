...
test.describe("Repo Analysis", () => {
	...
	test("navigates to Repo Analysis without crashing", async ({
		nexus,
		page,
	}) => {
	...
	test("shows repo count in subtitle", async ({ nexus, page }) => {
		await nexus.navigateTo("Repo Analysis");
		await expect(page.locator("text=/d+ repos/i")).toBeVisible();
	});
	...
	test("displays repo cards with analysis data", async ({ nexus, page }) => {
		...
		await expect(
			cards.first().locator('[data-testid="repo-name"]'),
		).toBeVisible();
		await expect(
			cards.first().locator('[data-testid="repo-analysis-text"]'),
		).toBeVisible();
	});
	...
	test("RESET SELECTION clears selected repos", async ({ nexus, page }) => {
		...
		await expect(page.locator("text=/d+ Repos Ready/i")).toBeVisible();
		await page
			.locator("button", { hasText: /RESET_SELECTION/i })
			.first()
			.click();
