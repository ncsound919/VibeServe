...
test.describe("Dashboard (Overview)", () => {
	...
	test("shows 5 stat cards with numeric values", async ({ page }) => {
		...
		for (let i = 0; i < 5; i++) {
			await expect(
				cards.nth(i).locator('[data-testid="stat-value"]'),
			).toBeVisible();
		}
	});
	...
	test("shows Neural Link Active status indicator", async ({ page }) => {
		await expect(page.locator("text=/Neural Link Active/i")).toBeVisible();
	});

	test("system status indicator renders", async ({ page }) => {
		const status = page.locator(
			"text=/Nexus_IDLE|Nexus_AGENT_SYNC|Nexus_DATA_FETCH|Nexus_TEAM_DEV/i",
		);
