...
test.describe("Navigation - Tabs", () => {
	...
	test("all sidebar navigation tabs are present after expanding Advanced", async ({
		page,
	}) => {
	...
	test("navigating to each tab preserves main content", async ({
		nexus,
		page,
	}) => {
		await page.getByRole("button", { name: /Advanced/i }).click();
	...
	test("no console errors during navigation through all tabs", async ({
		nexus,
		page,
	}) => {
		await page.getByRole("button", { name: /Advanced/i }).click();
...
test.describe("Navigation - Advanced Section", () => {
	...
	test("can navigate to Pipeline tab through Advanced", async ({ page }) => {
		...
		await expect(page.getByText("Nexus Automated Pipeline")).toBeVisible({
			timeout: 10000,
		});
	});
