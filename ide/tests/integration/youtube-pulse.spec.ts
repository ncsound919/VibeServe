...
test.describe("YouTube Pulse", () => {
	...
	test("navigates to YouTube Pulse without crashing", async ({
		nexus,
		page,
	}) => {
	...
	test("shows video count in subtitle", async ({ nexus, page }) => {
		await nexus.navigateTo("YouTube Pulse");
		await expect(page.locator("text=/d+ videos/i")).toBeVisible();
	});

	test("shows DEEPSEEK CURATED badge", async ({ nexus, page }) => {
		await nexus.navigateTo("YouTube Pulse");
		await expect(page.locator("text=/DEEPSEEK CURATED/i")).toBeVisible();
	});

	test("displays 4K SCAN labels on video cards", async ({ nexus, page }) => {
		await nexus.navigateTo("YouTube Pulse");
		await expect(page.locator("text=/4K SCAN/i").first()).toBeVisible();
	});
	...
	test("video cards show title and channel", async ({ nexus, page }) => {
		...
		await expect(
			videoCard.locator('[data-testid="video-title"]'),
		).toBeVisible();
		await expect(
			videoCard.locator('[data-testid="video-channel"]'),
		).toBeVisible();
	});
	...
	test("video scan subtitle mentions YouTube RSS", async ({ nexus, page }) => {
		await nexus.navigateTo("YouTube Pulse");
		await expect(page.locator("text=/YouTube RSS/i")).toBeVisible();
	});
});
