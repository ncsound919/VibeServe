...
test.describe("Phase 3: UX & Polish", () => {
	...
	test("VibeCoder - should have single-prompt mode elements", async ({
		page,
	}) => {
		await page.goto("/");
		...
		const vibeTab = page
			.locator('button:has-text("VibeCoder"), a:has-text("VibeCoder")')
			.first();
	...
	test("Settings - should persist personalization", async ({ page }) => {
		...
		if (await settingsTab.isVisible().catch(() => false)) {
			...
			const themeSelector = page
				.locator('select[name*="theme"], button:has-text("Theme")')
				.first();
			...
			if (hasTheme) {
				...
				const fontSizeInput = page
					.locator('input[name*="font"], select[name*="font"]')
					.first();
