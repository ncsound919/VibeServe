...
test.describe("Desktop - Electron Shell", () => {
	test("Electron preload script exists and defines electronAPI", async ({
		page,
	}) => {
...
test.describe("Desktop - Privileged API Scoping", () => {
	...
	test("privileged actions require explicit user interaction", async ({
		page,
		nexus,
	}) => {
	...
	test("file system paths are sandboxed", async ({ page, nexus, request }) => {
		...
		for (const p of traversalPaths) {
			const response = await request.get(
				`/api/editor/file?path=${encodeURIComponent(p)}`,
			);
			expect(response.status()).toBeGreaterThanOrEqual(400);
		}
	});

	test("editor file writes are validated for ownership", async ({
		page,
		request,
	}) => {
