...
test.describe("Retrieval - Graphify Search", () => {
	...
	test("retrieval results are provenance-tagged", async ({ page, nexus }) => {
		...
		if ((await searchInput.count()) > 0) {
			...
			const provenanceLabels = page.getByText(
				/source|provenance|file:|layer:/i,
			);
	...
	test("cross-user graph search is isolated", async ({
		page,
		request,
		nexus,
	}) => {
		...
		const leakedSecrets = results.filter(
			(r: any) =>
				r.label?.toLowerCase().includes("secret") ||
				r.label?.toLowerCase().includes("password"),
		);
		expect(leakedSecrets.length).toBe(0);
	});

	test("malformed search queries are handled safely", async ({
		page,
		request,
	}) => {
	...
	test("retrieval output cannot silently bypass review gating", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Command Center");
		...
		const destructiveBtn = page
			.getByRole("button", { name: /apply|execute|run/i })
			.first();
...
test.describe("Retrieval - Latest Response Wins (Race Safety)", () => {
	test("latest search response replaces stale results", async ({
		page,
		nexus,
	}) => {
		let requestCount = 0;
		...
		await page.route("**/api/coding/search", async (route) => {
			...
			if (requestCount === 1) {
				...
				return route.fulfill({
					json: {
						results: [{ id: "stale", label: "stale-result", score: 0.5 }],
					},
				});
			}
			...
			return route.fulfill({
				json: {
					results: [
						{ id: "latest", label: `latest-result-${query}`, score: 0.99 },
					],
				},
			});
		});
