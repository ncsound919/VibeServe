...
test.describe("Phase 2: Interactive Previews & Artifacts", () => {
	...
	test("Multimodal Preview - should show preview panel", async ({ page }) => {
		...
		const previewTab = page
			.locator('button:has-text("Preview"), a:has-text("Preview")')
			.first();
		...
		const previewContainer = page.locator(
			"text=Click a button above to generate a preview",
		);
	...
	test("Multimodal Preview - should generate documentation", async ({
		page,
	}) => {
		// Navigate to Preview
		const previewTab = page
		...
		const docsButton = page
			.locator('button:has-text("Generate Docs"), button:has-text("Docs")')
			.first();
	...
	test("Multimodal Preview - should upload image and generate UI", async ({
		page,
	}) => {
		// Navigate to Preview
		const previewTab = page
		...
		if (
			...
			const generateButton = page
				.locator(
					'button:has-text("Generate UI from Image"), button:has-text("Generate UI")',
				)
				.first();
	...
	test("Audit Store - should log artifacts for traceability", async () => {
		...
		const { useAuditStore } = await import(
			"../../src/agents/monitoring/auditStore"
		);
	...
	test("Integration - Preview generates and logs artifacts", async ({
		page,
	}) => {
		...
		if (await previewTab.isVisible().catch(() => false)) {
			...
			// Click "Generate Docs"
			const docsButton = page
				.locator('button:has-text("Generate Docs")')
				.first();
