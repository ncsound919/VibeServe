...
test.describe("Activity Tab", () => {
	...
	test("should display active pipeline section if running", async ({
		page,
	}) => {
		await page.waitForTimeout(1000);
		const hasActiveSection = await page
			.getByText(/Active|Pipeline|Execution/i)
			.count();
	...
	test("should handle empty state for generated apps", async ({
		page,
		nexus,
	}) => {
		await page.route("**/api/editor/list", async (route) => {
	...
	test("should not have critical console errors during navigation", async ({
		nexus,
		page,
	}) => {
		const errors: string[] = [];
