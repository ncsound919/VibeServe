...
test.describe("Memory Tab", () => {
	...
	test("should display memory management interface", async ({ page }) => {
		await expect(page.getByText(/memory|Memory/i).first()).toBeVisible({
			timeout: 5000,
		});
	});

	test("should show tier filter buttons", async ({ page }) => {
		const tierButtons = page.locator("button").filter({
			hasText:
				/memory|tier|semantic|episodic|procedural|graph|error|immediate|consolidat/i,
		});
	...
	test("should switch content when tier filter is clicked", async ({
		page,
	}) => {
		...
		if ((await semanticBtn.count()) > 0) {
			...
			const activeBtn = page
				.locator("button")
				.filter({ hasText: /Semantic/i })
				.first();
			const bg = await activeBtn.evaluate(
