...
test.describe("Race - Duplicate Submit Prevention", () => {
	...
	test("rapid duplicate plan generate does not create multiple plans", async ({
		page,
		nexus,
	}) => {
		...
		const generateBtn = page
			.getByRole("button", { name: /generate|create plan/i })
			.first();
...
test.describe("Race - WebSocket Reconnection", () => {
	...
	test("WebSocket reconnect does not duplicate pipeline events", async ({
		page,
		nexus,
	}) => {
	...
	test("WebSocket connection is cleaned up on tab change", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Pipeline");
...
test.describe("Race - Queue & Workflow Integrity", () => {
	...
	test("pipeline queue enforces job ownership", async ({
		page,
		request,
		nexus,
	}) => {
		...
		const response = await request.get(
			"/api/pipeline/status/another-users-job-id",
		);
		expect([400, 401, 403, 404]).toContain(response.status());
	});

	test("queue retries are idempotent for the same job", async ({
		page,
		request,
	}) => {
...
test.describe("Race - Concurrent Editor Safety", () => {
	test.beforeEach(async ({ page, nexus }) => {
		...
		await page.route("**/api/editor/list", async (route) => {
			await route.fulfill({
				json: { apps: [{ id: "app-1", name: "test-app" }] },
			});
		});
		await nexus.goto();
	});

	test("concurrent file saves do not corrupt content", async ({
		page,
		nexus,
	}) => {
...
test.describe("Race - Webhook & Socket Redelivery", () => {
	...
	test("duplicate WebSocket messages do not double-apply state", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Pipeline");
