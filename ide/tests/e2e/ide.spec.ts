...
test.describe("IDE Shell", () => {
	...
	test("terminal panel opens only when CLI tab is active", async ({
		page,
		nexus,
	}) => {
		...
		if ((await terminalInput.count()) > 0) {
			...
			const cmdCenter = page
				.locator('[data-testid="section-header"]')
				.filter({ hasText: /Distributed Context/i });
			await expect(cmdCenter).toBeVisible();
		}
	});

	test("review UI is visible only when a plan is active", async ({
		page,
		nexus,
	}) => {
		...
		const diffPanel = page.locator(
			'[data-testid="diff-viewer"], [data-testid="plan-approval-panel"]',
		);
