...
test.describe("Review - Plan Generation & Approval", () => {
		...
		await page.route("**/api/coding/plan", async (route) => {
			await route.fulfill({
				json: {
					...
					description:
						"Restructure JWT validation with improved error handling",
					steps: [
						{
							index: 0,
						...
						{
							index: 2,
							description: "Update middleware wiring",
	...
	test("plan cannot be applied without explicit approval", async ({
		page,
		nexus,
	}) => {
		...
		const hasApprovalGate = (await approveBtn.count()) > 0;
		const applyVisible = (await applyBtn.count()) > 0;

		expect(hasApprovalGate || !applyVisible).toBe(true);
	});

	test("duplicate apply clicks do not create duplicate execution", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Command Center");
	...
	test("rejected plan does not mutate files", async ({
		page,
		request,
		nexus,
	}) => {
	...
	test("auto-fix paths review-gated when NEXUS_ALLOW_AUTO_FIX is off", async ({
		page,
		nexus,
	}) => {
		...
		if ((await autoFixLabels.count()) > 0) {
			...
			const isGated =
				...
				(await page
					.getByRole("button", { name: /destructive fix|auto fix|self heal/i })
					.count()) === 0;
...
test.describe("Review - PlanReviewTab", () => {
	...
	test("PlanReviewTab is accessible from navigation", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Command Center");
		...
		const planElements = page.locator(
			'[data-testid*="plan"], [data-testid*="review"]',
		);
