...
test.describe("Settings Tab - Full Coverage", () => {
	test.beforeEach(async ({ page, nexus }) => {
				...
				json: {
					brain: {
						lanes: [
							...
							{ name: "agent_brain", enabled: false },
						],
					},
					brainStatus: {
						running: true,
					...
					integrations: [
						{ name: "qdrant", connected: true, configured: true },
						{ name: "nanobot", connected: false, configured: true },
					],
					agents: [
						{
							id: "agent-1",
		...
		await page.route("**/api/settings/integrations", async (route) => {
			await route.fulfill({
				json: { services: { qdrant: true, nanobot: false } },
			});
		});
		await nexus.goto();
	...
	test("should show brain status section", async ({ page }) => {
		...
		const hasStatus = await page
			.getByText(/Status|Running|Port|Python/i)
			.count();
	...
	test("should display integration toggles", async ({ page }) => {
		await page.waitForTimeout(2000);
		const integrationsText = page.getByText(
			/Integration|Connected|qdrant|nanobot/i,
		);
	...
	test("should show pipeline configuration panel", async ({ page }) => {
		await page.waitForTimeout(2000);
		const hasPipelineConfig = await page
			.getByText(/Pipeline|Config|Auto.?Fix|Recovery|Retry|Timeout/i)
			.count();
