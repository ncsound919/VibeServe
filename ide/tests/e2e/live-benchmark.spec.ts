...
test.describe('Live Benchmark — Real Backend', () => {
  ...
  test('L2. Pipeline sidebar steps render', async ({ page }) => {
    ...
    for (const step of steps) {
      const t0 = Date.now();
      const visible = await page.getByText(step).first().isVisible().catch(() => false);
      record(`sidebar-${step.toLowerCase().replace(/\s+/g, '-')}`, visible, Date.now() - t0, visible ? 'live' : 'missing');
    }
  });
  ...
  test('L5. Settings link navigates to settings', async ({ page }) => {
    ...
    if (visible) {
      ...
      await page.waitForTimeout(500);
      const hasSettings = await page.getByRole('heading', { name: 'System Settings' }).isVisible().catch(() => false);
      record('settings-navigate', hasSettings, 0, hasSettings ? 'live' : 'no settings');
    } else {
  ...
  test('L6. Command bar — open, search, close (live)', async ({ page }) => {
    ...
    record('cmdbar-open', opened, Date.now() - t0, opened ? 'live' : 'no dialog');
    ...
    record('cmdbar-commands', overviewVisible || composerVisible, 0, `live (${overviewVisible}/${composerVisible})`);
    ...
    record('cmdbar-close', closed, 0, closed ? 'live closed' : 'live still visible');
  });
  ...
  test('L8. Zero critical console errors across full navigation (live)', async ({ page }) => {
    const errors: string[] = [];
    const IGNORE = [
      ...
      'unique "key" prop',
      'GEMINI_API_KEY', 'falling back to DeepSeek',
    ];
    ...
    record('no-critical-errors', errors.length === 0, 0,
      errors.length === 0 ? 'live clean' : `live ${errors.length} errors: ${errors.slice(0, 3).join('; ')}`);
  });
  ...
  test('L9. Debt Radar button state (live)', async ({ page }) => {
    ...
    record('debt-radar-live', true, Date.now() - t0, debtVisible ? 'live visible' : 'live not visible (needs WS events)');
  });
  ...
  test('L10. Trajectory Sidebar visible (live)', async ({ page }) => {
    ...
    record('trajectory-sidebar', visible, Date.now() - t0, visible ? 'live visible' : 'live hidden');
  });

  // ─── 11. API ENDPOINTS ────────────────────────────────────────────────────

  test('L11. All public API endpoints respond from real server', async ({ request }) => {
    const endpoints = [
      '/api/data/repos', '/api/nexus/progression', '/api/settings',
      '/api/coding/templates', '/api/pipeline/prs', '/api/vibe/history',
      '/api/trajectory/history', '/api/hooks', '/api/quality/stats',
      '/api/quality/trends', '/api/autocoder/status', '/api/autocoder/patterns',
      '/api/coding-agent/apps', '/api/nexus/errors', '/api/vibe/latest', '/api/toon/stats',
    ];
    ...
    for (const ep of endpoints) {
      ...
      try {
        ...
        record(`api${ep}`, res.status() >= 200 && res.status() < 500, Date.now() - t0, `HTTP ${res.status()}`);
      } catch (err: any) {
        record(`api${ep}`, false, Date.now() - t0, `fetch failed: ${err.message}`);
      }
    }
  });
