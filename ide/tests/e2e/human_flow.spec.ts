...
test.describe("Human-like Workspace Flow", () => {
	test("should execute the full developer journey: compose, review, and analyze pipeline", async ({
		page,
		nexus,
	}) => {
		...
		const prompt =
			"Create a real-time analytics dashboard with React, featuring dark mode and glassmorphism styling.";
		...
		if (!(await results.isVisible())) {
			await expect(
				page
					.locator("text=Synthesis complete")
					.or(page.locator("text=ready in Editor")),
				{ timeout: 60000 },
			).toBeVisible();
		}
		...
		const acceptBtn = fileCard
			.locator("button")
			.filter({ has: page.locator("svg.lucide-check") });
		if (await acceptBtn.isVisible()) {
			await acceptBtn.click();
			await expect(
				page.locator("text=ACCEPTED").or(page.locator("text=success")),
				{ timeout: 5000 },
			).toBeVisible();
		}
		...
		await page.waitForSelector("text=Nexus Automated Pipeline", {
			state: "visible",
			timeout: 10000,
		});
		await expect(
			nexus.getSectionHeader("Nexus Automated Pipeline"),
		).toBeVisible();
		...
		await expect(
			page.locator("text=System Metrics").or(page.locator("text=Cluster Mem")),
		).toBeVisible();
	});
});
