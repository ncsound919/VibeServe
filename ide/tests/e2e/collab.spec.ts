...
test.describe("Collaboration - Co-editing Safety", () => {
	...
	test("two browser contexts can co-edit without silent overwrite loss", async ({
		browser,
	}) => {
	...
	test("presence bar is visible when active users exist", async ({
		page,
		nexus,
	}) => {
		...
		const presenceBar = page.locator(
			'[data-testid="presence-bar"], [data-testid="presence"]',
		);
	...
	test("collaboration state does not leak across sessions", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Command Center");
...
test.describe("Collaboration - Session Boundaries", () => {
	...
	test("presence/cursor state cannot cross user boundaries", async ({
		page,
		nexus,
	}) => {
	...
	test("file tabs cannot show another users files", async ({
		page,
		nexus,
		request,
	}) => {
		...
		const response = await request.get(
			"/api/editor/file?path=..%2F..%2Fanother-user%2Fsecret-file.ts",
		);
...
test.describe("Collaboration - Disconnect/Reconnect", () => {
	...
	test("navigation between tabs does not duplicate state", async ({
		page,
		nexus,
	}) => {
		const tabs = [
