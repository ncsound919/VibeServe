...
test.describe("Nexus Core API", () => {
	...
	test("GET /api/nexus/progression returns user progress", async ({
		request,
	}) => {
	...
	test("GET /api/nexus/errors returns system error count", async ({
		request,
	}) => {
	...
	test("POST /api/tools/debt returns technical debt report", async ({
		request,
	}) => {
