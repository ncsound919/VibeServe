...
test.describe('Humanized Benchmark — Full User Journey', () => {
  ...
  test('A1. App loads with full layout', async ({ page, nexus, mockDashboard }) => {
    const t0 = Date.now();
    await nexus.goto();
  ...
  test('A2. All primary tabs load and render', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    ...
    for (const tab of primaryTabs) {
      ...
      record(`tab-${tab.toLowerCase()}`, hasContent, Date.now() - t0, hasContent ? 'ok' : 'empty');
    }
  });

  // ─── 3. ADVANCED TABS ─────────────────────────────────────────────────────

  test('A3. Advanced section expands and tabs render', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    ...
    const advancedTabs = ['Overview', 'Pipeline', 'Activity', 'History', 'Audit',
      'Mission Control', 'Changes', 'Settings', 'System', 'Agent Eval'];
    ...
    for (const tab of advancedTabs) {
      ...
      record(`tab-${tab.toLowerCase().replace(/\s+/g, '-')}`, hasContent, Date.now() - tStart);
    }
  });

  // ─── 4. COMPOSER — FULL GENERATION FLOW ────────────────────────────────────

  test('A4. Composer — template selection and prompt input', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
  ...
  test('A5. Composer — triggered generation', async ({ page, nexus }) => {
    ...
    try {
      await expect(
        page.locator('text=Agent Reasoning Trace').or(page.locator('text=Synthesis complete'))
      ).toBeVisible({ timeout: 20000 });
      record('composer-generate', true, Date.now() - t0, 'Generation started');
    } catch {
      record('composer-generate', false, Date.now() - t0, 'Generation timed out');
    }
    
    // Check for error state
    const errorState = await page.locator('text=error').isVisible().catch(() => false);
    const loreState = await page.locator('text=Agent Architectural Lore').isVisible().catch(() => false);
    record('composer-result', !errorState, 0, errorState ? 'error' : loreState ? 'lore' : 'pending');
  });

  // ─── 5. SETTINGS — ALL SUB-PANELS ──────────────────────────────────────────

  test('A6. Settings — all sub-panels load', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    ...
    for (const panel of panels) {
      const t0 = Date.now();
      const visible = await page.getByText(panel).isVisible({ timeout: 5000 }).catch(() => false);
      record(`settings-${panel.toLowerCase().replace(/\s+/g, '-')}`, visible, Date.now() - t0);
    }
  });

  test('A7. Settings — privacy mode toggle', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await nexus.openAdvancedSection();
    await nexus.navigateTo('Settings');
    await page.waitForTimeout(1500);
    ...
    record('settings-toggle-count', count > 0, Date.now() - t0, `${count} toggles found`);
    
    if (count > 0) {
      await toggleButtons.first().click();
      await page.waitForTimeout(300);
      // Should now show "Local AI Engine" text if toggled on
      const localAiVisible = await page.getByText(/Local AI Engine/).isVisible().catch(() => false);
      record('settings-privacy-toggle', true, 0, localAiVisible ? 'local mode' : 'cloud mode');
    }
  });

  // ─── 6. PIPELINE TAB ───────────────────────────────────────────────────────

  test('A8. Pipeline tab loads and shows UI', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await nexus.openAdvancedSection();
    const t0 = Date.now();
    ...
    record('pipeline-tab', hasPipeline, Date.now() - t0, hasPipeline ? 'ok' : 'empty');
  });

  // ─── 7. COMMAND BAR ────────────────────────────────────────────────────────

  test('A9. Command bar opens, shows commands, searches, closes', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await page.waitForTimeout(500);
    ...
    const overviewVisible = await page.getByText('Go to Overview').isVisible().catch(() => false);
    const composerVisible = await page.getByText('Open Composer').isVisible().catch(() => false);
    record('cmdbar-commands', overviewVisible || composerVisible, 0, `${overviewVisible}/${composerVisible}`);

    // Type to filter
    const searchInput = page.locator('[role="dialog"] input').first();
    await searchInput.fill('pipeline');
    await page.waitForTimeout(200);
    const hasResults = await page.locator('[role="option"]').first().isVisible().catch(() => false);
  ...
  test('A10. Debt Radar visible and interactive', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await page.waitForTimeout(1000);
    ...
    if (debtVisible) {
      await debtBtn.click();
      await page.waitForTimeout(300);
      const radarOpen = await page.getByText('Technical Debt Radar').isVisible().catch(() => false);
      record('debt-radar', radarOpen, Date.now() - t0, radarOpen ? 'open' : 'no popup');
    } else {
  ...
  test('A11. Trajectory Sidebar visible', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await page.waitForTimeout(1000);
    ...
    record('trajectory-sidebar', visible, Date.now() - t0, visible ? 'visible' : 'hidden');
  });

  // ─── 10. OVERVIEW TAB DATA ─────────────────────────────────────────────────

  test('A12. Overview tab shows gamified dashboard', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    await nexus.openAdvancedSection();
    const t0 = Date.now();
    ...
    const content = await nexus.main.innerText();
    const hasStats = content.includes('Level') || content.includes('XP') || content.includes('Repo');
    record('overview-data', hasStats, Date.now() - t0, hasStats ? 'data' : 'no data');
  });

  // ─── 11. NO CRITICAL ERRORS ────────────────────────────────────────────────

  test('A13. Zero critical console errors across full navigation', async ({ page, nexus, mockDashboard }) => {
    const errors: string[] = [];
    const IGNORE = [
      'favicon', 'Failed to load resource', 'net::', 'ERR_',
      'GNews', 'gnews', '429', 'Failed to fetch', 'fetch failed',
      'Multiple GoTrueClient', 'Supabase URL', 'Codeix', 'No existing',
      'circuitBreaker', 'fallback used', 'WebSocket is closed before',
    ...
    const allTabs = ['Composer', 'Editor', 'Review', 'Memory', 'Preview',
      'Overview', 'Pipeline', 'Activity', 'History', 'Audit',
      'Mission Control', 'Changes', 'Settings', 'System', 'Agent Eval'];
    ...
    record('no-critical-errors', errors.length === 0, 0,
      errors.length === 0 ? 'clean' : `${errors.length} errors: ${errors.slice(0, 2).join('; ')}`);
  });

  // ─── 12. API ENDPOINT HEALTH ───────────────────────────────────────────────

  test('A14. All public API endpoints respond', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    ...
    for (const ep of endpoints) {
      ...
      try {
        const res = await page.evaluate(async (path) => {
          const r = await fetch(path, { headers: { 'x-api-key': 'nexus-alpha-dev-key' } });
          return r.status;
        }, ep.path);
        record(`api${ep.path}`, res >= 200 && res < 500, Date.now() - t0, `HTTP ${res}`);
      } catch {
  ...
  test('A15. Rapid tab switching does not crash', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    const rapidTabs = ['Composer', 'Memory', 'Review', 'Preview', 'Editor', 'Composer'];
  ...
  test('A16. Memory tab shows interface', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    const t0 = Date.now();
    await nexus.navigateTo('Memory');
    await page.waitForTimeout(500);
    const content = await nexus.main.innerText();
    record('memory-tab', content.length > 5, Date.now() - t0, `${content.length} chars`);
  });

  // ─── 15. EDITOR TAB ────────────────────────────────────────────────────────

  test('A17. Editor tab loads Monaco area', async ({ page, nexus, mockDashboard }) => {
    await nexus.goto();
    const t0 = Date.now();
  ...
  test.afterAll(() => {
    ...
    const avgTime = Math.round(scores.filter(s => s.responseTime > 0).reduce((a, s) => a + s.responseTime, 0) / 
    ...
    for (const [cat, feats] of Object.entries(categories)) {
      ...
      const bar = '█'.repeat(Math.round(p / feats.length * 20)) + '░'.repeat(20 - Math.round(p / feats.length * 20));
