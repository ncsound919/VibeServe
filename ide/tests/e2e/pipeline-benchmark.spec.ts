...
test.describe("E-Commerce Pipeline Benchmark", () => {
	test("build baby shopify via pipeline", async ({ page }) => {
		...
		// 1. Verify IDE loaded
		await expect(page.locator("text=VibeServe").first()).toBeVisible({
			timeout: 5000,
		});
		...
		const chatResponse = await page.evaluate(async () => {
			const res = await fetch("/api/ai/chat", {
				...
				body: JSON.stringify({
					message:
						"Build a baby shopify e-commerce platform with: product catalog, shopping cart, checkout flow, user auth, order history. Use React + TypeScript + Tailwind.",
				}),
			});
			return { status: res.status, data: await res.json() };
		});
		console.log(
			`[Benchmark] AI chat response: ${chatResponse.status} — ${JSON.stringify(chatResponse.data).slice(0, 100)}`,
		);
		...
		console.log(
			`[Benchmark] Pipeline start: ${pipelineResponse.status} — ${JSON.stringify(pipelineResponse.data)}`,
		);
		...
		while (!pipelineComplete && pollCount < maxPolls) {
			...
			if (status) {
				console.log(
					`[Benchmark] Poll #${pollCount}: ${JSON.stringify(status)}`,
				);
				if (status.steps) {
		...
		console.log(
			`Total test time: ${totalTime}ms (${(totalTime / 1000).toFixed(1)}s)`,
		);
		...
		await page.screenshot({
			path: "tests/e2e/screenshots/pipeline-final.png",
			fullPage: true,
		});
