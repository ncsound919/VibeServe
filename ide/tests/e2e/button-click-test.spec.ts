...
test.describe('Human Button Test — Full Click Audit', () => {
  ...
  test('01. Dashboard renders on load', async ({ page }) => {
    ...
    const metrics = await page.getByText('Live Metrics').isVisible().catch(() => false);
    console.log(`  Live Metrics: ${metrics ? '✅' : '❌'}`);

    const activity = await page.getByText('Activity Feed').isVisible().catch(() => false);
    console.log(`  Activity Feed: ${activity ? '✅' : '❌'}`);

    const mission = await page.getByText('Mission Control').isVisible().catch(() => false);
  ...
  test('03. Fix & Retest expansion', async ({ page }) => {
    ...
    if (visible) {
      ...
      const fixSub = await page.getByText('E2E Testing').isVisible().catch(() => false);
      const refix = await page.getByText('Re-fix').isVisible().catch(() => false);
      const reaudit = await page.getByText('Re-audit').isVisible().catch(() => false);
  ...
  test('04. Settings button works', async ({ page }) => {
    ...
    if (visible) {
      ...
      const hasSystemSettings = await page.getByRole('heading', { name: 'System Settings' }).isVisible().catch(() => false);
      console.log(`Settings panel loaded: ${hasSystemSettings ? '✅' : '❌'}`);

      if (hasSystemSettings) {
        // Check sub-panels
        const panels = ['AI Provider', 'Pipeline Configuration', 'Agent Registry'];
        for (const p of panels) {
          const pVisible = await page.getByText(p).isVisible().catch(() => false);
  ...
  test('06. Command bar opens and commands work', async ({ page }) => {
    ...
    if (opened) {
      // Check commands
      const commands = ['Go to Overview', 'Open Composer', 'Open Settings', 'Open Pipeline'];
      for (const cmd of commands) {
        const cmdVisible = await page.getByText(cmd).isVisible().catch(() => false);
  ...
  test('07. Dashboard stat cards are clickable', async ({ page }) => {
    ...
    for (const card of cards) {
      const cardText = await page.getByText(card.value).first().isVisible().catch(() => false);
      console.log(`Stat card "${card.label}" (${card.value}): ${cardText ? '✅' : '❌'}`);
    }
  });
  ...
  test('08. Metrics gauges render with data', async ({ page }) => {
    ...
    for (const g of gauges) {
      const visible = await page.getByText(g).isVisible().catch(() => false);
  ...
  test('09. Activity feed events render', async ({ page }) => {
    ...
    for (const ev of events) {
      const visible = await page.getByText(ev).isVisible().catch(() => false);
  ...
  test('10. Agent cards in Mission Control', async ({ page }) => {
    ...
    for (let i = 1; i <= 3; i++) {
      const visible = await page.getByText(`Agent-${i}`).isVisible().catch(() => false);
      console.log(`Agent-${i}: ${visible ? '✅' : '❌'}`);
    }

    // Check agent status badges
    const active = await page.getByText('▶ active').isVisible().catch(() => false);
    const paused = await page.getByText('⏸ paused').isVisible().catch(() => false);
  ...
  test('11. Zero console errors on full interaction', async ({ page }) => {
    ...
    const settingsBtn = page.getByText('Settings').first();
    if (await settingsBtn.isVisible().catch(() => false)) {
      try { await settingsBtn.click({ timeout: 2000 }); } catch {}
      await page.waitForTimeout(500);
    }
