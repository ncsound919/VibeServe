...
test.describe("System Benchmarks", () => {
	...
	test("concurrent API calls", async ({ page }) => {
		...
		console.log(
			`Concurrent API test: ${duration}ms for ${endpoints.length} endpoints`,
		);
...
test.describe("Edge Cases", () => {
	test.skip("concurrent navigation race - known race condition", async ({
		page,
		nexus,
	}) => {
		await nexus.goto();
	...
	test.skip("deep component tree rendering - timeout on rapid switch", async ({
		page,
		nexus,
	}) => {
		await nexus.navigateTo("Overview");
