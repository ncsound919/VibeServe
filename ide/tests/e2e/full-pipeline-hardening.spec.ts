...
test.describe("Security — Command Injection Prevention", () => {
	test("POST /api/brain/browser rejects disallowed commands", async ({
		request,
	}) => {
	...
	test("POST /api/brain/browser allows only allowlisted commands", async ({
		request,
	}) => {
...
test.describe("Security — Path Traversal Prevention", () => {
	test("GET /api/editor/file rejects path traversal", async ({ request }) => {
		...
		for (const path of traversals) {
			const res = await request.get(
				`${API_BASE}/api/editor/file?path=${encodeURIComponent(path)}`,
				{
					headers: AUTH_HEADERS,
				},
			);
	...
	test("POST /api/editor/file rejects writes to traversal paths", async ({
		request,
	}) => {
...
test.describe("Security — Secret Leak Prevention", () => {
	...
	test("GET /api/secrets returns masked values only", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/secrets`, {
			headers: AUTH_HEADERS,
		});
		if (res.ok()) {
			const body = await res.json();
...
test.describe("Race Conditions — Concurrent Pipelines", () => {
	test("concurrent pipeline submissions do not crash the server", async ({
		request,
	}) => {
	...
	test("concurrent editor saves produce consistent state", async ({
		request,
	}) => {
...
test.describe("Pipeline Integrity — Phase Ordering", () => {
	...
	test("health endpoint is always available during pipeline run", async ({
		request,
	}) => {
		// Start a pipeline
		request.post(`${API_BASE}/api/pipeline/run`, {
...
test.describe("Cross-Component — Integration Probes", () => {
	test("integration status endpoint enumerates all services", async ({
		request,
	}) => {
	...
	test("autocoder patterns endpoint returns valid patterns", async ({
		request,
	}) => {
	...
	test("audit stats endpoint returns correct structure", async ({
		request,
	}) => {
...
test.describe("Graceful Degradation — Service Unavailability", () => {
	test("gemini proxy returns 503 when API key is missing", async ({
		request,
	}) => {
...
test.describe("UI — Tab Navigation Stress Test", () => {
	...
	test("rapid tab switching does not crash the IDE", async ({
		page,
		nexus,
	}) => {
		const tabs = [
	...
	test("back-to-back identical tab clicks are safe", async ({
		page,
		nexus,
	}) => {
		for (let i = 0; i < 5; i++) {
