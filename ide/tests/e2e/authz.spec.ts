...
test.describe("Authorization - BOLA / Ownership Protection", () => {
	test("cannot access editor files belonging to another user", async ({
		request,
	}) => {
		const response = await request.get(
			`/api/editor/file?path=..%2F..%2Fanother-user%2Fsecret.txt`,
		);
	...
	test("pipeline status requires valid ownership", async ({ request }) => {
		const response = await request.get(
			"/api/pipeline/status/nonexistent-job-id",
		);
...
test.describe("Authorization - Rate Limiting", () => {
	test("strict limiter triggers on expensive endpoints", async ({
		request,
	}) => {
...
test.describe("Authorization - Hidden UI Bypass Prevention", () => {
	...
	test("direct POST to coding/plan/apply requires auth", async ({
		request,
	}) => {
