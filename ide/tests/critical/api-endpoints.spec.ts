...
test.describe("API Endpoints", () => {
	...
	test("GET /api/pipeline/status returns ws client count", async ({
		request,
	}) => {
	...
	test("GET /api/integrations/status returns service states", async ({
		request,
	}) => {
	...
	test("POST /api/integrations/agent/chat requires message", async ({
		request,
	}) => {
	...
	test("POST /api/integrations/search/web works with query", async ({
		request,
	}) => {
	...
	test("POST /api/integrations/memory/add requires userId and content", async ({
		request,
	}) => {
	...
	test("GET /api/wiki returns pages array", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/wiki`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("POST /api/wiki/ingest requires source and content", async ({
		request,
	}) => {
	...
	test("GET /api/wiki/lint returns results", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/wiki/lint`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/nexus/errors returns error stats", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/nexus/errors`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("POST /api/nexus/errors/track requires message and phase", async ({
		request,
	}) => {
	...
	test("POST /api/nexus/errors/track creates error entry", async ({
		request,
	}) => {
		const res = await request.post(`${API_BASE}/api/nexus/errors/track`, {
			headers: AUTH_HEADERS,
	...
	test("POST /api/nexus/errors/resolve requires errorId", async ({
		request,
	}) => {
	...
	test("GET /api/nexus/errors/recovery returns patterns", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/nexus/errors/recovery`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/nexus/progression returns gamification data", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/nexus/progression`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/settings/brain/status returns runtime info", async ({
		request,
	}) => {
	...
	test("GET /api/settings/brain/config returns lane config", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/settings/brain/config`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/settings/integrations returns service statuses", async ({
		request,
	}) => {
	...
	test("POST /api/autoresearch/start begins autonomous loop", async ({
		request,
	}) => {
	...
	test("GET /api/autoresearch/status returns loop state", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/autoresearch/status`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
	});
	...
	test("GET /api/graphify/summary returns graph state", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/graphify/summary`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/local-infra returns infrastructure status", async ({
		request,
	}) => {
	...
	test("POST /api/folders/upload requires name and files", async ({
		request,
	}) => {
	...
	test("POST /api/github/push-files requires mandatory fields", async ({
		request,
	}) => {
	...
	test("GET /api/wiki/raw returns raw files", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/wiki/raw`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
		const body = await res.json();
	...
	test("GET /api/wiki/:slug returns 404 for unknown page", async ({
		request,
	}) => {
		const res = await request.get(`${API_BASE}/api/wiki/nonexistent-page`, {
			headers: AUTH_HEADERS,
		});
		expect(res.status()).toBe(404);
	});

	test("POST /api/wiki/compile-all compiles all raw files", async ({
		request,
	}) => {
	...
	test("GET /api/toon/stats returns compression stats", async ({ request }) => {
		const res = await request.get(`${API_BASE}/api/toon/stats`, {
			headers: AUTH_HEADERS,
		});
		expect(res.ok()).toBe(true);
	});
	...
	test("POST /api/proxy/cli/stream requires provider and messages", async ({
		request,
	}) => {
