...
test.describe("Pipeline", () => {
	...
	test("Browse Repositories button navigates to Repo Analysis", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Pipeline");
		const browseBtn = page.locator("button", {
			hasText: /Browse Repositories/i,
		});
		await browseBtn.click();
		await expect(page.locator("#nav-item-repo-analysis")).toHaveClass(
			/text-emerald-400/,
		);
	});

	test("repo URL input and ingest button are present", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Pipeline");
	...
	test("Upload Folder button is present and clickable", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Pipeline");
	...
	test("Static Blueprint Analysis section is visible", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Pipeline");
