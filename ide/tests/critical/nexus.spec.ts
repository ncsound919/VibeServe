...
test.describe("Navigation", () => {
	test("can navigate to all main tabs", async ({ page }) => {
		...
		const tabs = ["Overview", "Pipeline", "Agents", "Repos", "Browser", "CLI"];
		...
		if (await pipelineTab.isVisible()) {
			...
			if (await runBtn.isVisible()) {
				...
				const runningIndicator = page.locator(
					"text=/running|in progress|building/i",
				);
				const hasActivity =
					(await progressBar.count()) > 0 ||
					(await runningIndicator.count()) > 0;
		...
		if (await cliTab.isVisible()) {
			...
			if (await input.isVisible()) {
				...
				const output = page.locator(
					"[data-testid='cli-output'], .cli-output, pre, code",
				);
...
test.describe("YouTube Pulse Tab", () => {
	test("navigates to YouTube Pulse without crashing", async ({ page }) => {
		...
		if (await ytTab.isVisible()) {
			...
			const hasContent =
				(await videoCards.count()) > 0 || (await emptyMessage.count()) > 0;
...
test.describe("Repo Analysis Tab", () => {
	test("navigates to Repo Analysis without crashing", async ({ page }) => {
		...
		if (await repoTab.isVisible()) {
			...
			const hasContent =
				(await repoCards.count()) > 0 || (await emptyMessage.count()) > 0;
			expect(hasContent).toBe(true);
		}
	});
