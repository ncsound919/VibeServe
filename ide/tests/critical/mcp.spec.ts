...
test.describe("MCP Bridge Status", () => {
	...
	test("MCP Bridge card shows active servers and connections", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Command Center");
	...
	test("MCP bridge has RESTART_MCP text", async ({ nexus, page }) => {
		await nexus.navigateTo("Command Center");
		await expect(page.locator("text=/RESTART_MCP/i")).toBeVisible();
	});

	test("Context Routing section shows Optimized status", async ({
		nexus,
		page,
	}) => {
		await nexus.navigateTo("Command Center");
	...
	test("deepseek integration card is visible", async ({ nexus, page }) => {
		await nexus.navigateTo("Command Center");
		await expect(page.locator("text=/DeepSeek-V4 Integration/i")).toBeVisible();
	});

	test("live reasoning chain shows 3 steps", async ({ nexus, page }) => {
		await nexus.navigateTo("Command Center");
		await expect(page.locator("text=/Live Reasoning Chain/i")).toBeVisible();
		const steps = [
			/Intent Extraction/i,
		...
		for (const step of steps) {
			await expect(
				page.locator(step as unknown as string).first(),
			).toBeVisible();
		}
	});
});
