...
test.describe("Pipeline Brain Integration", () => {
	test("pipeline shows brain activity logs", async ({ page }) => {
		...
		const hasPipelineContent =
			(await page.locator("text=/Build|Pipeline|Step|Environment/i").count()) >
			0;
	...
	test("pipeline brain and browser logs are displayed", async ({ page }) => {
		...
		const nexusReference =
			bodyText?.includes("Nexus") || bodyText?.includes("nexus");
...
test.describe("Dashboard Data Integrity", () => {
	...
	test("mock data banner shows when applicable", async ({ page }) => {
		...
		const banner = page.locator(
			"text=/DEMO MODE|Simulated data|Set GEMINI_API_KEY/i",
		);
...
test.describe("YouTube Pulse", () => {
	test("youtube pulse tab loads without crash", async ({ page }) => {
		...
		const ytTab = page
			.locator("button")
			.filter({ hasText: /YouTube Pulse/i })
			.first();
		...
		const hasContent =
			(await page
				.locator("text=/4K SCAN|No videos|Video Scan|videos/i")
				.count()) > 0;
...
test.describe("Repo Analysis", () => {
	test("repo analysis tab loads without crash", async ({ page }) => {
		...
		const repoTab = page
			.locator("button")
			.filter({ hasText: /Repo Analysis/i })
			.first();
		...
		const hasContent =
			(await page
				.locator(
					"text=/Analysis:|No repositories|Source Intelligence|Repository/i",
				)
				.count()) > 0;
		expect(hasContent).toBe(true);
	});
...
test.describe("API Proxy", () => {
	test("api proxy endpoints 404 gracefully on server down", async ({
		page,
	}) => {
