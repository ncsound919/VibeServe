...
test.describe("License Gate", () => {
	...
	test("shows license gate when unlicensed", async ({ page }) => {
		await expect(page.getByText("Unlock Unlimited Generations")).toBeVisible({
			timeout: 10000,
		});
	...
	test("zero telemetry message visible", async ({ page }) => {
		await expect(page.getByText(/Zero telemetry/i)).toBeVisible({
			timeout: 10000,
		});
	});
	...
	test("Try Free button shows trial count text", async ({ page }) => {
		...
		await tryFree.click();
		await expect(page.getByText(/generation|remaining in trial/i)).toBeAttached(
			{ timeout: 5000 },
		);
	});
