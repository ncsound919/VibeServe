...
test.describe("Error Tracking & Self-Healing", () => {
	...
	test("shows total errors count", async ({ page }) => {
		...
		await expect(
			totalCard.locator('[data-testid="error-stat-value"]'),
		).toContainText(/\d+/);
	});
	...
	test("resolved errors show check icon", async ({ page }) => {
		const resolvedItems = page.locator(
			'[data-testid="error-item"][data-resolved="true"]',
		);
	...
	test("active failures badge shows when streak > 0", async ({ page }) => {
		await expect(page.locator("text=/Active Failures/i")).toBeVisible();
	});
});
