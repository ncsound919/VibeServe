...
test("Execute SQL in Supabase via browser", async ({ page, context }) => {
	...
	const newQueryButton = page.locator(
		'button:has-text("New query"), button:has-text("New Query")',
	);
	...
	for (const editor of possibleEditors) {
		if (await editor.isVisible({ timeout: 2000 }).catch(() => false)) {
			console.log(
				"[4] Found editor:",
				await editor.evaluate((el) => el.tagName),
			);
			foundEditor = editor;
			break;
		}
	...
	if (foundEditor) {
		...
		const runBtn = page
			.locator('button:has-text("Run"), [class*="run"]')
			.first();
	...
	if (page.url().includes("sql") || page.url().includes("project")) {
		console.log(
			"[INFO] Schema should be created if SQL was entered successfully",
		);
		console.log(
			"   You can verify by checking the tables in the Supabase dashboard",
		);
	}
});
