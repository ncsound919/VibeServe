...
All handlers use Pydantic DTO serialization (model_dump), input validation,
and the audit_tool decorator for rate limiting + audit logging + correlation IDs.
"""
...
from vibeserve.models import (
    ArchitectInput, CodeInput, ReviewInput, VerifyInput, IterateInput,
    TestInput, DeployInput, DesignInput, BuildProInput, DocsInput,
    BenchmarkInput,
    ArchitectResponse, CodeResponse, ReviewResponse, VerifyResponse,
    IterateResponse, TestResponse, DeployResponse,
)
...
async def vibe_architect_tool(ctx, intent: str, constraints=None,
    ...
    plan = await architect.plan(validated.intent, constraints, context, validated.target_stack)
...
async def vibe_code_tool(ctx, intent: str, plan: Dict[str, Any], constraints=None,
    ...
    vibe_plan = VibePlan(intent=validated.intent, decisions=decisions,
    ...
    files = await implementer.implement(vibe_plan, validated.intent, constraints, validated.target_language)
...
async def vibe_review_tool(ctx, files: List[Dict[str, Any]], requirements: List[str]) -> Dict[str, Any]:
    ...
    result = await reviewer.review_code(code_files, validated.requirements)
    await ctx.report_progress(100, 100, "Complete!")
    return ReviewResponse(**result).model_dump()
...
async def vibe_verify_tool(ctx, specification=None,
    ...
    if validated.specification:
        results["spec_validation"] = VibeVerifier.verify_spec(validated.specification)
    if validated.files:
        results["code_quality"] = VibeVerifier.verify_code_quality([CodeFile(**f) for f in validated.files])
    await ctx.report_progress(100, 100, "Complete!")
...
async def vibe_iterate_tool(ctx, specification: Dict[str, Any], requirements: List[str],
    ...
    return IterateResponse(
        ...
        iterations=[h.model_dump() for h in history],
...
async def vibe_test_tool(ctx, files: List[Dict[str, Any]], requirements=None,
    ...
    test_files = await tester.generate_tests(code_files, requirements, validated.test_framework)
    quality = VibeVerifier.verify_code_quality(test_files)
    return TestResponse(
        test_files=[f.model_dump() for f in test_files],
...
async def vibe_deploy_tool(ctx, project_name: str, files: List[Dict[str, Any]],
    ...
    result = await deployer.generate_deploy(validated.project_name, code_files, targets)
    return DeployResponse(project=validated.project_name, targets=targets, **result).model_dump()
...
async def vibe_design_tool(ctx, intent: str, template=None,
    ...
    code_result = await vibe_code_tool(ctx=ctx, intent=validated.intent, plan=plan_result.get("plan", {}),
...
async def vibe_benchmark_tool(ctx, iterations: int = 5) -> Dict[str, Any]:
    ...
    for i in range(validated.iterations):
        ...
        auditor = SystemAuditor()
        audit_result = await auditor.audit([CodeFile(**m) for m in mock], ["Production-grade MCP server"])
        elapsed = (_t.time() - t0) * 1000
        score = audit_result["consensus_score"]
        scores.append(score)
        results.append({"iteration": i + 1, "score": score, "recommendation": audit_result["recommendation"], "time_ms": round(elapsed)})
...
async def vibe_build_pro_tool(ctx, intent: str, template=None,
    ...
    code = await vibe_code_tool(ctx=ctx, intent=validated.intent, plan=plan.get("plan", {}),
