...
class TokenBucket:
    ...
    FIX 1: replaced single global asyncio.Lock() with per-identity locks via
    defaultdict. The original design serialised ALL identities through one lock,
    meaning a slow/blocked identity could stall requests from every other user.

    FIX 2: eviction is now triggered on a time-based interval (every 5 minutes)
    rather than only when the dict reaches 1,000 entries. Under slow drip traffic
    from many IPs the dict previously grew unbounded until hitting 1,001.
    """

    _EVICT_INTERVAL = 300  # seconds between eviction sweeps
    ...
    def __init__(self, rate: float = 30.0, burst: int = 10):
        ...
        # FIX: per-identity locks — contention on one identity never blocks others.
        self._locks: defaultdict[str, asyncio.Lock] = defaultdict(asyncio.Lock)
        self._last_eviction: float = time.monotonic()

    async def allow(self, identity: str) -> bool:
        # Time-based eviction: sweep stale entries every _EVICT_INTERVAL seconds.
        now = time.monotonic()
        if now - self._last_eviction > self._EVICT_INTERVAL:
            self._evict_stale()
            self._last_eviction = now

        # FIX: lock is per-identity, not global.
    ...
    def _evict_stale(self):
        ...
        if stale:
            log.debug(f"[RateLimiter] Evicted {len(stale)} stale identities")
...
def audit_tool(func: Callable):
    """Decorator: rate limiting, audit logging, correlation IDs."""
    import functools
